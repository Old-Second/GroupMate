...
test('legacy chat delegates reply input construction to the TypeScript adapter', async () => {
  ...
  assert.match(method, /await buildModelMessageInput\(\{\s*event: e,\s*currentPrompt: prompt\s*\}\)/)
  ...
  assert.ok(authorizationIndex >= 0 && authorizationIndex < inputIndex)
  assert.ok(inputIndex < imageIndex)
