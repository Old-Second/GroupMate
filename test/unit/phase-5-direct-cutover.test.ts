...
test('Phase 5 ordinary chat and BYM have no legacy model fallback', async () => {
  ...
  assert.match(bym, /dist\/runtime\/bym-trigger\.js/)
  assert.match(bym, /hasLeadingAlias:\s*e\.hasAlias === true/)
  assert.match(bym, /recognizeLeadingAlias:\s*Config\.bymRecognizeLeadingAlias/)
  assert.match(bym, /if \(trigger\.prompt === null\) return false/)
  assert.match(bym, /\.handleEphemeral\(e,\s*trigger\.prompt,/)
  assert.doesNotMatch(bym, /e\.msg\?\.includes\(Config\.assistantLabel\)/)
