...
test('production approval survives graceful shutdown, defers under pressure, and finalizes once', async () => {
  ...
  try {
    ...
    if (paused.schemaVersion !== 5) assert.fail('approval checkpoint must use schema v5')
    ...
    if (preserved.schemaVersion !== 5) assert.fail('preserved checkpoint must use schema v5')
    ...
    if (deferred.schemaVersion !== 5) assert.fail('deferred checkpoint must use schema v5')
