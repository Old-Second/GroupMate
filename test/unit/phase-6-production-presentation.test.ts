...
test('production approval survives graceful shutdown, defers under pressure, and finalizes once', async () => {
  ...
  try {
    ...
    if (paused.schemaVersion !== 4) assert.fail('approval checkpoint must use schema v4')
    ...
    if (preserved.schemaVersion !== 4) assert.fail('preserved checkpoint must use schema v4')
    ...
    if (deferred.schemaVersion !== 4) assert.fail('deferred checkpoint must use schema v4')
