...
export const MEMORY_EXTRACTION_JOB_HASH_DOMAIN_V1 =
  'groupmate.memory.extraction-job.v1'

export type MemoryExtractionRequestedModeV1 = 'shadow' | 'automatic'
export type MemoryExtractionPriorityV1 = 'asserted' | 'inferred'
export type MemoryCandidateDerivationV1 = 'stated' | 'inferred'

export interface MemoryExtractionJobV1 {
  ...
  readonly subject: QqIdentitySnapshotV1
  readonly source: MemorySourceV1
  readonly sceneRef: string
  readonly sourceRunRef: string
  readonly sourceModelProfile: string
  readonly requestedMode: MemoryExtractionRequestedModeV1
...
export interface MemoryExtractedCandidateV1 {
  ...
  readonly derivation: MemoryCandidateDerivationV1
  readonly confidence: number
  readonly sensitivity: MemorySensitivityV1
}

export type MemoryExtractorResultV1 =
  | {
      readonly schemaVersion: 1
      readonly status: 'no_op'
      readonly extractorVersion: string
      readonly modelProfile: string
      readonly reason: 'no_durable_fact' | 'insufficient_evidence'
    }
  | {
      readonly schemaVersion: 1
      ...
      readonly extractorVersion: string
      readonly modelProfile: string
      readonly candidates: readonly MemoryExtractedCandidateV1[]
    }

export interface MemoryCandidateProvenanceV1 {
  readonly schemaVersion: 1
  readonly subjectUserId: string
  readonly sourceActorUserIds: readonly string[]
  readonly speakerRelation: 'self' | 'third_party'
  readonly derivation: MemoryCandidateDerivationV1
  readonly sourceIds: readonly string[]
}

export type MemoryCredentialRejectionReasonV1 =
  | 'api_key'
  | 'authorization'
  | 'cookie'
  | 'password'
...
export type MemoryCandidateValueRejectionReasonV1 =
  | 'credential'
  | 'no_durable_value'
  | 'low_confidence'

const MEMORY_KINDS: readonly MemoryKindV1[] = Object.freeze([
  'profile_fact', 'preference', 'relationship', 'group_rule', 'group_culture',
  'task_fact', 'other'
])
const MEMORY_SENSITIVITIES: readonly MemorySensitivityV1[] = Object.freeze([
  'public', 'group', 'personal', 'sensitive'
])
const EXTRACTION_MODES: readonly MemoryExtractionRequestedModeV1[] = Object.freeze([
  'shadow', 'automatic'
])
const EXTRACTION_PRIORITIES: readonly MemoryExtractionPriorityV1[] = Object.freeze([
  'asserted', 'inferred'
])
const CANDIDATE_DERIVATIONS: readonly MemoryCandidateDerivationV1[] = Object.freeze([
  'stated', 'inferred'
])
...
function opaqueId (value: unknown, prefix: string | null = null): string {
...
function assistantReply (value: unknown): string {
  if (!memoryCanonicalTextWithinLimits(
    value,
    MAXIMUM_ASSISTANT_REPLY_BYTES,
    MAXIMUM_ASSISTANT_REPLY_CODE_POINTS
  ) || value.trim() === '') return invalidMemoryValue()
  return value
}

function jobPreimage (
  value: Omit<MemoryExtractionJobV1, 'schemaVersion' | 'jobId'>
): string {
...
function parseJobFields (
  ...
  if (subject.userId !== namespace.scope.subjectUserId) return invalidMemoryValue()
  ...
  if (includeComputed && (
    input.jobId !== jobId || input.namespaceRef !== namespaceRef
  )) return invalidMemoryValue()
...
export function createMemoryExtractionJobV1 (value: unknown): MemoryExtractionJobV1 {
  return parseJobFields(value, false)
}

export function parseMemoryExtractionJobV1 (value: unknown): MemoryExtractionJobV1 {
  return parseJobFields(value, true)
}

export function encodeMemoryExtractionJobV1 (value: unknown): string {
  const wire = JSON.stringify(parseMemoryExtractionJobV1(value))
  return wire
}

export function decodeMemoryExtractionJobV1 (value: unknown): MemoryExtractionJobV1 {
  ...
  if (JSON.stringify(job) !== value) return invalidMemoryValue()
  return job
}
...
function parseCandidate (
...
): MemoryExtractedCandidateV1 {
  const input = inspectMemoryRecord(value, [
    'kind', 'text', 'sourceIds', 'derivation', 'confidence', 'sensitivity'
  ])
  if (!memoryTextWithinLimits(input.text) || input.text.trim() === '') return invalidMemoryValue()
  ...
  if (sourceIds.length === 0 || new Set(sourceIds).size !== sourceIds.length ||
    sourceIds.some(sourceId => sourceId !== job.source.sourceId)) return invalidMemoryValue()
  return Object.freeze({
...
export function parseMemoryExtractorResultV1 (
  value: unknown,
  jobValue: unknown
  ...
  const discriminator = inspectMemoryRecord(
    value,
    ['schemaVersion', 'status', 'extractorVersion', 'modelProfile'],
    ['reason', 'candidates']
  )
  ...
  if (discriminator.status !== 'candidates') return invalidMemoryValue()
  const input = inspectMemoryRecord(value, [
  ...
  if (candidates.length === 0) return invalidMemoryValue()
  return Object.freeze({
    schemaVersion: 1 as const,
...
export function deriveMemoryCandidateProvenanceV1 (
  jobValue: unknown,
  candidateValue: unknown
): MemoryCandidateProvenanceV1 {
  const job = parseMemoryExtractionJobV1(jobValue)
...
export function memoryCredentialRejectionReasonV1 (
  value: unknown
): MemoryCredentialRejectionReasonV1 | null {
  if (typeof value !== 'string') return invalidMemoryValue()
  if (AUTHORIZATION_PATTERN.test(value)) return 'authorization'
  if (COOKIE_PATTERN.test(value)) return 'cookie'
  if (PASSWORD_PATTERN.test(value)) return 'password'
  if (VERIFICATION_CODE_PATTERN.test(value)) return 'verification_code'
...
export function memoryCandidateValueRejectionReasonV1 (
  jobValue: unknown,
  candidateValue: unknown
): MemoryCandidateValueRejectionReasonV1 | null {
  const job = parseMemoryExtractionJobV1(jobValue)
  const candidate = parseCandidate(candidateValue, job)
  ...
  if (candidate.confidence < 0.55) return 'low_confidence'
  return null
}

export function memoryCandidateHashV1 (
  jobValue: unknown,
  candidateValue: unknown
): string {
  const job = parseMemoryExtractionJobV1(jobValue)
...
export function memoryCandidateProvenanceHashV1 (value: unknown): string {
  const input = inspectMemoryRecord(value, [
  ...
  const sourceActorUserIds = inspectMemoryArray(
    input.sourceActorUserIds,
    MEMORY_RESOURCE_LIMITS.sources
  ).map(item => {
    if (typeof item !== 'string' || !/^\d+$/.test(item)) return invalidMemoryValue()
    return item
  })
  const sourceIds = inspectMemoryArray(input.sourceIds, MEMORY_RESOURCE_LIMITS.sources)
    .map(exactHash)
  const canonical = Object.freeze({
    ...
    speakerRelation: enumValue(input.speakerRelation, ['self', 'third_party'] as const),
    derivation: enumValue(input.derivation, CANDIDATE_DERIVATIONS),
    sourceIds: Object.freeze(sourceIds)
  })
  return createHash('sha256')
