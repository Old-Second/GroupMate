...
import {
  ...
  type MemoryLifecycleCanonicalAuthorityRequirementV1
...
interface CreateSqliteMemoryLifecycleMutationAdapterOptionsV1 {
...
interface CommandUsageV1 {
  readonly records: number
  readonly bytes: number
}

interface UsageStateV1 {
  readonly namespace: CoreUsageV1
  readonly generationCommands: CommandUsageV1
  readonly namespaceCommands: CommandUsageV1
  readonly global: CoreUsageV1 & CommandUsageV1 & { readonly namespaceRecords: number }
}

interface UsageDeltaV1 {
...
interface PreparedEventV1 {
  readonly event: ReturnType<typeof createMemoryOutboxEventV1>
...
const RECORD_OPERATIONS = new Set<MutationOperationV1>([
...
function rowValue (row: Row, name: string): SQLOutputValue {
  if (!Object.hasOwn(row, name)) throw new CanonicalLifecycleMutationDataErrorV1()
  return row[name]
}

function exactString (value: SQLOutputValue): string {
  if (typeof value !== 'string') throw new CanonicalLifecycleMutationDataErrorV1()
  return value
}

function exactInteger (value: SQLOutputValue): number {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0 ||
    Object.is(value, -0)) throw new CanonicalLifecycleMutationDataErrorV1()
  return value
}

function positiveInteger (value: SQLOutputValue): number {
  const parsed = exactInteger(value)
  if (parsed === 0) throw new CanonicalLifecycleMutationDataErrorV1()
  return parsed
}

function requireOneChange (changes: number | bigint): void {
  if (changes !== 1 && changes !== 1n) throw new CanonicalLifecycleMutationDataErrorV1()
}
...
function freezeTrustedNow (database: DatabaseSync, now: () => string): string {
  ...
  if (row === undefined) throw new CanonicalLifecycleMutationDataErrorV1()
...
function stableResult (result: MemoryLifecycleResultV1): MemoryLifecycleStableResultV1 {
...
function canonicalRequirement (
  ...
  value: Pick<MemoryRecordV2, 'sensitivity'> | Pick<MemoryProposalV2, 'sensitivity'>
): MemoryLifecycleCanonicalAuthorityRequirementV1 {
  if (namespace.scope.kind === 'group' && value.sensitivity === 'sensitive') return 'elevated'
  return 'ordinary'
}

function groupAdmissionAllows (
  namespace: MemoryNamespaceV1,
  value: Pick<MemoryRecordV2, 'kind' | 'sensitivity' | 'sources'> |
  Pick<MemoryProposalV2, 'kind' | 'sensitivity' | 'sources'>
): boolean {
...
function policyAllows (
  ...
  if (envelope.authority.kind !== 'policy' || evidence.policyRef === null ||
    evidence.policyGeneration === null || !memoryConsentMatchesNamespaceV1(
      proposal.namespace,
  ...
  const bindingAllowed = memoryPolicyCapabilityAllowsBindingV1(
...
function loadNamespace (
  ...
  namespaceRef: string
): { readonly namespace: MemoryNamespaceV1; readonly generation: number; readonly wireBytes: number } | null {
  const row = database.prepare(`
  ...
  const loaded = canonicalWire(
    ...
    wire => parseMemoryNamespaceV1(JSON.parse(wire) as unknown)
  )
...
function loadProposal (
  ...
  generation: number,
  proposalId: string
): { readonly proposal: MemoryProposalV2; readonly wire: string; readonly bytes: number } | null {
  const row = database.prepare(`
  ...
  `).get(namespaceRef, generation, proposalId) as Row | undefined
  if (row === undefined) return null
  const loaded = canonicalWire(
...
function cursorRefForRevision (revision: MemoryRevisionV2): string {
...
function loadHeadRevision (
  database: DatabaseSync,
  namespaceRef: string,
  generation: number,
  memoryId: string
): { readonly revision: MemoryRevisionV2; readonly wire: string; readonly bytes: number } | null {
  const row = database.prepare(`
  ...
  `).get(namespaceRef, generation, memoryId) as Row | undefined
  if (row === undefined) return null
  const loaded = canonicalWire(
...
function loadRevision (
  ...
  const row = database.prepare(`
    ...
    WHERE r.namespace_ref = ? AND r.namespace_generation = ? AND r.memory_id = ?
      AND r.revision = ?
  `).get(namespaceRef, generation, memoryId, revisionNumber) as Row | undefined
  if (row === undefined) return null
  const loaded = canonicalWire(
    rowValue(row, 'revision_wire'),
    rowValue(row, 'revision_wire_bytes'),
...
function coreUsage (row: Row): CoreUsageV1 {
  return Object.freeze({
...
function assertActualNotGreater (actual: Row, stored: Readonly<Record<string, number>>): void {
...
function validateUsageLowerBounds (
  database: DatabaseSync,
  namespaceRef: string,
  generation: number,
  usage: UsageStateV1,
  mutationDelta: Omit<UsageDeltaV1, 'commandRecords' | 'commandBytes'>
): void {
  ...
  if (actual === undefined) throw new CanonicalLifecycleMutationDataErrorV1()
  assertActualNotGreater(actual, {
    pending_proposal_records: addExact(
      usage.namespace.pendingProposalRecords,
      mutationDelta.pendingProposalRecords
    ),
    active_memory_records: addExact(
      usage.namespace.activeMemoryRecords,
      mutationDelta.activeMemoryRecords
    ),
    retained_revision_records: addExact(
  ...
  `).get() as Row | undefined
  if (payloadCoverage === undefined) throw new CanonicalLifecycleMutationDataErrorV1()
  ...
  const globalActual = database.prepare(`
    SELECT (SELECT count(*) FROM namespaces) AS namespace_records,
      ...
      (SELECT coalesce(sum(result_wire_bytes), 0) FROM lifecycle_commands)
        AS lifecycle_command_logical_bytes
  `).get() as Row | undefined
  if (globalActual === undefined) throw new CanonicalLifecycleMutationDataErrorV1()
  ...
  `).get(namespaceRef) as Row | undefined
  if (namespaceCommands === undefined) throw new CanonicalLifecycleMutationDataErrorV1()
...
function projectedUsage (
  ...
  const addCore = (value: CoreUsageV1): CoreUsageV1 => Object.freeze({
...
function storeUsage (
  ...
  `).run(
    ...
    usage.generationCommands.records,
    usage.generationCommands.bytes,
    nowMs,
    namespaceRef,
    generation
  ).changes)
  ...
  `).run(
    usage.namespaceCommands.records,
    usage.namespaceCommands.bytes,
    nowMs,
    namespaceRef
  ).changes)
...
function nextOutboxSequence (database: DatabaseSync, count: number): number {
  ...
  if (!Number.isSafeInteger(next + count - 1)) throw new CanonicalLifecycleMutationDataErrorV1()
  return next
}
...
function insertEvent (database: DatabaseSync, prepared: PreparedEventV1): void {
...
function storedResult (
  ...
  resultRevision: number,
  resultHash: string,
  status: 'stored' | 'unchanged' = 'stored'
...
function existingLedgerResult (
  ...
  if (storedCommandHash !== commandHash || storedOperation !== wire.operation) return 'conflict'
  ...
  if ((result.status === 'stored' || result.status === 'unchanged') && 'resultRef' in result) {
    if (result.receiptHash !== mutationReceiptHash(
      commandHash,
      result.resultRef,
...
function recordEvent (
  ...
  return prepareEvent(
    sequence,
    revision.record.namespaceRef,
    revision.record.namespaceGeneration,
    'record',
    revision.memoryId,
    revision.revision,
    'record_upserted',
...
function insertRevisionEvidence (
  database: DatabaseSync,
  material: MemoryRevisionChangeMaterialV1
): number {
  ...
  `).run(
    evidence.namespaceRef,
    evidence.namespaceGeneration,
    evidence.evidenceId,
    evidence.memoryId,
    evidence.revision,
    ...
    wire,
    bytes,
    Date.parse(evidence.changedAt)
  ).changes)
  return bytes
}

function updateHead (database: DatabaseSync, revision: MemoryRevisionV2): void {
  requireOneChange(database.prepare(`
...
function insertConsentEvidence (
  database: DatabaseSync,
  evidence: MemoryConsentEvidenceV1
): number {
  ...
  `).run(
    ...
    wire,
    bytes,
    Date.parse(evidence.approvedAt)
  ).changes)
  return bytes
}
...
function updateProposal (
  database: DatabaseSync,
  before: { readonly proposal: MemoryProposalV2; readonly bytes: number },
  ...
  `).run(
    after.revision,
    after.state,
    after.decision === null ? null : Date.parse(after.decision.decidedAt),
    after.decision?.resultingMemoryId ?? null,
    after.decision?.resultingMemoryRevision ?? null,
    after.decision?.resultingRevisionHash ?? null,
    wire,
    bytes,
    after.namespaceRef,
    after.namespaceGeneration,
    after.proposalId
  ).changes)
  return bytes - before.bytes
}

function executeProposalCreate (
  database: DatabaseSync,
  envelope: MemoryLifecycleAuthorizationEnvelopeV1,
  wire: MemoryLifecycleCommandWireV1,
  commandHash: string,
  ...
  const proposal = envelope.command.material as MemoryProposalV2
  ...
  if (!actorAllows(
    ...
    'none',
    'none',
    proposal.initiatedByActorRef,
    freshNow
  ) || !groupAdmissionAllows(proposal.namespace, proposal)) {
    return denial('proposal.create', commandHash, 'authority')
  }
  ...
  if (existingNamespace !== null && (
    memoryNamespaceWireV1(existingNamespace.namespace) !== memoryNamespaceWireV1(proposal.namespace)
  )) throw new CanonicalLifecycleMutationDataErrorV1()
  ...
  if (proposal.intent.kind === 'correction') {
    const head = loadHeadRevision(
      ...
      namespaceRef,
      wire.expectedNamespaceGeneration,
      proposal.intent.targetMemoryId
    )
    if (head === null) {
      if (existingNamespace === null) return denial('proposal.create', commandHash, 'authority')
      const result = resultFor('proposal.create', commandHash, {
        status: 'conflict', category: 'revision'
      })
      return commitStableResult(
        database, wire, commandHash, aggregateRef, stableResult(result),
        ZERO_DELTA, [], freshNow
      )
    }
  ...
  if (existing !== null) {
    ...
    const unchanged = storedResult(
      'proposal.create', commandHash, proposal.proposalId, 1, proposalResultHash(proposalWire), 'unchanged'
    )
    return commitStableResult(
      database, wire, commandHash, aggregateRef, unchanged,
      ZERO_DELTA, [], freshNow
    )
  }
  requireOneChange(database.prepare(`
  ...
  const result = storedResult(
    'proposal.create', commandHash, proposal.proposalId, 1, proposalResultHash(proposalWire)
  )
...
function proposalDecisionAfterImage (
  operation: Exclude<MutationOperationV1, 'proposal.create' | 'proposal.approve' |
  'record.correct' | 'record.renew' | 'record.changeConflict'>,
  wire: MemoryLifecycleCommandWireV1,
  proposal: MemoryProposalV2,
  freshNow: string
...
function proposalDecisionAuthorityAllows (
  envelope: MemoryLifecycleAuthorizationEnvelopeV1,
  wire: MemoryLifecycleCommandWireV1,
  operation: 'proposal.approve' | 'proposal.reject' | 'proposal.withdraw' | 'proposal.expire',
  namespace: MemoryNamespaceV1,
  proposal: MemoryProposalV2 | null,
  freshNow: string
): boolean {
  if (Date.parse(wire.occurredAt) > Date.parse(freshNow)) return false
  if (operation === 'proposal.expire') return maintenanceAllowsExpire(envelope, wire, freshNow)
  if (operation === 'proposal.approve') {
    ...
    if (evidence.approvedAt !== wire.occurredAt || proposal === null ||
      !groupAdmissionAllows(namespace, proposal)) return false
    if (evidence.evidenceKind === 'explicit') {
      return actorAllows(
        envelope,
        wire,
        'approve',
        canonicalRequirement(namespace, proposal),
        canonicalRequirement(namespace, proposal),
        proposal.initiatedByActorRef,
        freshNow
      )
  ...
  if (operation === 'proposal.withdraw' && proposal === null) return false
  return actorAllows(
    envelope,
    wire,
    operation === 'proposal.reject' ? 'reject' : 'withdraw_own_proposal',
    proposal === null ? 'none' : canonicalRequirement(namespace, proposal),
    'none',
    proposal?.initiatedByActorRef ?? null,
    freshNow
  )
}

function executeProposalDecision (
  database: DatabaseSync,
  envelope: MemoryLifecycleAuthorizationEnvelopeV1,
  wire: MemoryLifecycleCommandWireV1,
  commandHash: string,
  ...
  const operation = wire.operation as 'proposal.approve' | 'proposal.reject' |
  'proposal.withdraw' | 'proposal.expire'
  const aggregateRef = wire.aggregateRef as string
  ...
  if (!proposalDecisionAuthorityAllows(
    envelope,
    wire,
    operation,
    namespace.namespace,
    loaded?.proposal ?? null,
    freshNow
  )) return denial(operation, commandHash, 'authority')
  const replay = existingLedgerResult(database, wire, commandHash, aggregateRef)
  if (replay === 'conflict') {
  ...
  if (operation === 'proposal.approve') {
    ...
    const head = correctionIntent !== null
      ? loadHeadRevision(
          ...
          wire.namespaceRef,
          wire.expectedNamespaceGeneration,
          correctionIntent.targetMemoryId
        )
      : null
    ...
    if (head !== null && (
      head.revision.revision !== correctionIntent?.targetRevision ||
    ...
    if (encodeMemoryConsentEvidenceV1(bundle.consentEvidence) !==
      encodeMemoryConsentEvidenceV1(evidence)) throw new CanonicalLifecycleMutationDataErrorV1()
    ...
    const result = storedResult(
      ...
      bundle.proposal.proposalId,
      bundle.proposal.revision,
      proposalResultHash(encodeMemoryProposalV2(bundle.proposal))
    )
    const committed = commitStableResult(
      database,
      wire,
...
function executeRecordMutation (
  database: DatabaseSync,
  envelope: MemoryLifecycleAuthorizationEnvelopeV1,
  wire: MemoryLifecycleCommandWireV1,
  commandHash: string,
  ...
  const operation = wire.operation as 'record.correct' | 'record.renew' | 'record.changeConflict'
  const aggregateRef = wire.aggregateRef as string
  ...
  if (head === null) {
    if (Date.parse(wire.occurredAt) > Date.parse(freshNow) || !actorAllows(
      envelope,
      wire,
      action,
      'none',
  ...
  const before = loadRevision(
    database,
    wire.namespaceRef,
    wire.expectedNamespaceGeneration,
    aggregateRef,
    wire.expectedRevision as number
  )
  if (before === null) {
    if (Date.parse(wire.occurredAt) > Date.parse(freshNow) || !actorAllows(
      envelope,
      wire,
      action,
      canonicalRequirement(namespace.namespace, head.revision.record),
      'none',
      null,
      freshNow
    )) return denial(operation, commandHash, 'authority')
  ...
  const material = envelope.command.material as MemoryRevisionChangeMaterialV1
  ...
  if (bundle.revision.changedAt !== wire.occurredAt ||
    ...
    !groupAdmissionAllows(namespace.namespace, bundle.revision.record) || !actorAllows(
      envelope,
      wire,
      action,
      canonicalRequirement(namespace.namespace, before.revision.record),
      canonicalRequirement(namespace.namespace, bundle.revision.record),
      null,
      freshNow
    )) return denial(operation, commandHash, 'authority')
  const replay = existingLedgerResult(database, wire, commandHash, aggregateRef)
...
function failureResult (
  ...
  if (primary === 5 || primary === 6) return resultFor(operation, commandHash, {
    status: 'unavailable', category: 'busy', retryable: true
  })
  if (primary === 10) return resultFor(operation, commandHash, {
    status: 'unavailable', category: 'io', retryable: true
  })
  if (primary === 11 || primary === 26) return resultFor(operation, commandHash, {
...
export function createSqliteMemoryLifecycleMutationAdapterV1 (
  options: CreateSqliteMemoryLifecycleMutationAdapterOptionsV1
  ...
  if (options === null || typeof options !== 'object' || utilTypes.isProxy(options) ||
    ...
    utilTypes.isProxy(options.now)) throw new TypeError('invalid lifecycle mutation adapter options')
  const { database, now } = options
  return Object.freeze({
    execute: async (envelope: MemoryLifecycleAuthorizationEnvelopeV1) => {
      let operation: MutationOperationV1 = 'proposal.create'
      let commandHash = '0'.repeat(64)
      try {
        const command = parseMemoryLifecycleCommandV1(envelope.command)
        const wire = decodeMemoryLifecycleCommandWireV1(command.wire)
        if (SUPPORTED_OPERATIONS.has(wire.operation)) operation = wire.operation as MutationOperationV1
