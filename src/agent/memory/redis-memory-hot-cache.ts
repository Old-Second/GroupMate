...
export interface RedisMemoryHotCacheClient {
...
export interface RedisMemoryHotCacheOptions {
  readonly client: RedisMemoryHotCacheClient
}
...
export const MEMORY_HOT_CACHE_RECORD_FIELD_HASH_DOMAIN =
...
export function memoryHotCacheRecordFieldV1 (
  head: Readonly<{
  ...
  if (typeof input.namespaceRef !== 'string' || !/^[0-9a-f]{64}$/.test(input.namespaceRef) ||
    ...
    input.memoryId.length === 'memory:'.length) return invalidMemoryValue()
...
export function memoryHotCacheNamespaceFieldV1 (namespaceRef: string): string {
  if (!/^[0-9a-f]{64}$/.test(namespaceRef)) return invalidMemoryValue()
  return `g:${namespaceRef}`
}

export function memoryHotCacheHeadFieldV1 (recordField: string): string {
  if (!/^[0-9a-f]{64}$/.test(recordField)) return invalidMemoryValue()
  return `h:${recordField}`
}

export function memoryHotCacheHeadValueV1 (revision: number, recordField: string): string {
  if (!/^[0-9a-f]{64}$/.test(recordField)) return invalidMemoryValue()
...
export function memoryHotCacheIndexEntryBytesV1 (field: string): number {
  return utf8Bytes(field) + 16
}

export function memoryHotCacheMetadataEntryBytesV1 (field: string, value: string): number {
...
function redisStringArray (value: unknown, length: number): readonly string[] | null {
  if (!Array.isArray(value) || utilTypes.isProxy(value)) return null
  const descriptors = Object.getOwnPropertyDescriptors(value)
  const keys = Reflect.ownKeys(value)
  if (keys.length !== length + 1 || !Object.hasOwn(descriptors, 'length')) return null
  const result: string[] = []
...
function exactStatus (value: unknown): string | null {
  return typeof value === 'string' ? value : null
}

export class RedisMemoryHotCache implements MemoryHotCachePortV1 {
  readonly #evaluate!: RedisMemoryHotCacheClient['eval']
  readonly #port!: MemoryHotCachePortV1

  constructor (optionsValue: RedisMemoryHotCacheOptions) {
    const options = inspectMemoryRecord(optionsValue, ['client'])
    if (options.client === null || typeof options.client !== 'object' ||
      utilTypes.isProxy(options.client)) return invalidMemoryValue()
    const client = options.client as object
    ...
    const evaluate = descriptor.value as RedisMemoryHotCacheClient['eval']
  ...
  async execute (request: unknown, signal?: AbortSignal): Promise<MemoryHotCacheResultV1> {
    return await this.#port.execute(request, signal)
  }

  async #eval (argumentsValue: readonly string[]): Promise<unknown> {
    try {
  ...
  async #executeAdapter (
    ...
    if (request.operation === 'record.put') return await this.#put(request.record)
    if (request.operation === 'record.get') return await this.#get(request.head)
  ...
  async #put (record: CanonicalMemoryRecordV1): Promise<unknown> {
    ...
    if (result === 'stored' || result === 'unchanged') return Object.freeze({ status: result })
    if (result === 'stale' || result === 'expired' || result === 'capacity') {
      return Object.freeze({
        status: 'skipped' as const,
        reason: result === 'capacity' ? 'capacity' as const : result
      })
    }
    return Object.freeze({ status: 'unavailable' as const })
  }

  async #get (head: MemoryHotCacheHeadV1): Promise<unknown> {
    ...
    if (status === 'missing') return Object.freeze({ status: 'miss' as const, reason: 'not_found' as const })
    if (status === 'expired' || status === 'stale' || status === 'mismatch' || status === 'corrupt') {
      return Object.freeze({ status: 'miss' as const, reason: status })
    }
    ...
    if (!matches) {
      await this.#deleteCorrupt(field, headField, wire)
      return Object.freeze({ status: 'miss' as const, reason: 'mismatch' as const })
    }
    const confirmed = exactStatus(await this.#eval([
      'confirm_hit', field, generationField, headField, generationRaw, headValue, wire,
      String(Date.parse(record.retention.validUntil))
    ]))
    if (confirmed === 'hit') return Object.freeze({ status: 'hit' as const, record })
    if (confirmed === 'expired' || confirmed === 'stale' ||
      confirmed === 'mismatch' || confirmed === 'corrupt') {
      return Object.freeze({ status: 'miss' as const, reason: confirmed })
    }
    return Object.freeze({ status: 'unavailable' as const })
  }

  async #deleteCorrupt (field: string, headField: string, wire: string): Promise<void> {
    await this.#eval(['delete_corrupt', field, headField, wire])
  }

  async #recordInvalidate (
    request: Extract<MemoryHotCacheRequestV1, { readonly operation: 'record.invalidate' }>
  ): Promise<unknown> {
    const field = memoryHotCacheRecordFieldV1({
    ...
    return result === 'invalidated' || result === 'unchanged' || result === 'missing'
      ? Object.freeze({ status: result === 'missing' ? 'unchanged' as const : result })
      : Object.freeze({ status: 'unavailable' as const })
  }

  async #namespaceInvalidate (
    request: Extract<MemoryHotCacheRequestV1, { readonly operation: 'namespace.invalidate' }>
  ): Promise<unknown> {
    const result = exactStatus(await this.#eval([
  ...
  async #usage (): Promise<unknown> {
    ...
    const [recordCount, generationCount, recordEntryBytes, expiryIndexBytes,
      lruIndexBytes, dynamicMetadataBytes, staticBytes, totalLogicalBytes] = values
    return Object.freeze({
