...
import {
  MEMORY_INITIAL_RETENTION_MAX_DAYS_V2,
...
export const MEMORY_PROPOSAL_ID_HASH_DOMAIN_V2 =
  'groupmate.memory.proposal-id.v2'
export const MEMORY_PLANNED_MEMORY_ID_HASH_DOMAIN_V2 =
  'groupmate.memory.planned-memory-id.v2'
export const MEMORY_CONSENT_EVIDENCE_ID_HASH_DOMAIN_V1 =
  'groupmate.memory.consent-evidence-id.v1'
export const MEMORY_REVISION_EVIDENCE_ID_HASH_DOMAIN_V1 =
...
export const MEMORY_INITIAL_RETENTION_DAYS_BY_KIND_V2 = Object.freeze({
  ...
  task_fact: 90,
  other: 180
} as const satisfies Readonly<Record<MemoryKindV1, number>>)
...
export type MemoryLifecycleBuilderOperationV1 =
  typeof MEMORY_LIFECYCLE_BUILDER_OPERATIONS_V1[number]

export interface MemoryLifecycleDeterministicReferenceInputV1 {
  readonly commandRef: string
  readonly namespaceRef: MemoryNamespaceRefV1
  readonly namespaceGeneration: number
  readonly operation: MemoryLifecycleBuilderOperationV1
}

export interface MemoryPlannedMemoryIdInputV2
  extends MemoryLifecycleDeterministicReferenceInputV1 {
  readonly proposalId: string
  readonly intent: MemoryProposalIntentV2
}

export interface MemoryConsentEvidenceIdInputV1
  extends MemoryLifecycleDeterministicReferenceInputV1 {
  readonly proposalId: string
  readonly consentTargetHash: string
}

export interface MemoryRevisionEvidenceIdInputV1
  extends MemoryLifecycleDeterministicReferenceInputV1 {
...
export interface MemoryProposalLifecycleProjectionV2 {
  readonly logicalState:
    | MemoryProposalV2['state']
    | 'expired_due'
  readonly approvalCutoff: string
  readonly approveEligible: boolean
  readonly fullWirePurgeAt: string
  readonly fullWireReadable: boolean
}

export interface MemoryRecordLifecycleProjectionV2 {
  readonly state: 'current' | 'expired' | 'purge_due'
  readonly retrievalEligible: boolean
  readonly controlContentReadable: boolean
  readonly renewEligible: boolean
}

export interface MemoryProposalApprovalBundleV1 {
...
export interface MemoryRevisionChangeBundleV1 {
  readonly schemaVersion: 1
...
export interface MemoryCorrectionProposalApprovalBundleV1 {
  readonly schemaVersion: 1
  readonly kind: 'correction_proposal_approval_v1'
  readonly proposal: MemoryProposalV2
  readonly consentEvidence: MemoryConsentEvidenceV1
  readonly revisionEvidence: MemoryRevisionEvidenceV1
  readonly record: MemoryRecordV2
  readonly revision: MemoryRevisionV2
}

const MEMORY_KINDS = Object.freeze(Object.keys(
...
const RECORD_CANONICAL_FIELDS = Object.freeze([
  ...
  'contentHash'
] as const satisfies readonly (keyof MemoryRecordV2)[])

function canonicalEqual (left: unknown, right: unknown): boolean {
  return JSON.stringify(left) === JSON.stringify(right)
}

function assertDirectSaveProposalV2 (proposal: MemoryProposalV2): void {
...
function parseBuilderOperation (value: unknown): MemoryLifecycleBuilderOperationV1 {
  if (typeof value !== 'string' ||
    !MEMORY_LIFECYCLE_BUILDER_OPERATIONS_V1.includes(
      value as MemoryLifecycleBuilderOperationV1
    )) return invalidMemoryValue()
  return value as MemoryLifecycleBuilderOperationV1
}

function operationAllowed<T extends MemoryLifecycleBuilderOperationV1> (
  operation: MemoryLifecycleBuilderOperationV1,
  allowed: readonly T[]
): operation is T {
  return (allowed as readonly MemoryLifecycleBuilderOperationV1[]).includes(operation)
}
...
function parseDeterministicReferenceInput (
  value: unknown
  ...
  return Object.freeze({
    ...
    namespaceGeneration: parseMemoryLifecyclePositiveIntegerV1(
...
function deterministicReferencePreimage (
  value: MemoryLifecycleDeterministicReferenceInputV1
): string {
...
function deriveReference (
  ...
  return `${prefix}${memoryLifecycleDomainHashV1(
    domain,
    deterministicReferencePreimage(value)
  )}`
}

export function deriveMemoryProposalIdV2 (value: unknown): string {
...
export function deriveMemoryPlannedMemoryIdV2 (value: unknown): string {
  ...
  if (proposalId !== deriveMemoryProposalIdV2(input)) return invalidMemoryValue()
  const intent = parseMemoryProposalIntentV2(raw.intent)
  if (intent.kind === 'correction') return intent.targetMemoryId
  return `memory:${memoryLifecycleDomainHashV1(
...
export function deriveMemoryConsentEvidenceIdV1 (value: unknown): string {
  const raw = inspectMemoryRecord(value, [
    'commandRef', 'namespaceRef', 'namespaceGeneration', 'operation',
  ...
  return `evidence:${memoryLifecycleDomainHashV1(
...
export function deriveMemoryRevisionEvidenceIdV1 (value: unknown): string {
  const raw = inspectMemoryRecord(value, [
...
function parseMemoryKind (value: unknown): MemoryKindV1 {
  if (typeof value !== 'string' || !MEMORY_KINDS.includes(value as MemoryKindV1)) {
    return invalidMemoryValue()
  }
  return value as MemoryKindV1
}

function parseNonNegativeInstant (value: unknown): string {
  let instant: string
  try {
  ...
  if (Date.parse(instant) < 0) return invalidMemoryValue()
  return instant
}

function addLifecycleDays (instant: unknown, days: unknown): string {
  try {
    return parseNonNegativeInstant(addMemoryLifecycleDaysV1(
      parseNonNegativeInstant(instant),
      days
    ))
  } catch {
    return invalidMemoryValue()
...
function assertSourceDates (sources: readonly MemorySourceV1[]): void {
  for (const source of sources) parseNonNegativeInstant(source.observedAt)
}

function assertProposalDates (proposal: MemoryProposalV2): void {
  ...
  if (proposal.decision !== null) parseNonNegativeInstant(proposal.decision.decidedAt)
  assertSourceDates(proposal.sources)
}

function assertRecordDates (record: MemoryRecordV2): void {
  ...
  if (record.validity.validFrom !== null) parseNonNegativeInstant(record.validity.validFrom)
  assertSourceDates(record.sources)
}

function assertMaterialLimit<T> (value: T): T {
...
export function buildMemoryInitialRetentionV2 (value: unknown): MemoryRetentionV1 {
  ...
  const purgeAt = addLifecycleDays(
    validUntil,
    MEMORY_RETENTION_PURGE_GRACE_DAYS_V2
  )
...
export function buildMemoryRenewalRetentionV2 (value: unknown): MemoryRetentionV1 {
  const input = inspectMemoryRecord(value, [
  ...
  if (Date.parse(freshNow) >= Date.parse(oldPurgeAt) ||
    Date.parse(newValidUntil) <= Date.parse(oldValidUntil)) return invalidMemoryValue()
  ...
  if (Date.parse(newValidUntil) < Date.parse(minimum) ||
    Date.parse(newValidUntil) > Date.parse(maximum)) return invalidMemoryValue()
  return Object.freeze({
...
export function projectMemoryProposalLifecycleV2 (
  proposalValue: unknown,
  freshNowValue: unknown
): MemoryProposalLifecycleProjectionV2 {
  ...
  const deadline = addLifecycleDays(
    proposal.proposedAt,
    MEMORY_PROPOSAL_DEADLINE_DAYS_V2
  )
...
export function projectMemoryRecordLifecycleV2 (
  recordValue: unknown,
  freshNowValue: unknown
): MemoryRecordLifecycleProjectionV2 {
...
function referenceFromFields (input: Readonly<Record<string, unknown>>) {
...
function assertReferenceBinding (
  reference: MemoryLifecycleDeterministicReferenceInputV1,
  namespaceRef: MemoryNamespaceRefV1,
  namespaceGeneration: number
): void {
  if (reference.namespaceRef !== namespaceRef ||
    reference.namespaceGeneration !== namespaceGeneration) return invalidMemoryValue()
}

export function buildMemoryProposalDraftV2 (value: unknown): MemoryProposalV2 {
  const input = inspectMemoryRecord(value, [
  ...
  if (intent.kind !== 'create') return invalidMemoryValue()
  const kind = parseMemoryKind(input.kind)
  const proposedAt = parseNonNegativeInstant(input.proposedAt)
...
export function buildMemoryCorrectionProposalDraftV2 (value: unknown): MemoryProposalV2 {
  const input = inspectMemoryRecord(value, [
    'commandRef', 'operation', 'namespaceRef', 'namespaceGeneration',
  ...
  const reference = referenceFromFields(input)
  if (reference.operation !== 'proposal.create') return invalidMemoryValue()
  const namespace = parseMemoryNamespaceV1(input.namespace)
  const namespaceRef = memoryNamespaceRefV1(namespace)
  assertReferenceBinding(reference, namespaceRef, reference.namespaceGeneration)
...
function proposalAfterDecision (
  proposal: MemoryProposalV2,
  state: Exclude<MemoryProposalV2['state'], 'pending'>,
  decision: MemoryProposalV2['decision']
): MemoryProposalV2 {
...
export function buildMemoryProposalDecisionV2 (value: unknown): MemoryProposalV2 {
  const input = inspectMemoryRecord(value, [
...
function parseProposalApprovalBundle (
  value: unknown
...
function proposalImmutableFieldsMatch (
  before: MemoryProposalV2,
  after: MemoryProposalV2
): boolean {
...
export function assertMemoryProposalApprovalBundleV1 (
  pendingProposalValue: unknown,
  bundleValue: unknown
): MemoryProposalApprovalBundleV1 {
  ...
  const approvalProjection = projectMemoryProposalLifecycleV2(
    pending,
    bundle.consentEvidence.approvedAt
  )
  assertMemoryCreateApprovalResultBindingV2(
    bundle.proposal,
    bundle.consentEvidence,
    bundle.record,
    bundle.revision
  )
...
export function buildMemoryProposalApprovalBundleV1 (
  value: unknown
): MemoryProposalApprovalBundleV1 {
  const input = inspectMemoryRecord(value, [
    'commandRef', 'operation', 'namespaceRef', 'namespaceGeneration', 'proposal',
  ...
  if (proposal.state !== 'pending' || proposal.revision !== 1 ||
    proposal.decision !== null || proposal.intent.kind !== 'create') return invalidMemoryValue()
  assertReferenceBinding(reference, proposal.namespaceRef, proposal.namespaceGeneration)
  if (reference.operation === 'proposal.createAndApprove') {
    assertDirectSaveProposalV2(proposal)
  ...
  const freshNow = parseNonNegativeInstant(input.freshNow)
  const projection = projectMemoryProposalLifecycleV2(proposal, freshNow)
  if (!projection.approveEligible || !projection.fullWireReadable) return invalidMemoryValue()
  const approvedByActorRef = parseMemoryLifecycleHashedRefV1(
    input.approvedByActorRef,
    'actor:'
  )
  if (reference.operation === 'proposal.createAndApprove' && (
    proposal.proposedAt !== freshNow ||
  ...
  if (consentEvidence.source !== null) parseNonNegativeInstant(consentEvidence.source.observedAt)
  if (reference.operation === 'proposal.createAndApprove' &&
    !canonicalEqual(proposal.sources[0], consentEvidence.source)) return invalidMemoryValue()
...
function recordAfterChange (
  ...
  const changed = <K extends keyof typeof changes> (
    key: K,
    fallback: NonNullable<(typeof changes)[K]>
  ): unknown => Object.hasOwn(changes, key) ? changes[key] : fallback
...
function parseCorrectionProposalApprovalBundle (
  value: unknown
): MemoryCorrectionProposalApprovalBundleV1 {
  const input = inspectMemoryRecord(value, [
  ...
  return assertMaterialLimit(Object.freeze({
    schemaVersion: 1 as const,
    kind: 'correction_proposal_approval_v1' as const,
    proposal: parseMemoryProposalV2(input.proposal),
    consentEvidence: parseMemoryConsentEvidenceV1(input.consentEvidence),
...
export function assertMemoryCorrectionProposalApprovalBundleV1 (
  pendingProposalValue: unknown,
  beforeRevisionValue: unknown,
  bundleValue: unknown
): MemoryCorrectionProposalApprovalBundleV1 {
  const pending = parseMemoryProposalV2(pendingProposalValue)
...
export function buildMemoryCorrectionProposalApprovalBundleV1 (
  value: unknown
): MemoryCorrectionProposalApprovalBundleV1 {
  const input = inspectMemoryRecord(value, [
    'commandRef', 'operation', 'namespaceRef', 'namespaceGeneration', 'proposal',
  ...
  const reference = referenceFromFields(input)
  if (reference.operation !== 'proposal.approve') return invalidMemoryValue()
  ...
  if (consentEvidence.source !== null) parseNonNegativeInstant(consentEvidence.source.observedAt)
  const record = recordAfterChange(
    before.record,
    before.revision + 1,
    freshNow,
    {
  ...
  return assertMemoryCorrectionProposalApprovalBundleV1(
    pending,
    before,
    Object.freeze({
      schemaVersion: 1 as const,
...
function parseRevisionChangeBundle (value: unknown): MemoryRevisionChangeBundleV1 {
  const input = inspectMemoryRecord(value, [
...
function expectedRevisionOperation (
  operation: typeof REVISION_EVIDENCE_OPERATIONS[number]
): MemoryRevisionV2['operation'] {
  if (operation === 'record.correct') return 'corrected'
  if (operation === 'record.renew') return 'retention_changed'
  return 'conflict_changed'
}

function assertRecordFieldDelta (
  before: MemoryRecordV2,
  after: MemoryRecordV2,
  operation: Exclude<MemoryRevisionV2['operation'], 'created'>
): void {
  const allowed = new Set<keyof MemoryRecordV2>([
    'revision', 'updatedAt', 'contentHash',
    ...(operation === 'corrected'
      ? ['text', 'confidence', 'validity', 'conflict', 'supersedes'] as const
      : operation === 'retention_changed'
        ? ['retention'] as const
        : ['validity', 'conflict'] as const)
  ])
  ...
  const businessFields: readonly (keyof MemoryRecordV2)[] = operation === 'corrected'
...
export function assertMemoryRevisionChangeBundleV1 (
  beforeRevisionValue: unknown,
...
function buildRevisionChange (
  value: {
    readonly reference: MemoryLifecycleDeterministicReferenceInputV1
    readonly before: MemoryRevisionV2
  ...
  if (evidence.source !== null) parseNonNegativeInstant(evidence.source.observedAt)
  const revision = createMemoryRevisionV2({
...
function parseRevisionBuilderBase (
  input: Readonly<Record<string, unknown>>,
  expectedOperationValue: typeof REVISION_EVIDENCE_OPERATIONS[number]
): {
  readonly reference: MemoryLifecycleDeterministicReferenceInputV1
  readonly before: MemoryRevisionV2
  readonly changedByActorRef: string
  ...
  const reference = referenceFromFields(input)
  if (reference.operation !== expectedOperationValue) return invalidMemoryValue()
  const before = parseMemoryRevisionV2(input.beforeRevision)
  assertRecordDates(before.record)
  assertReferenceBinding(
    reference,
    before.record.namespaceRef,
    before.record.namespaceGeneration
  )
  const freshNow = parseNonNegativeInstant(input.freshNow)
...
export function buildMemoryCorrectionBundleV1 (
  value: unknown
): MemoryRevisionChangeBundleV1 {
  const input = inspectMemoryRecord(value, [
...
export function buildMemoryRenewalBundleV1 (
  value: unknown
): MemoryRevisionChangeBundleV1 {
  const input = inspectMemoryRecord(value, [
    'commandRef', 'operation', 'namespaceRef', 'namespaceGeneration', 'beforeRevision',
...
export function buildMemoryConflictChangeBundleV1 (
  value: unknown
): MemoryRevisionChangeBundleV1 {
  const input = inspectMemoryRecord(value, [
    'commandRef', 'operation', 'namespaceRef', 'namespaceGeneration', 'beforeRevision',
  ...
  const record = recordAfterChange(
    ...
    base.before.revision + 1,
    base.freshNow,
    { validity: input.validity, conflict: input.conflict }
  )
  return buildRevisionChange({
    ...base,
    evidenceKind: input.evidenceKind,
...
export function requireMemoryV1ToV2ManifestInventoryV1 (
  value: unknown
  ...
  const revisionEvidenceValues = inspectMemoryArray(
    input.revisionEvidence,
    MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleMigrationRevisionBindings
  )
  const manifest = requireMemoryV1ToV2AggregateManifestV1(
    input.legacyAggregateCount,
    input.manifest,
    input.expectation
  )
  if (manifest === null) {
  ...
  let consentEvidence: MemoryConsentEvidenceV1 | null = null
  ...
  if ((manifest.consentEvidenceId === null) !== (consentEvidence === null) ||
    (consentEvidence !== null && (
      consentEvidence.evidenceId !== manifest.consentEvidenceId ||
  ...
  for (let index = 0; index < revisionEvidence.length; index += 1) {
    ...
    if (evidence.namespaceRef !== manifest.namespaceRef ||
      ...
      evidence.revision !== index + 2) return invalidMemoryValue()
  }
