...
import type { AgentErrorCode, SerializedAgentError } from './error.js'

const agentErrorCodes: readonly AgentErrorCode[] = [
  'invalid_request',
  'invalid_session',
  'storage_unavailable',
...
export type AgentResult =
  | { readonly status: 'completed'; readonly output: AgentMessage }
...
export function parseAgentResult (value: unknown): AgentResult {
  ...
  if (result.status === 'completed') parseAgentMessage(result.output)
  if (result.status === 'failed') {
    ...
    if (Object.keys(error).some(key => !keys.includes(key))) throw new TypeError('agent result error contains unknown keys')
    if (!agentErrorCodes.includes(error.code as AgentErrorCode) || typeof error.stage !== 'string' || typeof error.retryable !== 'boolean' || typeof error.userMessage !== 'string') {
