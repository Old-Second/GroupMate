...
function timestampOrNow (value: unknown, now: Date, label: string): string {
  return value === undefined ? now.toISOString() : isoTimestamp(value, label)
}

function turnCount (value: unknown): number {
  if (!Number.isSafeInteger(value) || Number(value) < 0) throw new TypeError('turn count must be a non-negative safe integer')
  return Number(value)
}

function messages (value: unknown): readonly Record<string, unknown>[] {
  if (!Array.isArray(value)) throw new TypeError('session messages must be an array')
  for (const message of value) record(message, 'session message')
  return value as readonly Record<string, unknown>[]
}

function sameScope (left: ConversationScope, right: ConversationScope): boolean {
  if (left.kind !== right.kind) return false
  if (left.kind === 'private' && right.kind === 'private') return left.userId === right.userId
...
function parseScope (value: unknown): ConversationScope {
...
function parseState (value: unknown): LegacyConversationState {
  ...
  if (state.conversationId !== undefined) parsed.conversationId = text(state.conversationId, 'conversation ID')
  if (state.parentMessageId !== undefined) parsed.parentMessageId = text(state.parentMessageId, 'parent message ID')
  return parsed
}

function parseCanonicalRecord (value: unknown, address: SessionAddress): SessionRecord<LegacyConversationState> {
  ...
  if (canonical.schemaVersion !== 1) throw new TypeError('canonical session schema version is invalid')
  ...
  const parsedStartedBy: { userId: string; displayName?: string } = {
    userId: text(startedBy.userId, 'starter user ID')
  }
  if (startedBy.displayName !== undefined) parsedStartedBy.displayName = text(startedBy.displayName, 'starter display name')
  return {
    schemaVersion: 1,
    sessionId,
    botId,
...
export const legacySessionCodec: SessionCodec<LegacyConversationState> = {
  encode (session) {
  ...
  decodeLegacy (raw, input) {
    const legacy = record(JSON.parse(raw) as unknown, 'legacy session')
    const sender = record(legacy.sender, 'legacy sender')
    const userId = identifier(sender.user_id, 'legacy sender user ID')
    const startedBy: { userId: string; displayName?: string } = { userId }
