...
export interface ProviderProtocolSpan {
  readonly kind: 'provider_protocol_span'
  readonly id: string
...
export type CanonicalConversationItem = SemanticConversationMessage | ProviderProtocolSpan

export interface AgentSessionState {
  readonly schemaVersion: 1
  readonly messages: readonly CanonicalConversationItem[]
  readonly migratedFrom?: Readonly<{
...
function protocolAssistant (
  value: unknown,
  profileId: string,
  profileVersion: number
): Readonly<Extract<ModelMessage, { role: 'assistant' }>> {
...
function protocolTool (value: unknown): Readonly<Extract<ModelMessage, { role: 'tool' }>> {
...
function parseProtocolSpan (value: unknown): ProviderProtocolSpan {
  ...
  if (tools.length !== callIds.length || tools.some((tool, index) => (
    tool.toolCallId !== callIds[index]
  ))) {
    throw new TypeError('provider protocol span is incomplete or out of order')
  }
  const messages = Object.freeze([assistant, ...tools])
  if (jsonByteLength(messages as unknown as JsonValue) >
...
function parseMigratedFrom (value: unknown): AgentSessionState['migratedFrom'] {
...
export function parseAgentSessionState (value: unknown): AgentSessionState {
