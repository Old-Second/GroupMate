...
const ATTEMPT_KINDS = new Set<ProviderTraceEventV1['attemptKind']>([
  'primary', 'retry', 'recovery', 'correction'
])
const TOOL_OUTCOMES = new Set<ToolAttemptOutcome>([
  'succeeded', 'failed', 'denied', 'indeterminate'
])
const APPROVAL_DECISIONS = new Set<ApprovalTraceEventV1['decision']>([
  'requested', 'approved', 'denied', 'expired'
])
const RUN_STATES = new Set<RunStateTraceEventV1['state']>([
...
export interface UnknownOptionalTraceEventV1 {
...
export type StoredRunTraceEventV1 = RunTraceEventV1 | UnknownOptionalTraceEventV1
...
export type TraceCandidateProjectionFailureCode = 'projection_rejected'

function isUnknownOptionalTraceEvent (
  event: StoredRunTraceEventV1
): event is UnknownOptionalTraceEventV1 {
  return Object.hasOwn(event, 'optional')
}

export function internalExactTraceData (
  value: unknown,
...
export function internalExactTraceArray (
  value: unknown,
  maximum: number,
  label: string
): readonly unknown[] {
...
function traceDuration (value: unknown): number | null {
  return value === null ? null : nonNegativeInteger(value, 'trace duration')
}

function parseProviderTraceEvent (value: unknown): ProviderTraceEventV1 {
  ...
  if (input.type !== 'provider_request' || typeof input.outcome !== 'string' ||
    !PROVIDER_OUTCOMES.has(input.outcome as ProviderTraceEventV1['outcome']) ||
    typeof input.attemptKind !== 'string' ||
    !ATTEMPT_KINDS.has(input.attemptKind as ProviderTraceEventV1['attemptKind']) ||
    (input.errorCode !== null && !isAgentErrorCode(input.errorCode))) {
    throw new TypeError('provider trace event fields are invalid')
  }
  const outcome = input.outcome as ProviderTraceEventV1['outcome']
  ...
  return Object.freeze({
    ...
    outcome,
    attemptKind: input.attemptKind as ProviderTraceEventV1['attemptKind'],
    errorCode: input.errorCode
  })
}

function parseToolTraceEvent (value: unknown): ToolExecutionTraceEventV1 {
  const input = internalExactTraceData(value, [
    'type', 'sequence', 'occurredAt', 'durationMs', 'outcome', 'resultCode'
  ], 'tool trace event')
  if (input.type !== 'tool_execution' || typeof input.outcome !== 'string' ||
    !TOOL_OUTCOMES.has(input.outcome as ToolAttemptOutcome)) {
...
function parseApprovalTraceEvent (value: unknown): ApprovalTraceEventV1 {
  const input = internalExactTraceData(value, [
    'type', 'sequence', 'occurredAt', 'durationMs', 'decision'
  ], 'approval trace event')
  if (input.type !== 'approval' || typeof input.decision !== 'string' ||
    !APPROVAL_DECISIONS.has(input.decision as ApprovalTraceEventV1['decision'])) {
  ...
  return Object.freeze({
    ...
    occurredAt: timestamp(input.occurredAt, 'trace timestamp'),
    durationMs: traceDuration(input.durationMs),
    decision: input.decision as ApprovalTraceEventV1['decision']
  })
}

function parseRunStateTraceEvent (value: unknown): RunStateTraceEventV1 {
  const input = internalExactTraceData(value, [
  ...
  if (input.type !== 'run_state' || typeof input.state !== 'string' ||
    !RUN_STATES.has(input.state as RunStateTraceEventV1['state']) ||
    (input.errorCode !== null && !isAgentErrorCode(input.errorCode))) {
    throw new TypeError('run state trace event fields are invalid')
  }
  const state = input.state as RunStateTraceEventV1['state']
...
function parseTruncatedTraceEvent (value: unknown): TraceTruncatedEventV1 {
  const input = internalExactTraceData(value, [
  ...
  if (omittedCount === 0) throw new TypeError('trace omitted count is invalid')
  return Object.freeze({
...
function parseUnknownOptionalTraceEvent (value: unknown): UnknownOptionalTraceEventV1 {
  const input = internalExactTraceData(value, [
...
export function internalParseTraceEvent (
  value: unknown,
  ...
  switch (descriptor.value) {
    ...
    default:
      if (allowUnknownOptional) return parseUnknownOptionalTraceEvent(value)
      throw new TypeError('trace event type is invalid')
  }
}

function terminalState (snapshot: RunTerminalSnapshotV2): RunStateTraceEventV1['state'] {
  return snapshot.status
}

export function internalValidateTraceEvents (
  events: readonly StoredRunTraceEventV1[],
  omittedEventCount: number,
  terminal: RunTerminalSnapshotV2
): void {
  if (events.length < 2) throw new TypeError('trace events are incomplete')
  let previous = -1
  let marker: TraceTruncatedEventV1 | null = null
  const source: StoredRunTraceEventV1[] = []
  for (const event of events) {
    if (event.sequence <= previous) throw new TypeError('trace event order is invalid')
    previous = event.sequence
    if (!isUnknownOptionalTraceEvent(event) && event.type === 'trace_truncated') {
      if (marker !== null) throw new TypeError('trace marker is duplicated')
      marker = event
    } else {
      source.push(event)
    }
  ...
  const terminals = source.filter((event): event is RunStateTraceEventV1 => (
    !isUnknownOptionalTraceEvent(event) && event.type === 'run_state' &&
  ...
  if (omittedEventCount === 0) {
    if (marker !== null) throw new TypeError('trace marker is unexpected')
  } else if (marker === null || marker.omittedCount !== omittedEventCount) {
    throw new TypeError('trace marker count is invalid')
  }
}

function isRequiredTraceEvent (
  event: StoredRunTraceEventV1,
  index: number,
  lastIndex: number
): boolean {
  if (index === 0 || index === lastIndex) return true
  if (isUnknownOptionalTraceEvent(event)) return false
  if (event.type === 'provider_request') return event.outcome !== 'succeeded'
  if (event.type === 'tool_execution') return event.outcome !== 'succeeded'
...
export function internalFinalizeSerialized<T extends { readonly serializedBytes: number }> (
  build: (serializedBytes: number) => T
): T {
  let serializedBytes = 0
  for (let index = 0; index < 16; index += 1) {
    const current = build(serializedBytes)
    const next = Buffer.byteLength(JSON.stringify(current), 'utf8')
    if (next === serializedBytes) return current
    serializedBytes = next
  }
  throw new TypeError('trace serialized byte fixed point is invalid')
}

export interface InternalTraceEventSelection {
  readonly events: readonly StoredRunTraceEventV1[]
  readonly omittedEventCount: number
}

export function internalSelectTraceEvents (input: {
  readonly events: readonly Exclude<StoredRunTraceEventV1, TraceTruncatedEventV1>[]
  readonly existingOmittedCount: number
  readonly existingMarkerSequence: number | null
  readonly build: (
    events: readonly StoredRunTraceEventV1[],
    omittedEventCount: number
  ) => { readonly serializedBytes: number }
}): InternalTraceEventSelection {
  const existingOmittedCount = nonNegativeInteger(
    input.existingOmittedCount,
    'existing trace omitted count'
  )
  if (input.events.length < 2) throw new TypeError('trace source events are incomplete')
  ...
  const compose = (selected: ReadonlySet<number>): InternalTraceEventSelection => {
    ...
    let events: StoredRunTraceEventV1[] = [...retained]
    if (omittedEventCount > 0) {
      const markerSequence = Math.min(
        input.existingMarkerSequence ?? Number.POSITIVE_INFINITY,
        ...omitted.map(event => event.sequence)
      )
  ...
  input.events.forEach((event, index) => {
    if (isRequiredTraceEvent(event, index, lastIndex)) selected.add(index)
  })
...
function projectAgentEvent (
  event: AgentEvent,
  snapshot: RunTerminalSnapshotV2
): Exclude<RunTraceEventV1, TraceTruncatedEventV1> | null {
  const runState = projectRunState(event, snapshot)
  if (runState !== null) return runState
    ...
    const decision = rawDecision === 'approved'
      ...
      : rawDecision === 'rejected'
        ...
        : rawDecision === 'expired'
          ? 'expired'
          : null
    if (decision === null) throw new TypeError('approval trace decision is invalid')
    return Object.freeze({
      type: 'approval',
      sequence: event.sequence,
      occurredAt: event.occurredAt,
...
function projectEvents (
  ...
  snapshot: RunTerminalSnapshotV2
): readonly Exclude<RunTraceEventV1, TraceTruncatedEventV1>[] {
  const projected: Array<Exclude<RunTraceEventV1, TraceTruncatedEventV1>> = []
  let previous = -1
  checkpoint.events.forEach((rawEvent, index) => {
    const event = parseAgentEvent(rawEvent)
    if (event.sequence <= previous || (index === 0 && (
      event.sequence !== 0 || event.type !== 'run.created'
    ))) {
      throw new TypeError('source event order is invalid')
    }
    previous = event.sequence
    const safe = projectAgentEvent(event, snapshot)
    if (safe !== null) projected.push(safe)
  })
...
function durationSummary (durations: readonly (number | null)[]): DurationBucketCountsV1 {
  ...
  ])) as Pick<DurationBucketCountsV1, typeof DURATION_KEYS[number]>
  return Object.freeze({
...
function metricSummary (
  events: readonly Exclude<RunTraceEventV1, TraceTruncatedEventV1>[]
): RunTraceMetricSummaryV1 {
  const providers = new Map<string, ProviderTraceEventV1[]>()
  const tools = new Map<ToolAttemptOutcome, ToolExecutionTraceEventV1[]>()
  const approvals = new Map<ApprovalTraceEventV1['decision'], number>()
  for (const event of events) {
...
function buildCandidate (input: {
  ...
  readonly omittedEventCount: number
  readonly expiresAt: string
}, serializedBytes: number): TraceCandidateV1 {
  return Object.freeze({
    schemaVersion: 1,
    runRef: input.runRef,
...
export function createTraceCandidate (input: {
  ...
  const selected = internalSelectTraceEvents({
    ...
    build: (events, omittedEventCount) => internalFinalizeSerialized(bytes => buildCandidate({
      ...core,
      events: events as readonly RunTraceEventV1[],
      omittedEventCount
    }, bytes))
  })
  const candidate = internalFinalizeSerialized(bytes => buildCandidate({
    ...core,
    events: selected.events as readonly RunTraceEventV1[],
    omittedEventCount: selected.omittedEventCount
  }, bytes))
  return parseTraceCandidate(candidate)
}

export function parseTraceCandidate (value: unknown): TraceCandidateV1 {
  const input = internalExactTraceData(value, [
  ...
  const events = Object.freeze(internalExactTraceArray(
    input.events,
    512,
    'trace candidate events'
  ).map(event => internalParseTraceEvent(event, false) as RunTraceEventV1))
  ...
  const presentation = internalExactTraceData(
    input.presentation,
    ['kind'],
    'trace candidate presentation'
  )
