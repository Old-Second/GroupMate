...
import { contextWireHash, type ContextPlanV1 } from '../context/context-plan.js'
import type {
  PresentationRouteV1,
  TrustedRequestKind
...
export interface PlannedRunContext extends PreparedRunContext {
  readonly plan: ContextPlanV1
  readonly artifactRefs: readonly string[]
}

export interface RunContextPlanningRequest {
  readonly kind: ProviderGenerationKindV1
  readonly transition: 'normal' | 'recovery_prefix_reset'
}
...
interface KnownProviderFailure {
...
interface ModelTurnCommitPlan {
  readonly kind: 'commit'
  readonly status: RunStatus
  readonly changes: RunCheckpointChanges
  readonly drafts: readonly EventDraft[]
}

interface ModelTurnFailurePlan {
  readonly kind: 'fail'
  readonly error: AgentError
  readonly changes: RunCheckpointChanges
  readonly drafts: readonly EventDraft[]
  readonly detailOverride?: Readonly<Record<string, string | number | boolean | null>>
}

type ModelTurnTransitionPlan = ModelTurnCommitPlan | ModelTurnFailurePlan
...
class ModelAttemptFailure extends Error {
  ...
  readonly estimatedInputTokens: number
  readonly recoveryUsed: boolean
  readonly knownProviderFailure: KnownProviderFailure | null

  constructor (
    agentError: AgentError,
    state: ModelAttemptState,
    knownProviderFailure: KnownProviderFailure | null = null
  ) {
...
class ModelSuccessPersistenceError extends Error {
  readonly failure: unknown

  constructor (failure: unknown) {
    super('successful model turn persistence failed')
    this.name = 'ModelSuccessPersistenceError'
...
function detachModelUsage (value: ModelUsage | undefined): ModelUsage | undefined {
  ...
  const requiredKeys = ['inputTokens', 'outputTokens', 'totalTokens']
  if (keys.some(key => ![...requiredKeys, 'inputCache'].includes(key)) ||
    requiredKeys.some(key => !Object.hasOwn(input, key))) {
    throw new TypeError('model usage is invalid')
  }
  const inputCache = input.inputCache
  ...
  if (inputCache === null || typeof inputCache !== 'object' || Array.isArray(inputCache)) {
    throw new TypeError('model cache usage is invalid')
  }
  const cache = inputCache as JsonObject
  const cacheKeys = Object.keys(cache)
  if (cacheKeys.length !== 2 ||
    cacheKeys.some(key => key !== 'hitTokens' && key !== 'missTokens')) {
    throw new TypeError('model cache usage is invalid')
  }
  return Object.freeze({
    inputTokens: input.inputTokens as number,
    ...
    inputCache: Object.freeze({
      hitTokens: cache.hitTokens as number,
      missTokens: cache.missTokens as number
    })
  })
}
...
function synchronizedObservationCounters (
  ...
  const uniqueCallIds = new Set(calls.map(call => call.callId))
  return Object.freeze({
    ...current,
    modelTurns: incrementObservation(current.modelTurns, completedModelTurns),
    toolCalls: knownObservation(current.toolCalls, uniqueCallIds.size),
...
function unavailableProviderObservations (
  current: RunObservationCountersV1
): RunObservationCountersV1 {
  return Object.freeze({
    ...current,
...
function unavailableEngineObservations (
  current: RunObservationCountersV1
): RunObservationCountersV1 {
  return Object.freeze({
    ...current,
...
class ConcurrentTerminalError extends Error {
  readonly tombstone: NormalizedRunTombstoneV1

  constructor (tombstone: NormalizedRunTombstoneV1) {
...
function observedTerminalResult (
  runId: string,
  tombstone: NormalizedRunTombstoneV1
): RunAdvanceResult {
...
function providerOutcomeUnknown (): AgentError {
  return new AgentError({
...
export class RunEngine {
  ...
  readonly #engineActivityStarts = new Map<string, number>()
  readonly #terminalFacts = new WeakMap<RunCheckpoint, TerminalFactsV1>()
  ...
  async loadCheckpoint (runId: string): Promise<RunCheckpoint | null> {
    ...
    if (loaded === null || loaded.schemaVersion === 6) return loaded
  ...
  async decideApproval (
    ...
    let checkpoint = await this.loadCheckpoint(input.runId)
    if (checkpoint === null || checkpoint.status !== 'waiting_approval' ||
    ...
    try {
      ...
      if (isTerminalRunStatus(checkpoint.status)) return this.#terminalResult(checkpoint)
      if (kind === 'approved') {
        let prepared: {
  ...
  async start (
    ...
    try {
      ...
      if (isTerminalRunStatus(stored.status)) return this.#terminalResult(stored)
      return await this.#drive(stored, controller)
    } catch (error) {
      if (error instanceof ConcurrentTerminalError) {
        return observedTerminalResult(input.runId, error.tombstone)
  ...
  async resume (
    ...
    try {
      ...
      if (isTerminalRunStatus(resumable.status)) return this.#terminalResult(resumable)
      try {
  ...
  async cancel (runId: string, reason = 'user_cancelled'): Promise<RunAdvanceResult> {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      if (checkpoint === null) {
        ...
        if (tombstone !== null) return observedTerminalResult(runId, tombstone)
        return Object.freeze({
          kind: 'cancelled',
          runId,
          runRef: 'unavailable',
  ...
  async #beginEngineActivity (
    checkpoint: RunCheckpoint
  ): Promise<RunCheckpoint> {
    let current = await this.#recoverObservationReservations(checkpoint)
    ...
    if (!isTerminalRunStatus(current.status)) {
      this.#engineActivityStarts.set(current.runId, this.#safeMonotonicNow())
    }
    return current
  }

  async #recoverObservationReservations (
    checkpoint: RunCheckpoint
  ): Promise<RunCheckpoint> {
    if (checkpoint.providerDispatch.state === 'idle' &&
      checkpoint.engineActivity.state === 'idle') return checkpoint
  ...
  async #drive (
    ...
    while (!isTerminalRunStatus(checkpoint.status)) {
      if (checkpoint.status === 'waiting_approval') return this.#terminalResult(checkpoint)
      ...
      try {
        ...
        if (error instanceof ModelSuccessPersistenceError) throw error.failure
  ...
  async #planGeneration (
    ...
    request: RunContextPlanningRequest,
    signal: AbortSignal,
    leadingDrafts: readonly EventDraft[] = []
  ): Promise<RunCheckpoint> {
    const runtime = this.#runtime(checkpoint.runId)
    if (runtime.planModelTurn === undefined) {
      throw new TypeError('generation planner runtime is unavailable')
    }
    const planned = await this.#raceAbort(
      runtime.planModelTurn(checkpoint, request, signal),
      signal
    )
  ...
  async #reserveNormalTurn (checkpoint: RunCheckpoint): Promise<RunCheckpoint> {
    ...
    const providerGeneration = checkpoint.contextRuntimeMode === 'generation_planner'
      ? this.#createProviderGeneration(
          checkpoint,
          this.#buildModelRequest(checkpoint, reservation.turn, false),
          'normal'
        )
      : null
  ...
  async #planGenerationForCorrection (
    checkpoint: RunCheckpoint,
    signal: AbortSignal
  ): Promise<RunCheckpoint> {
    const runtime = this.#runtime(checkpoint.runId)
    ...
    const planned = await this.#raceAbort(runtime.planModelTurn(
      checkpoint,
      Object.freeze({
    ...
    if (planned.messages.length !== expectedCorrectionPrefix.length + 1 ||
      ...
      !contextArtifactRefsRetainedOrCleared(
        checkpoint.contextArtifactRefs,
        planned.artifactRefs
      )) {
  ...
  #validatePlannedContext (
    ...
    if (!Number.isSafeInteger(planned.estimatedInputTokens) ||
      ...
      planned.plan.included.some(entry => contextWireHash(Object.freeze(planned.messages.slice(
        entry.wireStart,
  ...
  async #reserveCorrectionTurn (checkpoint: RunCheckpoint): Promise<RunCheckpoint> {
    ...
    const turn = Object.freeze({ kind: 'correction' as const, maxOutputTokens })
    const providerGeneration = checkpoint.contextRuntimeMode === 'generation_planner'
      ? this.#createProviderGeneration(
          checkpoint,
          this.#buildModelRequest(checkpoint, turn, true),
          'correction'
        )
      : null
  ...
  #buildModelRequest (
    ...
    correction: boolean,
    messages: readonly ModelMessage[] = checkpoint.messages
  ): ModelRequest {
    const runtime = this.#runtime(checkpoint.runId)
    return Object.freeze({
  ...
  #createProviderGeneration (
    checkpoint: RunCheckpoint,
    request: ModelRequest,
    kind: ProviderGenerationKindV1,
    ambiguityRecoveryUsed = false
  ) {
    const plan = checkpoint.contextPlan
    if (plan === null) throw new TypeError('provider generation lacks context plan')
    let identity
    if (this.#adapter.requestIdentity !== undefined) {
      identity = this.#adapter.requestIdentity(request)
    } else {
      const value = parseJsonValue(request as unknown as JsonValue, {
      ...
      identity = providerRequestWireIdentity(value as JsonObject)
    }
  ...
  async #callModel (
    ...
    try {
      ...
      return await this.#fail(
        error.checkpoint,
        error.agentError,
        error.knownProviderFailure?.drafts ?? [],
        {
          ...knownFailureChanges,
        ...
        undefined,
        error.knownProviderFailure?.occurredAt
      )
    }
  ...
  async #reserveProviderDispatch (
    ...
    const observationCounters: RunObservationCountersV1 = Object.freeze({
      ...
      providerRetries: knownObservation(
        checkpoint.observationCounters.providerRetries,
        checkpoint.budgetCounters.providerRetries
      ),
      recoveryAttempts: knownObservation(
        checkpoint.observationCounters.recoveryAttempts,
        checkpoint.budgetCounters.recoveryAttempts
      ),
      correctionTurns: knownObservation(
        checkpoint.observationCounters.correctionTurns,
        checkpoint.budgetCounters.correctionTurns
      ),
      providerInputTokens: 'unavailable',
      providerOutputTokens: 'unavailable',
      providerTotalTokens: 'unavailable'
  ...
  async #completeFailedProviderDispatch (
    ...
    changes: RunCheckpointChanges = Object.freeze({})
  ): Promise<RunCheckpoint> {
    return await this.#commit(checkpoint, checkpoint.status, {
  ...
  #failedProviderDispatchChanges (
    checkpoint: RunCheckpoint,
    counters: RunBudgetCounters
  ): RunCheckpointChanges {
    const observationCounters: RunObservationCountersV1 = Object.freeze({
      ...checkpoint.observationCounters,
      providerActiveDurationMs: knownObservation(
  ...
  async #attemptModel (
    ...
    let knownProviderFailure: KnownProviderFailure | null = null

    try {
      while (true) {
        const request = this.#buildModelRequest(
          current,
          Object.freeze({ kind: reserved.kind, maxOutputTokens }),
          correction,
          messages
        )
        if (current.contextRuntimeMode === 'generation_planner') {
          const frozen = current.providerGeneration
          if (frozen === null || !sameJson(
            frozen,
            this.#createProviderGeneration(
              current,
              request,
              frozen.kind,
              frozen.ambiguityRecoveryUsed
            )
          )) {
          ...
          if (error instanceof ModelProviderError) {
            ...
            if (recoveryHint === 'drop_optional_context_once' && recoveryAllowed &&
              ...
              const recovered = current.contextRuntimeMode === 'generation_planner'
                ? runtime.planModelTurn === undefined
                  ? undefined
                  : await this.#raceAbort(runtime.planModelTurn(
                    current,
                    Object.freeze({
                      kind: 'context_recovery' as const,
                      transition: 'recovery_prefix_reset' as const
                    }),
                    signal
                  ), signal)
              ...
              if (recovered !== undefined) {
                ...
                if (current.contextRuntimeMode === 'generation_planner') {
                  ...
                  providerGeneration = this.#createProviderGeneration(
                    candidate,
                    this.#buildModelRequest(candidate, recoveryTurn, false, recoveredMessages),
                    'context_recovery'
                  )
  ...
  async #evaluateModelTurn (
    ...
    try {
      ...
      const common: RunCheckpointChanges = Object.freeze({
        budgetCounters: counters,
        observationCounters,
        usage: recordedUsage,
        providerDispatch: Object.freeze({ state: 'idle' }),
  ...
  #planSuccessfulModelTurn (
    ...
    try {
      ...
      const rawProviderState = turn.providerState
      ...
      if (refusal !== undefined && refusal.length > 0) {
        return Object.freeze({
          kind: 'fail',
          error: modelProtocolError('provider_refusal'),
          changes: common,
          drafts: Object.freeze([attemptedEvent, completedEvent]),
          detailOverride: Object.freeze({ reason: 'provider_refusal' })
        })
      }
      ...
      if (toolCalls.length > 0) {
        ...
        const priorCallIds = new Set(checkpoint.toolLedgers.flatMap(ledger => (
          ledger.calls.map(call => call.callId)
        )))
        ...
        const providerState = rawProviderState === undefined
          ? undefined
          : parseProviderTurnState(rawProviderState)
        if (providerState !== undefined &&
          (providerState.profileId !== checkpoint.profileId ||
            providerState.profileVersion !== checkpoint.profileVersion)) {
        ...
        return Object.freeze({
          ...
          drafts: Object.freeze([
            ...
            completedEvent,
            {
              type: 'tool.batch_planned' as const,
      ...
      if (finishReason === 'stop' && normalizedText.length > 0) {
        const output = this.#assistantMessage(checkpoint, normalizedText)
      ...
      try {
        const budget = this.#runBudget(checkpoint)
        let correctionCounters = budget.recordCorrection(counters)
        const maxOutputTokens = this.#availableOutputTokens(
          checkpoint,
          correctionCounters,
          attempted.estimatedInputTokens,
        ...
        correctionCounters = budget.reserveModelTurn(correctionCounters, {
          kind: 'correction',
          estimatedInputTokens: attempted.estimatedInputTokens,
          maxOutputTokens
        })
        return Object.freeze({
          ...
          changes: Object.freeze({
            ...common,
            budgetCounters: correctionCounters,
          ...
          drafts: Object.freeze([
            ...
            completedEvent,
            {
              ...
              payload: {
                kind: 'correction' as const,
                turn: correctionCounters.modelTurns
              }
            }
          ])
        })
  ...
  async #persistSuccessfulModelTurn (
    checkpoint: RunCheckpoint,
    plan: ModelTurnTransitionPlan,
    occurredAt: string
  ): Promise<RunCheckpoint> {
    try {
      if (plan.kind === 'fail') {
    ...
    const common: RunCheckpointChanges = {
      ...
      observationCounters: Object.freeze({
        ...
        toolAttempts: incrementObservation(
          ...
          countScheduledToolAttempts(execution.results)
        )
      }),
  ...
  async #commit (
    ...
    try {
      ...
      if (closesActivity) this.#engineActivityStarts.delete(checkpoint.runId)
      this.#notifyNewEvents(checkpoint, stored)
      return stored
    } catch (error) {
      if (error instanceof RunStoreConflictError) {
        ...
        const tombstone = await this.#store.loadTombstone(checkpoint.runId)
          .catch(() => null)
        if (tombstone !== null) throw new ConcurrentTerminalError(tombstone)
  ...
  #next (
    ...
    const budgetCounters = boundedChanges.budgetCounters ?? checkpoint.budgetCounters
    const toolLedgers = boundedChanges.toolLedgers ?? checkpoint.toolLedgers
    const interruption = boundedChanges.interruption === undefined
      ? checkpoint.interruption
      : boundedChanges.interruption
    const approvalHistory = boundedChanges.approvalHistory ?? checkpoint.approvalHistory
    const observationCounters = synchronizedObservationCounters(
      boundedChanges.observationCounters ?? checkpoint.observationCounters,
      budgetCounters,
      toolLedgers,
      interruption,
      approvalHistory,
      drafts.filter(draft => draft.type === 'model.completed').length
    )
  ...
  #boundaryObservationChanges (
    checkpoint: RunCheckpoint,
    status: RunStatus,
    changes: RunCheckpointChanges,
    activityBoundary: boolean | 'degrade'
  ): RunCheckpointChanges {
    if (activityBoundary === false) return changes
    if (!isTerminalRunStatus(status) && status !== 'waiting_approval') return changes
    let observationCounters = changes.observationCounters ??
      checkpoint.observationCounters
    const providerDispatch = changes.providerDispatch ?? checkpoint.providerDispatch
    const engineActivity = changes.engineActivity ?? checkpoint.engineActivity
    let exactEngineSpan = activityBoundary === true
    if (providerDispatch.state === 'reserved') {
      observationCounters = unavailableProviderObservations(observationCounters)
      exactEngineSpan = false
    }
    if (engineActivity.state === 'reserved') {
      const startedAt = this.#engineActivityStarts.get(checkpoint.runId)
      if (exactEngineSpan && startedAt !== undefined) {
        observationCounters = Object.freeze({
          ...observationCounters,
          engineActiveDurationMs: incrementObservation(
            observationCounters.engineActiveDurationMs,
            boundedMonotonicDurationMs(
              startedAt,
              this.#safeMonotonicNow()
            )
