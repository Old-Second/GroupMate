...
import {
  asWireRecord,
  iterateResponseBytes,
  readBoundedResponseText,
  readBoundedWireError,
  type BoundedOpenAIWireError,
  type OpenAIFetch,
  type OpenAIFetchInit,
  type OpenAIResponseLike
} from './openai-wire.js'
import {
  normalizeCompleteToolCalls,
  SseToolCallAccumulator
...
function safeErrorName (value: unknown): string | undefined {
  ...
  for (let depth = 0; current && depth < 4; depth += 1) {
    try {
      ...
      if (descriptor) return Object.hasOwn(descriptor, 'value') && typeof descriptor.value === 'string'
        ? descriptor.value
        : undefined
...
function assertString (value: unknown, reason: string, maxLength: number): string {
...
function assertFiniteNumber (value: unknown, reason: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value)) throw modelRequestError(reason)
  return value
}

function cloneJsonObject (value: unknown, reason: string, maxBytes: number): JsonObject {
...
function wireToolCall (call: Readonly<{
  ...
  if (!TOOL_NAME.test(call.name)) throw modelRequestError('invalid_tool_name')
  const argumentsValue = cloneJsonObject(
    call.arguments,
...
function wireMessage (
  ...
  if (message.role === 'tool') {
    if (!CALL_ID.test(message.toolCallId)) throw modelRequestError('invalid_tool_result_call_id')
...
function wireToolDefinition (value: ModelRequest['tools'][number]): JsonObject {
  if (!TOOL_NAME.test(value.name)) throw modelRequestError('invalid_tool_name')
...
export function buildImmutableChatRequest (
  ...
  if (!Array.isArray(request.tools)) throw modelRequestError('invalid_model_tools')
  if (!['auto', 'required', 'disabled'].includes(request.toolMode)) {
    throw modelRequestError('invalid_tool_mode')
  }
  if (typeof request.streaming !== 'boolean') throw modelRequestError('invalid_streaming_mode')
  ...
  if (Object.hasOwn(toolControls, 'functions')) throw modelRequestError('legacy_functions_forbidden')
...
function parseUsage (value: unknown): ModelUsage | undefined {
  ...
  if (![inputTokens, outputTokens, totalTokens].every(token => (
...
function parseResponseId (value: unknown): string | undefined {
...
async function readBoundedJsonTurn (
  ...
  const bounded = await readBoundedResponseText(
    response,
    signal,
    RUN_RESOURCE_LIMITS.providerResponseBytes,
    { overflowReason: 'provider_response_too_large' }
  )
  ...
  const choice = asWireRecord(
    root.choices.find(value => asWireRecord(value, 'invalid_choice').index === 0) ?? root.choices[0],
    'invalid_choice'
  )
  ...
  if (message.function_call !== undefined) throw modelProtocolError('legacy_function_call_unsupported')
  ...
  try {
    ...
    frozenMessage = parsed as JsonObject
  } catch {
...
class AssistantExtensionAccumulator {
  ...
  addDelta (delta: Record<string, unknown>): void {
    if (delta.function_call !== undefined) throw modelProtocolError('legacy_function_call_unsupported')
    for (const [key, value] of Object.entries(delta)) {
      if (['role', 'content', 'refusal', 'tool_calls'].includes(key) || value === undefined) continue
      ...
      if (jsonByteLength(Object.freeze(Object.fromEntries(this.#values)) as JsonObject) >
  ...
  value (): JsonObject {
    return Object.freeze(Object.fromEntries(this.#values)) as JsonObject
  }
}

function parseSseChoice (root: Record<string, unknown>): Record<string, unknown> | undefined {
  if (!Array.isArray(root.choices)) throw modelProtocolError('invalid_stream_choices')
  if (root.choices.length === 0) return undefined
...
async function readBoundedEventStream (
  ...
  const parser = createParser(event => {
    ...
    if (done) throw modelProtocolError('stream_data_after_done')
    if (event.data === '[DONE]') {
      done = true
      return
    }
    ...
    if (delta.content !== undefined && delta.content !== null) {
      if (typeof delta.content !== 'string') throw modelProtocolError('invalid_assistant_content')
      text += delta.content
    }
    if (delta.refusal !== undefined && delta.refusal !== null) {
      if (typeof delta.refusal !== 'string') throw modelProtocolError('invalid_assistant_refusal')
      refusal += delta.refusal
    }
    if (delta.tool_calls !== undefined) {
      if (!Array.isArray(delta.tool_calls)) throw modelProtocolError('invalid_tool_calls')
  ...
  parser.feed('\n\n')
  if (!done) throw modelProtocolError('stream_missing_done')
  if (finishReason === undefined) throw modelProtocolError('missing_finish_reason')
...
function baseWireErrorClassification (error: BoundedOpenAIWireError): ModelErrorOverride {
...
function classifyTransportFailure (error: unknown, signal: AbortSignal): ModelProviderError {
  if (signal.aborted) {
...
export class OpenAICompatibleAdapter implements ModelAdapter {
  ...
  async complete (request: ModelRequest, signal: AbortSignal): Promise<ModelTurn> {
    if (signal.aborted) throw classifyTransportFailure(signal.reason, signal)
