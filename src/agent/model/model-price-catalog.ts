...
export function parseModelPriceSnapshot (value: unknown): ModelPriceSnapshotV1 {
  ...
  const boundedString = (item: unknown): item is string => typeof item === 'string' &&
    item.length > 0 && item.length <= MAX_PRICE_STRING_LENGTH
  const rate = (item: unknown): item is number => typeof item === 'number' &&
