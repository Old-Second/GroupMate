...
export function createWeatherTool (options: WeatherToolOptions): ToolDefinition {
  ...
  return readOnlyDefinition({
    ...
    execute: async (input, context) => {
      ...
      if (city === '') return invalidArguments('天气查询地点不能为空。')
      ...
      if (isToolResult(districtResponse)) return districtResponse
      const districtJson = parseJsonResponse(districtResponse)
      if (isToolResult(districtJson)) return districtJson
      const districts = (districtJson as { districts?: unknown[] })?.districts
      const first = Array.isArray(districts) && districts[0] !== null && typeof districts[0] === 'object'
        ? districts[0] as Record<string, unknown> : undefined
      const adcode = typeof first?.adcode === 'string' ? first.adcode : ''
      if (adcode === '') return upstreamFailure('未找到对应的天气区域。', false)
      ...
      if (isToolResult(weatherResponse)) return weatherResponse
      const weatherJson = parseJsonResponse(weatherResponse)
      if (isToolResult(weatherJson)) return weatherJson
      const lives = (weatherJson as { lives?: unknown[] })?.lives
      if (!Array.isArray(lives) || lives.length === 0) return upstreamFailure('天气服务暂时没有可用数据。', false)
