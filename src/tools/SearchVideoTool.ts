...
export interface SearchVideoToolOptions { readonly policyFetch: PolicyFetch }
...
export function createSearchVideoTool (options: SearchVideoToolOptions): ToolDefinition {
  ...
  return readOnlyDefinition({
    ...
    execute: async (input, context) => {
      ...
      if (keyword === '') return invalidArguments('视频搜索关键词不能为空。')
      ...
      const data = parsed !== null && typeof parsed === 'object'
        ? (parsed as Record<string, unknown>).data : undefined
      const rows = data !== null && typeof data === 'object' && Array.isArray((data as Record<string, unknown>).result)
        ? (data as Record<string, unknown>).result as unknown[] : []
      if (rows.length === 0) return upstreamFailure('视频搜索暂时没有结果。', false)
