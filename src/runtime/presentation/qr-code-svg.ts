...
function appendBits (value: number, length: number, output: number[]): void {
  if (length < 0 || length > 31 || value >>> length !== 0) throw new RangeError('Value out of range')
  for (let index = length - 1; index >= 0; index--) output.push((value >>> index) & 1)
}
...
function reedSolomonMultiply (left: number, right: number): number {
  if ((left >>> 8) !== 0 || (right >>> 8) !== 0) throw new RangeError('Byte out of range')
  let result = 0
...
class MediumQrMatrix {
  ...
  constructor (version: number, data: readonly number[]) {
    ...
    for (let row = 0; row < positions.length; row++) {
      for (let column = 0; column < positions.length; column++) {
        if ((row === 0 && column === 0) ||
          ...
          (row === positions.length - 1 && column === 0)) continue
  ...
  drawFormatBits (mask: number): void {
    ...
    for (let index = 0; index <= 5; index++) this.setFunction(8, index, bit(bits, index))
    this.setFunction(8, 7, bit(bits, 6))
    this.setFunction(8, 8, bit(bits, 7))
    this.setFunction(7, 8, bit(bits, 8))
    for (let index = 9; index < 15; index++) this.setFunction(14 - index, 8, bit(bits, index))
    for (let index = 0; index < 8; index++) this.setFunction(this.size - 1 - index, 8, bit(bits, index))
    for (let index = 8; index < 15; index++) this.setFunction(8, this.size - 15 + index, bit(bits, index))
  ...
  addErrorCorrection (data: readonly number[]): number[] {
    if (data.length !== dataCodewords(this.version)) throw new RangeError('Invalid data length')
    ...
    for (let block = 0; block < blockCount; block++) {
      ...
      if (block < shortBlockCount) current.push(0)
    ...
    for (let y = 0; y < this.size; y++) {
      ...
      for (let x = 0; x < this.size; x++) {
        if (this.modules[y]![x] === runColor) {
          ...
          if (!runColor) result += this.countPatterns(history) * 40
    ...
    for (let y = 0; y < this.size - 1; y++) {
      for (let x = 0; x < this.size - 1; x++) {
        ...
        if (color === this.modules[y]![x + 1] && color === this.modules[y + 1]![x] &&
          color === this.modules[y + 1]![x + 1]) result += 3
      }
    }
  ...
  addHistory (length: number, history: number[]): void {
    if (history[0] === 0) length += this.size
    history.pop()
    history.unshift(length)
  }

  countPatterns (history: readonly number[]): number {
  ...
  terminateAndCount (color: boolean, length: number, history: number[]): number {
    if (color) {
...
function encodeMediumQr (text: string): MediumQrMatrix {
  const bytes = [...new TextEncoder().encode(text)]
  let version = 1
  for (; version <= 40; version++) {
    const countBits = version <= 9 ? 8 : 16
    if (bytes.length < 2 ** countBits && 4 + countBits + bytes.length * 8 <= dataCodewords(version) * 8) break
  }
  ...
  for (let index = 0; index < bits.length; index++) {
    if ((index & 7) === 0) data.push(0)
    data[data.length - 1] |= bits[index]! << (7 - (index & 7))
  }
  return new MediumQrMatrix(version, data)
}

function matrixPath (modules: readonly (readonly boolean[])[], margin: number): string {
...
function validatedPageUrl (value: string): string | null {
...
function qrShape (pageUrl: string): { readonly size: number; readonly path: string } | null {
...
export function createQrCodeSvg (pageUrl: string): QrCodeSvg | null {
...
function browserEncoderSource (): string {
  return [
...
export function qrCodeBrowserScript (): string {
  if (browserScript !== undefined) return browserScript
