...
import type { TtsMode } from './presentation-settings.js'
...
export type TtsSynthesisResult =
  | { readonly kind: 'ready'; readonly audio: ToolResource }
  | {
...
export type TtsRecordEncodingV1 = 'default' | 'shamrock_passthrough'

export interface TtsTargetEncodingPort {
  forTarget(target: SessionAddress): Promise<TtsRecordEncodingV1 | 'unavailable'>
}

export interface YunzaiTtsBackendPort {
  synthesize(input: {
    readonly text: string
    readonly mode: TtsMode
    readonly voice: string
    readonly emotion?: string
    readonly emotionDegree?: number
    readonly recordEncoding: TtsRecordEncodingV1
  }, signal?: AbortSignal): Promise<TtsSynthesisResult>
}
...
type BackendRace =
  | { readonly kind: 'fulfilled'; readonly value: TtsSynthesisResult }
  | { readonly kind: 'exception' }
  | { readonly kind: 'abort' }
  | { readonly kind: 'timeout' }

const DEFINITE_CODES = new Set<TtsSynthesisErrorCode>([
  'empty_after_filter',
  'unsupported_mode',
  'unsupported_voice',
  'aborted_before_dispatch',
...
const UNKNOWN_CODES = new Set<TtsSynthesisErrorCode>([
  'synthesis_exception',
  'synthesis_abort_after_dispatch',
  'synthesis_timeout'
])
...
function canonicalTarget (value: unknown): SessionAddress | null {
  ...
  if (!validIdentifier(botId) || scope === null || typeof scope !== 'object') return null
  if (exactDataRecord(scope, ['kind', 'userId']) &&
    ...
    return Object.freeze({
      botId,
      scope: Object.freeze({
        kind: 'private', userId: ownData(scope, 'userId')?.value as string
      })
    })
  }
  if (exactDataRecord(scope, ['kind', 'groupId']) &&
    ...
    return Object.freeze({
      botId,
      scope: Object.freeze({
        kind: 'group', groupId: ownData(scope, 'groupId')?.value as string
      })
    })
  }
...
function normalizedText (value: unknown, maximum?: number): string | null {
  ...
  try {
    ...
    if (maximum !== undefined && [...normalized].length > maximum) return null
    return normalized
  } catch {
    return null
  }
...
function normalizedMode (value: unknown): TtsMode | null {
  return value === 'vits-uma-genshin-honkai' || value === 'azure' || value === 'voicevox'
    ? value
    : null
}

function normalizedResource (value: unknown): ToolResource | null {
  if (value === null || typeof value !== 'object') return null
  const kind = ownData(value, 'kind')?.value
  const contentKey = kind === 'buffer' ? 'data' : kind === 'remote_url' ? 'url' : 'path'
  if (!exactDataRecord(value, ['kind', contentKey, 'mimeType', 'byteLength'])) return null
  ...
  if (kind === 'buffer') {
    ...
    if (!(data instanceof Uint8Array) || data.byteLength !== byteLength) return null
    return Object.freeze({
  ...
  if (kind === 'local_path') {
    ...
    if (typeof path !== 'string' || path.length === 0 || path.length > 4_096) return null
    return Object.freeze({ kind: 'local_path', path, mimeType, byteLength })
  }
  return null
}

function projectedBackendResult (value: unknown): TtsSynthesisResult | null {
  if (value === null || typeof value !== 'object') return null
  const kind = ownData(value, 'kind')?.value
  if (kind === 'ready') {
  ...
  if (kind === 'failed_definite' && DEFINITE_CODES.has(code as TtsSynthesisErrorCode)) {
    return Object.freeze({
      kind: 'failed_definite',
      code: code as Extract<TtsSynthesisErrorCode,
      'empty_after_filter' | 'unsupported_mode' | 'unsupported_voice' |
      'aborted_before_dispatch' | 'synthesis_rejected'>
    })
  }
  if (kind === 'outcome_unknown' && UNKNOWN_CODES.has(code as TtsSynthesisErrorCode)) {
    return Object.freeze({
      kind: 'outcome_unknown',
      code: code as 'synthesis_exception' | 'synthesis_abort_after_dispatch' | 'synthesis_timeout'
    })
  }
  return null
}

function raceBackend (
  invoke: () => Promise<TtsSynthesisResult>,
  signal: AbortSignal | undefined
): Promise<BackendRace> {
  return new Promise(resolve => {
    let settled = false
    let timer: ReturnType<typeof setTimeout> | undefined
    const finish = (result: BackendRace): void => {
      if (settled) return
      settled = true
    ...
    const onAbort = (): void => finish(Object.freeze({ kind: 'abort' }))
...
function definite (
  code: Extract<TtsSynthesisErrorCode,
  'empty_after_filter' | 'unsupported_mode' | 'unsupported_voice' |
  'aborted_before_dispatch' | 'synthesis_rejected'>
): TtsSynthesisResult {
...
export function createYunzaiTtsReplyPort (input: {
  readonly targets: TtsTargetEncodingPort
  readonly backend: YunzaiTtsBackendPort
}): TtsReplyPort {
  return Object.freeze({
    async synthesize (unsafeInput: Parameters<TtsReplyPort['synthesize']>[0], signal?: AbortSignal) {
      let backendDispatched = false
      try {
        const text = normalizedText(unsafeInput?.text)
        if (text === null || text === '') return definite('empty_after_filter')
        const mode = normalizedMode(unsafeInput?.mode)
        if (mode === null) return definite('unsupported_mode')
        const voice = normalizedText(unsafeInput?.voice, 256)
        if (voice === null || voice === '') return definite('unsupported_voice')
        const target = canonicalTarget(unsafeInput?.target)
        if (target === null) return definite('synthesis_rejected')

        let emotion: string | undefined
        if (unsafeInput.emotion !== undefined) {
          const normalized = normalizedText(unsafeInput.emotion, 64)
          if (normalized === null || normalized === '') return definite('synthesis_rejected')
          emotion = normalized
        }
        ...
        if (isAborted(signal)) return definite('aborted_before_dispatch')

        let recordEncoding: TtsRecordEncodingV1 | 'unavailable'
        try {
...
export const UNAVAILABLE_TTS_REPLY_PORT: TtsReplyPort = Object.freeze({
