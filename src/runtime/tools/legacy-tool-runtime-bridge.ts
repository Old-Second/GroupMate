...
import { ToolExecutor, type ToolExecutionOutcome, type ToolExecutionRequest } from '../../agent/tools/tool-executor.js'
import { ToolPolicyEngine, type ToolPolicyProfile } from '../../agent/tools/policy-engine.js'
import { ToolRegistry, type ModelToolDefinition, type ToolSnapshot } from '../../agent/tools/tool-registry.js'
import {
...
import { extractIntentEvidence, type IntentEvidence } from './intent-evidence.js'
...
function approvalFeedback (outcome: Extract<ToolExecutionOutcome, { kind: 'approval_required' }>): string {
...
export function createLegacyToolRuntimeBridge (
  ...
  return Object.freeze({
    async begin (input: LegacyToolRunInput): Promise<LegacyToolRun> {
      ...
      options.onRunCaptured?.(runs.get(snapshotId) as CapturedRun)
...
export interface YunzaiToolRuntimeBridgeOptions {
  ...
  readonly redis: RedisToolControlClient
  ...
  return {
    ...
    reportProgress: async (text, context) => {
      ...
      return Object.freeze({
        ...
        effect: 'none' as const,
        reasonCode: 'invalid_arguments' as const,
