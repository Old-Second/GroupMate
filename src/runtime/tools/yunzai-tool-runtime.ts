...
import type { ToolRegistry, ToolSnapshot } from '../../agent/tools/tool-registry.js'
...
interface YunzaiToolCapture {
  readonly profile: unknown
  readonly registry: ToolRegistry
  readonly enabledTools: readonly string[]
  readonly initialFacts: ToolRuntimeFacts
  readonly intent: ReturnType<typeof extractIntentEvidence>
...
export interface YunzaiToolRunInput {
...
export class ToolRuntimeConfigurationError extends Error {
  readonly code = 'unknown_policy_profile' as const
...
function preparedIntentText (evidence: PreparedYunzaiMessageEvidenceV1): string {
  if (!evidence.hasReply) return boundedIntentText(evidence.prompt)
  if (!evidence.prompt.startsWith(MESSAGE_CONTEXT_PREFIX)) {
    throw new TypeError('prepared reply intent evidence is invalid')
  }
  try {
    const payload: unknown = JSON.parse(evidence.prompt.slice(MESSAGE_CONTEXT_PREFIX.length))
    ...
    if (current === null || typeof current !== 'object' || Array.isArray(current)) throw new Error()
...
async function eligibleApprovers (
  ...
  if (actorGroupRole === 'owner' && !actors.has(actorId)) actors.set(actorId, 'group_owner')
  ...
  if (event.isGroup === true) {
    try {
      ...
      for (const [key, value] of members) {
        ...
        if (memberRole === 'owner' && !actors.has(userId)) actors.set(userId, 'group_owner')
