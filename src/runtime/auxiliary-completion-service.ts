...
export type AuxiliaryCompletionPurpose = 'random_greeting' | 'suggestion'

export interface AuxiliaryCompletionInput {
  readonly purpose: AuxiliaryCompletionPurpose
  readonly messages: readonly ModelMessage[]
  readonly maxOutputTokens?: number
}

export interface AuxiliaryCompletionService {
  completeText(input: AuxiliaryCompletionInput): Promise<string>
}

export interface AuxiliaryCompletionServiceOptions {
...
const DEFAULT_OUTPUT_TOKENS: Readonly<Record<AuxiliaryCompletionPurpose, number>> =
  Object.freeze({ random_greeting: 128, suggestion: 512 })
const PURPOSES: readonly AuxiliaryCompletionPurpose[] = Object.freeze([
...
function cloneMessage (message: ModelMessage): ModelMessage {
  if (message.role === 'tool') throw new TypeError('auxiliary tool messages are forbidden')
  ...
  if (message.content === '') throw new TypeError('auxiliary message is empty')
  return Object.freeze({ role: message.role, content: message.content })
}

function requestFor (
  input: AuxiliaryCompletionInput,
  options: AuxiliaryCompletionServiceOptions,
  model: string
): ModelRequest {
...
function currentModel (configured: AuxiliaryCompletionServiceOptions['model']): string {
...
export function createAuxiliaryCompletionService (
  options: AuxiliaryCompletionServiceOptions
): AuxiliaryCompletionService {
  ...
  return Object.freeze({
    async completeText (input: AuxiliaryCompletionInput): Promise<string> {
      ...
      timer.unref?.()
      try {
        return completedText(await options.adapter.complete(
          requestFor(input, options, model),
          controller.signal
        ))
      } finally {
        clearTimeout(timer)
...
let activeService: AuxiliaryCompletionService | null = null

export function configureAuxiliaryCompletionService (
  options: AuxiliaryCompletionServiceOptions
): AuxiliaryCompletionService {
