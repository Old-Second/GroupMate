...
function arrayState (value: object): boolean | null {
  try {
 ...
 */
export function normalizeYunzaiHostDispatchResult (value: unknown): unknown {
  if (value === null || typeof value !== 'object') return value
  const valueIsArray = arrayState(value)
  if (valueIsArray === null) return null
  if (valueIsArray) return value
  ...
  if (!envelopeClaimed) return value
