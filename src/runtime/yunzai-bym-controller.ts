...
export interface BymPolicySnapshot {
  ...
  readonly retaliationWords: readonly string[]
  readonly retaliationBlacklistActorIds: readonly string[]
  readonly retaliationPrompt: string
  readonly retaliationRecallEnabled: boolean
  readonly retaliationRecallSeconds: number
}

export interface BymPolicyPort {
  snapshot(): BymPolicySnapshot
}

export interface BymPromptScreeningPort {
  isBlocked(input: {
...
export interface PreparedBymRequest {
...
export interface BymRequestPreparationPort {
  prepare(input: {
    readonly event: YunzaiMessageEvent
    readonly prompt: string
    ...
    >
  }): Promise<PreparedBymRequest>
}

export interface BymEphemeralAgentPort {
  handleEphemeral(input: {
    readonly event: YunzaiMessageEvent
    readonly prepared: PreparedBymRequest
    readonly systemInstructions: readonly string[]
    readonly enableGroupContext: true
    readonly thinkingMode: ConfiguredThinkingMode
    readonly reasoningEffort: ConfiguredReasoningEffort
...
export interface YunzaiBymController {
...
export interface YunzaiBymControllerOptions {
  ...
  readonly promptScreening: BymPromptScreeningPort
  readonly requests: BymRequestPreparationPort
  readonly agent: BymEphemeralAgentPort
...
function recallAfterMs (
  policy: BymPolicySnapshot,
  retaliation: boolean
): number | null {
  if (!retaliation || !policy.retaliationRecallEnabled) return null
  const seconds = Math.min(
    Math.max(Math.trunc(Number(policy.retaliationRecallSeconds) || 100), 1),
    3_600
  )
  return seconds * 1_000
}

function proactiveSystemInstruction (
  policy: BymPolicySnapshot,
  retaliation: boolean
): string {
...
function assertPreparedIntent (
  prepared: PreparedBymRequest,
  expectedRecallAfterMs: number | null
): void {
...
async function completeProactive (
  options: YunzaiBymControllerOptions,
...
async function runBym (
  ...
  if (groupId !== null && policy.disabledGroupIds.includes(groupId)) return

  const trigger = decideBymTrigger({
    message: event.msg,
    assistantLabel: policy.assistantLabel,
    hasLeadingAlias: (event as { readonly hasAlias?: unknown }).hasAlias === true,
    recognizeLeadingAlias: policy.recognizeLeadingAlias
  })
  if (trigger.prompt === null) return
  ...
  if (probabilitySample >= policy.ratePercent) return
...
export function createYunzaiBymController (
  options: YunzaiBymControllerOptions
): YunzaiBymController {
  return Object.freeze({
    async bym (event: YunzaiMessageEvent): Promise<false> {
      try {
