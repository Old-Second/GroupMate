...
async function maximumDurableSequence (filePath: string): Promise<number | null> {
  ...
  try {
    ...
    while (true) {
      ...
      while (offset < bytesRead) {
        ...
        const match = SEQUENCE_PREFIX_PATTERN.exec(
          prefix.subarray(0, prefixBytes).toString('ascii')
        )
        if (match !== null) {
          const sequence = Number(match[1])
          if (Number.isSafeInteger(sequence) && sequence > 0 &&
            (maximum === null || sequence > maximum)) maximum = sequence
        }
        prefixBytes = 0
        offset = newline + 1
      }
...
export class GroupMateDiskLog {
  ...
  readonly #writerDates = new Set<string>()
  #sequence = 0
  #sequenceBase: number | null = null
  #pendingBytes = 0
  #pendingRecords = 0
  ...
  async #recoverSequence (operationRoot: string): Promise<number> {
  ...
  #materialize (entry: DiskLogEntry): DiskLogEntry {
    if (this.#sequenceBase === null) throw new TypeError('disk log sequence is unavailable')
    ...
    if (line === entry.line) throw new TypeError('disk log sequence marker is missing')
  ...
  async #targetFor (entry: DiskLogEntry, operationRoot: string): Promise<string | null> {
    ...
    if (latest === undefined) {
      ...
      if (latest.index >= 9_999) return null
