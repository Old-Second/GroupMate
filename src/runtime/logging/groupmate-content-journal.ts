...
function jsonArray (value: unknown, maximum: number, label: string): readonly JsonValue[] {
...
function parseSessionAddress (value: unknown): SessionAddress {
  const input = jsonObject(value, 'session address') as UnknownRecord
  exactKeys(input, ['botId', 'scope'], ['botId', 'scope'], 'session address')
  text(input.botId, 'session bot ID')
  const scope = jsonObject(input.scope, 'conversation scope') as UnknownRecord
...
function parseRunModelConfig (value: unknown): void {
  ...
  const reasoning = jsonObject(input.reasoning, 'run reasoning') as UnknownRecord
...
function parseModelRequest (value: unknown): ModelRequest {
  ...
  const reasoning = jsonObject(input.reasoning, 'model reasoning') as UnknownRecord
...
function projectRunEvent (value: RunContentJournalEvent): ProjectedJournalEvent {
...
interface OwnDataRecordOptions {
...
function ownDataArray (value: unknown, maximum: number, label: string): readonly unknown[] {
...
function projectRawSessionAddress (value: unknown): SessionAddress {
...
function projectOutboundEvent (value: GroupMateOutboundJournalEvent): ProjectedJournalEvent {
