...
import {
  type TraceClearReceiptV1,
  type TraceLookupResult,
  type TraceOffBarrierReceiptV1,
  type TraceRetentionV1,
  type TraceStore,
  type TraceSummaryV1,
  type TraceUsageV1,
...
function rejected (code: Extract<TraceWriteReceiptV1, { kind: 'rejected' }>['code']): TraceWriteReceiptV1 {
  return Object.freeze({ schemaVersion: 1, kind: 'rejected', code })
}

function retainedAs (record: StoredTraceRecordV1): TraceRetentionV1 {
  if (record.terminal.status === 'failed') return 'failed'
  if (record.terminal.status === 'cancelled') return 'cancelled'
  if (record.presentation.kind === 'observed' && (
    record.presentation.value.postprocessAnomaly ||
  ...
  if (record.policy.levelAtStart === 'diagnostic') return 'diagnostic'
  return 'sampled_success'
}

function indexClass (record: StoredTraceRecordV1): 'success' | 'failure' {
...
function sameEngineFact (
  left: StoredTraceRecordV1,
  right: StoredTraceRecordV1
): boolean {
  const {
    presentation: _leftPresentation,
    serializedBytes: _leftBytes,
    ...leftEngine
  } = left
  const {
    presentation: _rightPresentation,
    serializedBytes: _rightBytes,
    ...rightEngine
  } = right
...
export class RedisTraceStore implements TraceStore {
  ...
  constructor (options: { readonly client: RedisRunClient; readonly now?: () => number }) {
  ...
  async upsertEngine (value: TraceCandidateV1): Promise<TraceWriteReceiptV1> {
    ...
    if (existingRaw !== null && existingRaw !== JSON.stringify(record)) {
      try {
        const existing = parseStoredTraceRecord(JSON.parse(existingRaw) as unknown)
  ...
  async appendPresentation (value: PresentationObservationV1): Promise<TraceWriteReceiptV1> {
    ...
    if (presentation.runRef === 'unavailable' ||
      ...
      presentation.terminalObservationId === 'not_attempted') return rejected('not_found')
    ...
    if (raw === null) return rejected('not_found')
    let record: StoredTraceRecordV1
    try {
      record = parseStoredTraceRecord(JSON.parse(raw) as unknown)
    } catch {
    ...
    let merged: StoredTraceRecordV1
    try {
  ...
  async load (runRef: string): Promise<TraceLookupResult> {
  ...
  async listRecent (limit: number): Promise<readonly TraceSummaryV1[]> {
    const bounded = this.#boundedLimit(
      limit,
      TRACE_STORE_LIMITS.maxListRecords,
      'trace list limit'
    )
    try {
      const result = await this.#eval('list', [String(finiteNow(this.#now)), String(bounded)])
      const rawRows = tuple(result, 0, 'trace list result')
      if (rawRows.length % 2 !== 0) throw new TypeError('trace list result is invalid')
      const records: StoredTraceRecordV1[] = []
      for (let index = 0; index < rawRows.length; index += 2) {
        ...
        if (typeof key !== 'string' || !key.startsWith(TRACE_KEY_PREFIX) ||
          typeof raw !== 'string') continue
        try {
          records.push(parseStoredTraceRecord(JSON.parse(raw) as unknown))
        } catch {
          await this.#repairCorrupt(key, raw)
        }
      }
  ...
  async usage (): Promise<TraceUsageV1> {
    try {
  ...
  async clear (
    maxRecords: number = TRACE_STORE_LIMITS.cleanupBatchRecords
  ): Promise<TraceClearReceiptV1> {
    const bounded = this.#boundedLimit(
      maxRecords,
      TRACE_STORE_LIMITS.cleanupBatchRecords,
      'trace clear limit'
    )
  ...
  async advanceGenerationAndClear (
    maxRecords: number = TRACE_STORE_LIMITS.maxRecords
  ): Promise<TraceOffBarrierReceiptV1> {
    const bounded = this.#boundedLimit(
      maxRecords,
      TRACE_STORE_LIMITS.maxRecords,
      'trace clear limit'
    )
    ...
    try {
      ...
      const result = tuple(
        await this.#eval('advance_clear', [String(firstLimit)]),
        5,
        'trace barrier result'
      )
      ...
      while (clear.remainingRecords > 0 && removedRecords < bounded) {
        const batchLimit = Math.min(
          TRACE_STORE_LIMITS.cleanupBatchRecords,
          bounded - removedRecords
        )
        const batch = this.#clearReceipt(
          await this.#eval('clear', [String(batchLimit)])
        )
  ...
  async #writeWithGeneration (
    ...
    if (parsed.kind === 'stale') return rejected('stale_generation')
    this.#generation = parsed.generation
    this.#generationKnown = true
    ...
    if (parsed.code === 'conflict' || parsed.code === 'capacity' ||
      parsed.code === 'not_found') return rejected(parsed.code)
    return rejected('conflict')
  }

  #writeResult (value: unknown):
  | { readonly kind: 'stale'; readonly generation: number }
  | { readonly kind: 'result'; readonly code: string; readonly generation: number } {
  ...
  async #repairCorrupt (key: string, raw: string): Promise<void> {
    try {
  ...
  async #eval (operation: string, args: readonly string[], runRef?: string): Promise<unknown> {
  ...
  #clearReceipt (value: unknown): TraceClearReceiptV1 {
  ...
  #boundedLimit (value: number, maximum: number, label: string): number {
  ...
  #assertRunRef (runRef: string): void {
