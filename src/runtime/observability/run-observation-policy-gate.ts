...
export interface RunObservationPolicyGateSnapshotV1 {
  readonly schemaVersion: 1
  readonly currentLevel: ObservabilityLevel
  readonly entries: number
  readonly missingDrops: number
...
function canonicalPolicy (policy: FrozenObservationPolicyV1): string {
  return JSON.stringify(policy)
}

function eventRunRef (event: ObservationEventV1): string | 'unavailable' {
  return event.value.runRef
}

export class RunObservationPolicyGate {
  readonly #now: () => number
  readonly #entries = new Map<string, GateEntry>()
  ...
  register (runRef: string, value: FrozenObservationPolicyV1): void {
  ...
  allow (value: ObservationEventV1): boolean {
    ...
    if (runRef === 'unavailable') return this.#currentLevel !== 'off'
    return this.#allowRun(runRef)
  }

  allowCommittedCandidate (value: TraceCandidateV1): boolean {
    let candidate: TraceCandidateV1
    try {
    ...
    if (entry.canonical !== canonicalPolicy(candidate.policy)) return false
  ...
  snapshot (): RunObservationPolicyGateSnapshotV1 {
    this.#purgeExpired()
  ...
  #purgeExpired (): void {
    ...
    for (const [runRef, entry] of this.#entries) {
      if (now - entry.touchedAt >= TTL_MS) this.#entries.delete(runRef)
    }
  }

  #effectiveLevel (start: ObservabilityLevel): ObservabilityLevel {
