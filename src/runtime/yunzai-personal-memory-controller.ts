...
import type {
  PersonalMemoryLifecycleFacadeOperationV1,
...
import type {
  PersonalMemoryCommandPortV1,
  PersonalMemoryCommandRequestV1
} from './personal-memory-command.js'
...
interface NamespaceStateV1 {
  ...
  readonly enrollmentState: 'opted_in' | 'opted_out' | null
}

interface PersonalMemoryAuthorizationV1 {
  readonly now: string
...
export interface PersonalMemoryExportDeliveryV1 {
  readonly redeemAndSend: (
    handle: MemoryExportDeliveryHandleV1,
    sendFile: PersonalMemoryCommandRequestV1['sendPrivateFile']
  ) => Promise<'delivered' | 'already_consumed' | 'unavailable'>
}

export interface CreateYunzaiPersonalMemoryControllerOptionsV1 {
  readonly botInstanceId: string
  readonly database: DatabaseSync
  readonly mode: () => ProductionPersonalMemoryModeV1
  readonly groupAllowlist: () => readonly string[]
  readonly now: () => string
  readonly facade: PersonalMemoryLifecycleFacadeV1
...
): ProductionPersonalMemoryModeV1 {
  try {
...
function groupIsAllowed (source: () => readonly string[], groupId: string): boolean {
  try {
    const values = Reflect.apply(source, undefined, []) as unknown
...
function messageId (event: YunzaiRecord): string {
  const value = event.message_id ?? event.messageId
  if ((typeof value === 'string' || typeof value === 'number' || typeof value === 'bigint') &&
    /^[\x21-\x7e]{1,128}$/.test(String(value))) return String(value)
...
function boundedSourceText (value: string): string {
  ...
  for (const codePoint of value.normalize('NFC').replace(/\r\n?/g, '\n')) {
    if (Buffer.byteLength(result + codePoint, 'utf8') >
      MEMORY_RESOURCE_LIMITS.sourceExcerptUtf8Bytes) break
    result += codePoint
  }
  return result.trim() || '长期记忆控制请求'
}

function commandRef (source: MemorySourceV1, operation: string): string {
...
function actorRef (botInstanceId: string, accountId: string, userId: string): string {
  return `actor:${sha256(`${botInstanceId}\0qq\0${accountId}\0${userId}`)}`
}

function accountId (event: YunzaiRecord): string {
  const value = qqId(event.self_id ?? event.bot?.uin ?? event.bot?.self_id)
  if (value === null) throw new PersonalMemoryCommandError('无法确认当前机器人账号，长期记忆操作未执行。')
  return value
}
...
function messageEvidence (text: string, currentMessageId: string): PreparedYunzaiMessageEvidenceV1 {
  return Object.freeze({
    schemaVersion: 1,
    prompt: text,
...
function memorySource (
  participant: SceneParticipantV1,
...
function inferKind (text: string): MemoryKindV1 {
  if (/^(?:我|本人)?(?:喜欢|偏好|不喜欢|讨厌|常用|更喜欢)/u.test(text)) return 'preference'
  if (/^(?:我|本人|我的|本人是|我是)/u.test(text)) return 'profile_fact'
  if (/(?:关系|朋友|同事|家人|亲属)/u.test(text)) return 'relationship'
  if (/(?:任务|待办|计划|截止|提醒)/u.test(text)) return 'task_fact'
  return 'other'
}

function validMemoryText (value: string): string {
...
function recordId (value: MemoryRecordControlProjectionV1): string {
  return value.lifecycleState === 'purge_due' ? value.memoryId : value.record.memoryId
}

function recordReference (memoryId: string): string {
  return `@${memoryId.slice(-8)}`
}

function recordText (value: MemoryRecordControlProjectionV1): string {
  if (value.lifecycleState === 'purge_due') return '[已到物理清理期]'
  const text = [...value.record.text].slice(0, 120).join('')
  return text.length === value.record.text.length ? text : `${text}...`
}

function lifecyclePayload (
  auth: PersonalMemoryAuthorizationV1,
...
function controlBase (auth: PersonalMemoryAuthorizationV1, operation: string) {
  return {
    schemaVersion: 1 as const,
    operation,
...
function mutationSucceeded (result: PersonalMemoryLifecycleFacadeResultV1): boolean {
  const status = result.result.status
  return status === 'stored' || status === 'unchanged' || status === 'deletion_pending' ||
    status === 'deletion_complete'
}

function mutationFailureText (result: PersonalMemoryLifecycleFacadeResultV1): string {
  const status = result.result.status
  if (status === 'enrollment_required') return '请先发送“#长期记忆 开启”。'
  if (status === 'denied') return '当前场景或身份无权执行这个长期记忆操作。'
  if (status === 'conflict') return '记忆刚刚发生变化，请重新查看列表后再操作。'
  if (status === 'record_expired') return '这条记忆已过期，不能更正；可以续期或遗忘。'
  if (status === 'record_purge_due' || status === 'not_found') return '没有找到可操作的记忆。'
  if (status === 'capacity' || status === 'history_limit') return '长期记忆已达到当前容量上限。'
  if (status === 'aborted') return '长期记忆操作已取消。'
...
export function createYunzaiPersonalMemoryControllerV1 (
  options: CreateYunzaiPersonalMemoryControllerOptionsV1
): PersonalMemoryCommandPortV1 {
  ...
  const authorize = async (
    request: PersonalMemoryCommandRequestV1
  ): Promise<PersonalMemoryAuthorizationV1> => {
    const event = request.event as YunzaiRecord
    ...
    const currentActorRef = actorRef(
      options.botInstanceId,
      botAccountId,
      snapshot.current.identity.userId
    )
    const actorContext = {
      schemaVersion: 1,
      botInstanceId: options.botInstanceId,
  ...
  const readEnrollment = async (auth: PersonalMemoryAuthorizationV1) => await options.facade.execute(
    facadeRequest('enrollment.read', auth, Object.freeze({
      schemaVersion: 1,
      namespace: auth.namespace,
      access: auth.access
    }))
  ...
  const listRecords = async (
    auth: PersonalMemoryAuthorizationV1
  ): Promise<readonly MemoryRecordControlProjectionV1[]> => {
  ...
  const resolveRecord = async (
    auth: PersonalMemoryAuthorizationV1,
    selector: string
  ): Promise<MemoryRecordControlProjectionV1> => {
  ...
  const loadRevision = async (
    auth: PersonalMemoryAuthorizationV1,
    projection: MemoryRecordControlProjectionV1
  ): Promise<MemoryRevisionV2> => {
  ...
  const projectAfterMutation = async (): Promise<boolean> => {
    try {
  ...
  const handleEnrollment = async (
    request: PersonalMemoryCommandRequestV1,
    auth: PersonalMemoryAuthorizationV1,
    operation: 'enrollment.optIn' | 'enrollment.optOut'
  ): Promise<void> => {
    ...
    const candidateMode = operation === 'enrollment.optOut' || mode === 'explicit'
      ? 'off' as const
      : mode === 'shadow' ? 'shadow' as const : 'policy_approved' as const
    const ref = commandRef(auth.source, operation)
    const result = await options.facade.execute(facadeRequest(
      operation,
      auth,
      Object.freeze({
        schemaVersion: 1,
  ...
  const handleRemember = async (
    ...
    const result = await options.facade.execute(facadeRequest(
      'remember', auth, lifecyclePayload(auth, command)
    ))
  ...
  const handleCorrect = async (
    ...
    const result = await options.facade.execute(facadeRequest(
      'correct', auth, lifecyclePayload(auth, command)
    ))
    if (!mutationSucceeded(result)) {
      await request.replyText(mutationFailureText(result))
      return
  ...
  const handleRenew = async (
    ...
    const result = await options.facade.execute(facadeRequest(
      'renew', auth, lifecyclePayload(auth, command)
    ))
    if (!mutationSucceeded(result)) {
      await request.replyText(mutationFailureText(result))
      return
  ...
  const handleForget = async (
    ...
    const result = await options.facade.execute(facadeRequest(
      'forget', auth, lifecyclePayload(auth, command)
    ))
    if (!mutationSucceeded(result)) {
      await request.replyText(mutationFailureText(result))
      return
  ...
  const exportEnvelope = (
    auth: PersonalMemoryAuthorizationV1,
    command: ReturnType<typeof createMemoryExportCommandV1>
  ) => Object.freeze({
    schemaVersion: 1 as const,
    command,
    access: auth.access,
  ...
  const executeExport = async (
    ...
    return await options.facade.execute(facadeRequest(
      'export', auth, exportEnvelope(auth, command)
    ))
  }
  ...
  const handleExport = async (
    ...
    const delivery = await options.exportDelivery.redeemAndSend(
      claimed.result.handle,
      request.sendPrivateFile
    )
  ...
  const handleDelete = async (
    ...
    const result = await options.facade.execute(facadeRequest(
      'delete', auth, lifecyclePayload(auth, command)
    ))
    if (!mutationSucceeded(result)) {
      await request.replyText(mutationFailureText(result))
      return
