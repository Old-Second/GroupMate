...
export type Phase7VerificationCheckName =
  typeof PHASE_7_VERIFICATION_CHECKS[number]

export interface Phase7VerificationCommand {
  readonly name: Exclude<Phase7VerificationCheckName, 'security'>
  readonly file: string
  readonly arguments: readonly string[]
}

export interface Phase7VerificationCheck {
  readonly name: Phase7VerificationCheckName
  readonly mandatory: true
  readonly status: 'passed' | 'failed'
  readonly code: 'ok' | 'command_failed'
}

export interface Phase7VerificationResult {
  readonly checks: readonly Phase7VerificationCheck[]
  readonly passed: boolean
}

export interface Phase7VerificationOptions {
  readonly runCommand?: (command: Phase7VerificationCommand) => Promise<number>
  readonly securityAudit?: (projectRoot: string) => Promise<Phase7SecurityAuditResult>
  readonly distJavaScriptHash?: (projectRoot: string) => Promise<string>
}
...
function command (
  name: Phase7VerificationCommand['name'],
  file: string,
  arguments_: readonly string[]
): Phase7VerificationCommand {
...
Exclude<Phase7VerificationCheckName, 'security' | 'dist_reproducible'>,
Phase7VerificationCommand
>> = Object.freeze({
...
function result (
  name: Phase7VerificationCheckName,
  passed: boolean
): Phase7VerificationCheck {
  return Object.freeze({
    name,
    mandatory: true,
...
function defaultRunner (root: string): (
  command: Phase7VerificationCommand
) => Promise<number> {
  return async input => {
...
async function collectDistJavaScript (
  root: string,
  relative = ''
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (entry.isDirectory()) files.push(...await collectDistJavaScript(root, child))
...
function hashFrame (hash: ReturnType<typeof createHash>, value: Buffer): void {
...
export async function phase7DistJavaScriptHash (value: string): Promise<string> {
  const root = projectRoot(value)
  const dist = path.join(root, 'dist')
  const files = [...await collectDistJavaScript(dist)].sort()
  if (files.length === 0) throw new TypeError('Phase 7 dist JavaScript is missing')
...
export async function verifyPhase7 (
  value: string,
  options: Phase7VerificationOptions = {}
): Promise<Phase7VerificationResult> {
  const root = projectRoot(value)
  ...
  const checks: Phase7VerificationCheck[] = []
  ...
  const ordered = Object.freeze(PHASE_7_VERIFICATION_CHECKS.map(name => {
    ...
    if (check === undefined) throw new Error(`Phase 7 verification check missing: ${name}`)
    return check
  }))
  return Object.freeze({
    checks: ordered,
