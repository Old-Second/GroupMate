...
export interface Phase7SecurityAuditResult
  extends Omit<Phase6SecurityAuditResult, 'passed'> {
  readonly forbiddenRedactedBodyFields: readonly string[]
  readonly forbiddenProviderIsolationLeaks: readonly string[]
  readonly forbiddenContextArtifactLeaks: readonly string[]
  readonly invalidContentJournalBoundaries: readonly string[]
  readonly forbiddenPhase7Edges: readonly string[]
  readonly passed: boolean
}

export interface Phase7SecurityAuditOptions extends Phase6SecurityAuditOptions {}
...
const REDACTED_EXACT_ALLOWLIST: Readonly<Record<string, readonly string[]>> = Object.freeze({
...
const CONTENT_JOURNAL_PROJECTOR_PATH =
  'src/runtime/logging/content-journal-request-run-projector.ts'
const PHASE_7_THIN_SCRIPTS: Readonly<Record<string, string>> = Object.freeze({
...
function normalizeRelativePath (value: string): string {
  ...
  if (normalized === '' || normalized.startsWith('/') || normalized.includes('../') ||
    normalized.includes('\0')) throw new TypeError('Phase 7 security audit path is invalid')
  return normalized
}
...
function validateOverrides (
  value: Phase7SecurityAuditOptions['sourceOverrides']
...
function exactThinScript (source: string, target: string): boolean {
  return new RegExp(
    `^import \\{ main \\} from '../dist/verification/${escapeRegex(target)}';?\\nawait main\\(\\);?\\n?$`
  ).test(source)
}
...
function auditProviderIsolationSurface (
  relativePath: string,
  source: string,
  findings: FindingSet
): void {
  ...
  if (/\buser_id\b/.test(source)) finding(findings, relativePath, 'user_id')
...
function auditRedactedBodySurface (
  relativePath: string,
  source: string,
  findings: FindingSet
): void {
  ...
  for (const field of FORBIDDEN_REDACTED_BODY_FIELDS) {
    ...
    const persistedField = new RegExp(
      `\\breadonly\\s+(?:['"])?${escaped}(?:['"])?\\s*[?:]`
    )
    const rawProjection = new RegExp(
      `(?:^|[,{}\\n])\\s*(?:['"])?${escaped}(?:['"])?\\s*:\\s*` +
      '(?:request|checkpoint|event|record|artifact|trace|input)\\b',
      'm'
    )
...
function auditContextArtifactSurface (
  relativePath: string,
  source: string,
  findings: FindingSet
): void {
...
function auditContentJournal (
  ...
  const snapshot = methodSection(
    source,
    'export function snapshotModelRequestForJournal (',
    'export function snapshotModelTurnForJournal ('
  )
...
function auditContentJournalProjector (
  source: string | null,
  findings: FindingSet
): void {
  if (source === null) {
...
export async function auditPhase7SecurityBoundaries (
  projectRoot: string,
  ...
  const forbiddenRedactedBodyFields: FindingSet = new Set()
  const forbiddenProviderIsolationLeaks: FindingSet = new Set()
  const forbiddenContextArtifactLeaks: FindingSet = new Set()
  const invalidContentJournalBoundaries: FindingSet = new Set()
  const forbiddenPhase7Edges: FindingSet = new Set()
  ...
  auditContentJournalProjector(
    await readSource(CONTENT_JOURNAL_PROJECTOR_PATH),
