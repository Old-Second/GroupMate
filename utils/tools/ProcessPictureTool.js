...
export class ProcessPictureTool extends AbstractTool {
  ...
  func = async function (opts, e) {
    ...
    try {
      ...
      return `image processing failed: ${err.message}`
    }
    ...
    if (captionRes.status === 200) {
      ...
      return `image processing failed: upstream returned ${captionRes.status} ${captionRes.statusText || ''}, body preview=${previewBody(body)}`
    }
  }
}
