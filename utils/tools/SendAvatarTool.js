...
import { canSendToTarget, isTargetGroup, resolveTarget } from './ToolUtils.js'
...
export class SendAvatarTool extends AbstractTool {
  ...
  func = async function (opts, e) {
    let { qq, targetGroupIdOrQQNumber, sender } = opts
    ...
    return `the ${pictures.length > 1 ? 'users: ' + qq + '\'s avatar' : 'avatar'} has been sent to ${target}`
  }
