...
import { canSendToTarget, clampNumber, resolveTarget } from './ToolUtils.js'
export class SendVideoTool extends AbstractTool {
  ...
  func = async function (opts, e) {
    ...
    try {
      ...
      msg.push(Config.enableToolVideoDownload ? '\n视频在路上啦！' : '\n已发送视频信息和链接。')
      await group.sendMsg(msg)
      if (!Config.enableToolVideoDownload) {
        return `the video ${title.replace(/(<([^>]+)>)/ig, '')} information was shared to ${target}. video download is disabled.`
      }
      const videoResponse = await fetch(videoUrl, { headers })
      if (!videoResponse.ok) {
        return `fail to download video, but the video msg is found: ${msg}, error: ${videoResponse.status} ${videoResponse.statusText || ''}`
      }
      const contentType = videoResponse.headers.get('Content-Type') || ''
      if (!contentType.startsWith('video/')) {
        return `fail to download video, but the video msg is found: ${msg}, unsupported content-type=${contentType || 'unknown'}`
      }
      const maxBytes = clampNumber(Config.toolVideoMaxMB, 1, 200, 30) * 1024 * 1024
      const contentLength = Number(videoResponse.headers.get('Content-Length'))
      if (Number.isFinite(contentLength) && contentLength > maxBytes) {
        return `fail to download video, but the video msg is found: ${msg}, video is too large: ${contentLength} bytes`
      }
      const fileType = contentType.split('/')[1].split(';')[0] || 'mp4'
      let fileLoc = `data/chatgpt/videos/${bvid}.${fileType}`
      mkdirs('data/chatgpt/videos')
      const arrayBuffer = await videoResponse.arrayBuffer()
      if (arrayBuffer.byteLength > maxBytes) {
        return `fail to download video, but the video msg is found: ${msg}, video is too large: ${arrayBuffer.byteLength} bytes`
      }
