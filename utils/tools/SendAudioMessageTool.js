...
import { canSendToTarget, formatToolError, isTargetGroup, resolveTarget } from './ToolUtils.js'
...
export class SendAudioMessageTool extends AbstractTool {
  ...
  func = async function (opts, e) {
    ...
    let { pendingText, ttsMode, vitsModeRole, azureModeRole, voxModeRole, speakingEmotion, speakingEmotionDegree, targetGroupIdOrQQNumber, sender } = opts
    let sendable
    ...
    if (sendable) {
      ...
      try {
        ...
        return `failed to send audio, error: ${formatToolError(err)}`
      }
    } else {
