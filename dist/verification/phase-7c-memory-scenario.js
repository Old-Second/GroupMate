...
import { createMemoryLifecycleAuthorityRootV1, issueMemoryLifecycleActorCapabilityV1, issueMemoryMaintenanceCapabilityV1 } from '../agent/memory/memory-lifecycle-authority.js';
...
import { consumeMemoryExportDeliveryHandleV1, createMemoryExportCommandV1, createMemoryExportPortV1, MEMORY_EXPORT_MAX_CHUNK_BYTES_V1, MEMORY_EXPORT_MAX_WIRE_BYTES_V1, memoryExportStableResultHashV1 } from '../agent/memory/memory-export-port.js';
import { createMemoryMaintenanceCommandV1, createMemoryMaintenancePortV1 } from '../agent/memory/memory-maintenance-port.js';
import { createMemoryNamespaceV1, memoryNamespaceRefV1 } from '../agent/memory/memory-namespace.js';
import { createMemorySourceV1, createQqIdentitySnapshotV1 } from '../agent/memory/memory-domain.js';
import { MEMORY_LIFECYCLE_RESOURCE_LIMITS, MEMORY_RESOURCE_LIMITS } from '../agent/memory/memory-resource-limits.js';
import { openSqliteMemoryDatabaseV2 } from '../agent/memory/sqlite-memory-database.js';
import { createSqliteMemoryExportAdapterV1, MEMORY_EXPORT_ARTIFACT_CAPACITY_BYTES_V1 } from '../agent/memory/sqlite-memory-export.js';
import { createSqliteMemoryLifecycleMutationAdapterV1 } from '../agent/memory/sqlite-memory-lifecycle-mutation.js';
import { createSqliteMemoryLifecycleProposalAdapterV1 } from '../agent/memory/sqlite-memory-lifecycle-proposal.js';
...
export const PHASE_7C_NAMESPACE_LIFECYCLE_LOGICAL_LIMIT_BYTES = MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleCommandLedgerBytesPerNamespace +
...
function scenarioName(value) {
    if (typeof value !== 'string' ||
        !PHASE_7C_MEMORY_RESOURCE_SCENARIOS.includes(value)) {
        return fail();
    }
    return value;
}
export function validatePhase7cMemoryResourceSample(value, expectedScenario) {
    const input = exactRecord(value);
    const scenario = scenarioName(input.scenario);
    if (expectedScenario !== undefined && scenario !== expectedScenario)
        return fail();
    if (input.outcome !== PHASE_7C_MEMORY_RESOURCE_OUTCOMES[scenario])
        return fail();
    ...
    bounded(input.peakLifecycleAuditRecords, MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleCommandLedgerRecordsPerNamespace);
    bounded(input.peakLifecycleCommandRecords, MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleCommandLedgerRecordsPerNamespace);
    bounded(input.peakDeletionCheckpointRecords, MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleDeletionCheckpointsPerNamespace);
    bounded(input.peakExportJobRecords, MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleExportJobsPerNamespace);
    bounded(input.peakLifecycleLogicalBytes, PHASE_7C_NAMESPACE_LIFECYCLE_LOGICAL_LIMIT_BYTES);
    ...
    if (input.generatedArtifactBytes !== input.deliveredArtifactBytes &&
        scenario === 'streamingExport')
        return fail();
    if (scenario !== 'streamingExport' && (input.generatedArtifactBytes !== 0 || input.deliveredArtifactBytes !== 0))
        return fail();
    if (input.derivedLeakCount !== 0 || input.redisEvalCalls !== 0 ||
        input.residualArtifactFiles !== 0 || input.sqliteClosed !== true ||
        input.directoryRemoved !== true || input.timerResourceDelta !== 0)
        return fail();
    if (input.residualCarrierRecords !==
        PHASE_7C_MEMORY_EXPECTED_RESIDUAL_CARRIERS[scenario])
        return fail();
    nonnegativeInteger(input.residualCarrierRecords);
    return Object.freeze(value);
}
function commandRef(suffix) {
    return `command:${createHash('sha256').update(suffix, 'utf8').digest('hex')}`;
}
function plusMilliseconds(instant, milliseconds) {
...
function fixtureSource(observedAt) {
...
function actorAuthority(namespace, now, generation, actions) {
...
function actorEnvelope(namespace, command, now, actions, generation = 1) {
    const authority = actorAuthority(namespace, now, generation, actions);
    return Object.freeze({
        schemaVersion: 1,
        command,
        access: authority.access,
        authority: Object.freeze({ kind: 'actor', capability: authority.actor })
    });
}
function directMemory(scenario) {
...
function correction(before, changedAt) {
...
function renewal(before, changedAt) {
...
function recordForget(revision, occurredAt) {
...
function namespaceDelete(namespace, occurredAt) {
    return createMemoryLifecycleCommandV1({
...
function exportCommand(operation, namespace, now, values = {}) {
...
function exportEnvelope(namespace, command, now) {
...
function maintenanceEnvelope(namespace, now, operation, currentGeneration, targetGeneration, deletionRef) {
    ...
    const maintenance = issueMemoryMaintenanceCapabilityV1(createMemoryLifecycleAuthorityRootV1(() => true), {
        schemaVersion: 1,
        botInstanceId: namespace.botInstanceId,
        adapter: 'qq',
        accountId: namespace.accountId,
    ...
    return Object.freeze({ schemaVersion: 1, command, access: actor.access, maintenance });
}
...
function usageSnapshot(database) {
...
function derivedLeakCount(database) {
    const statements = [
    ...
    const wires = statements.flatMap(statement => database.prepare(statement).all().map(row => typeof row.wire === 'string' ? row.wire : fail()));
    return PHASE_7C_MEMORY_SENSITIVE_SENTINELS.reduce((total, sentinel) => total + wires.filter(wire => wire.includes(sentinel)).length, 0);
}
function residualCarrierRecords(database) {
    const row = database.prepare(`
    SELECT
...
function timeoutCount(resources) {
    return resources.filter(resource => resource === 'Timeout').length;
}
async function executeScenario(scenario, observe) {
    ...
    try {
        ...
        const seeded = await direct.execute(actorEnvelope(fixture.namespace, fixture.command, currentNow, ['propose_create', 'approve']));
        if (seeded.status !== 'stored')
            return fail();
        capture();
        if (scenario === 'lifecycleMutation') {
            ...
            if ((await mutation.execute(actorEnvelope(fixture.namespace, corrected.command, currentNow, ['correct']))).status !== 'stored')
                return fail();
            capture();
            currentNow = plusMilliseconds(currentNow, 1_000);
            const renewed = renewal(corrected.bundle.revision, currentNow);
            if ((await mutation.execute(actorEnvelope(fixture.namespace, renewed.command, currentNow, ['renew']))).status !== 'stored')
                return fail();
            capture();
        }
            ...
            const prepared = await port.execute(exportEnvelope(fixture.namespace, prepareCommand, currentNow));
            ...
            const deliverable = await port.execute(exportEnvelope(fixture.namespace, generateCommand, currentNow));
            if (deliverable.status !== 'deliverable')
                return fail();
            ...
            const claimed = await port.execute(exportEnvelope(fixture.namespace, claimCommand, currentNow));
            if (claimed.status !== 'delivery_claimed')
                return fail();
            const redeemed = await adapter.persistentDelivery.redeemOnce(consumeMemoryExportDeliveryHandleV1(claimed.handle));
            if (redeemed.status !== 'delivered')
                return fail();
            capture();
        }
        else {
            currentNow = plusMilliseconds(currentNow, 1_000);
            const forgotten = await mutation.execute(actorEnvelope(fixture.namespace, recordForget(fixture.bundle.revision, currentNow), currentNow, ['forget']));
            if (forgotten.status !== 'deletion_pending')
                return fail();
            capture();
            currentNow = plusMilliseconds(currentNow, 1_000);
            const deleted = await mutation.execute(actorEnvelope(fixture.namespace, namespaceDelete(fixture.namespace, currentNow), currentNow, ['delete_namespace']));
            ...
            currentNow = plusMilliseconds(currentNow, 1_000);
            if ((await maintenance.execute(maintenanceEnvelope(fixture.namespace, currentNow, 'namespace.scrubDeleted', 2, 1, deleted.receipt.deletionRef))).status !== 'completed')
                return fail();
            currentNow = plusMilliseconds(currentNow, 1_000);
            if ((await maintenance.execute(maintenanceEnvelope(fixture.namespace, currentNow, 'namespace.verifyScrubbed', 2, 1, deleted.receipt.deletionRef))).status !== 'completed')
                return fail();
            currentNow = plusMilliseconds(currentNow, 1_000);
            if ((await maintenance.execute(maintenanceEnvelope(fixture.namespace, currentNow, 'deletion.checkpoint', 2, 2, null))).status !== 'completed')
                return fail();
            capture();
        }
        leaks = Math.max(leaks, derivedLeakCount(store.database));
        ...
        const maximum = (key) => Math.max(0, ...snapshots.map(snapshot => snapshot[key]));
        return Object.freeze({
...
export async function runPhase7cMemoryResourceScenario(options = {}) {
