...
import { MEMORY_EXPORT_MAX_CHUNK_BYTES_V1, MEMORY_EXPORT_MAX_WIRE_BYTES_V1 } from '../agent/memory/memory-export-port.js';
import { MEMORY_LIFECYCLE_RESOURCE_LIMITS } from '../agent/memory/memory-resource-limits.js';
import { MEMORY_EXPORT_ARTIFACT_CAPACITY_BYTES_V1, MEMORY_EXPORT_LEASE_TTL_MS_V1 } from '../agent/memory/sqlite-memory-export.js';
import { auditPhase7SecurityBoundaries } from './phase-7-security-audit.js';
import { auditPhase7bMemoryWiring, phase7bOutboxSurfaceIsBodyFree } from './phase-7b-memory-report.js';
import { PHASE_7C_MEMORY_RESOURCE_SCENARIOS, PHASE_7C_MEMORY_SENSITIVE_SENTINELS, runPhase7cMemoryResourceScenario, validatePhase7cMemoryResourceSample } from './phase-7c-memory-scenario.js';
...
function scenarioReport(entries) {
    ...
    return Object.freeze({
        ...
        rss: Object.freeze({
            ...
            retainedDelta: statistics(samples.map(sample => (sample.retainedRssBytes - sample.baselineRssBytes))),
            peakDelta: statistics(samples.map(sample => (sample.peakRssBytes - sample.baselineRssBytes)))
        }),
        ...
        peakCanonicalLogicalBytes: statistics(samples.map(sample => sample.peakCanonicalLogicalBytes)),
        peakLifecycleLogicalBytes: statistics(samples.map(sample => sample.peakLifecycleLogicalBytes))
    });
}
export function buildPhase7cMemoryResourceReport(value) {
    const input = validateResourceInput(value);
    const scenarios = Object.freeze(Object.fromEntries(PHASE_7C_MEMORY_RESOURCE_SCENARIOS.map(scenario => [
        scenario,
        scenarioReport(input[scenario])
    ])));
    const samples = PHASE_7C_MEMORY_RESOURCE_SCENARIOS.flatMap(scenario => input[scenario].map(entry => entry.sample));
    const processIds = Object.freeze(PHASE_7C_MEMORY_RESOURCE_SCENARIOS.flatMap(scenario => input[scenario].map(entry => entry.processId)));
    ...
    const gates = Object.freeze({
        everySampleRss: samples.every(sample => sample.retainedRssBytes - sample.baselineRssBytes <= PHASE_7C_MEMORY_RSS_LIMIT_BYTES &&
            sample.peakRssBytes - sample.baselineRssBytes <= PHASE_7C_MEMORY_RSS_LIMIT_BYTES),
        cleanupComplete: samples.every(sample => sample.sqliteClosed && sample.directoryRemoved &&
        ...
        noRedisOrTimerSideEffects: samples.every(sample => sample.redisEvalCalls === 0 && sample.timerResourceDelta === 0),
        lifecycleLimitsExact: sameNumericRecord(MEMORY_LIFECYCLE_RESOURCE_LIMITS, PHASE_7C_EXPECTED_MEMORY_LIFECYCLE_RESOURCE_LIMITS),
        exportLimitsExact: sameNumericRecord(exportLimits, PHASE_7C_EXPECTED_EXPORT_RESOURCE_LIMITS)
    });
    return Object.freeze({
        schemaVersion: 1,
        samplesPerScenario: PHASE_7C_MEMORY_RESOURCE_SAMPLES,
...
function exactReadonlyInterface(source, interfaceName, fields, forbiddenTypeNames) {
    ...
    if ((declaration.typeParameters?.length ?? 0) !== 0 ||
        ...
        declaration.members.length !== fields.length)
        return false;
    const actual = [];
    for (const member of declaration.members) {
        ...
        if (forbiddenTypeNames.some(name => new RegExp(`\\b${name}\\b`).test(typeText)))
            return false;
...
export function phase7cLifecycleAuditSurfaceIsBodyFree(source) {
    return typeof source === 'string' && exactReadonlyInterface(source, 'MemoryLifecycleAuditV1', AUDIT_FIELDS, FORBIDDEN_BODY_TYPE_NAMES);
}
export function phase7cDeletionReceiptSurfaceIsBodyFree(source) {
    return typeof source === 'string' && exactReadonlyInterface(source, 'DeletionMutationReceiptV1', DELETION_RECEIPT_FIELDS, FORBIDDEN_BODY_TYPE_NAMES);
}
export function phase7cLifecycleCommandWireIsHashOnly(source) {
    return typeof source === 'string' && exactReadonlyInterface(source, 'MemoryLifecycleCommandWireV1', COMMAND_WIRE_FIELDS, FORBIDDEN_BODY_TYPE_NAMES);
}
export function phase7cLifecycleCommandLedgerIsBodyFree(source) {
    if (typeof source !== 'string' || Buffer.byteLength(source, 'utf8') > MAX_AUDIT_FILE_BYTES) {
        return false;
    }
...
export async function auditPhase7cLifecycleSurfaces(projectRootValue) {
...
export function parsePhase7cMemoryChildObservation(value, scenario) {
    const fail = () => {
    ...
    try {
        ...
        if (!sameStrings(Object.keys(input).sort(), ['failed', 'processId', 'status', 'stdout']) ||
            ...
            PHASE_7C_MEMORY_SENSITIVE_SENTINELS.some(sentinel => input.stdout.includes(sentinel)))
            return fail();
        return Object.freeze({
            processId: Number(input.processId),
            sample: validatePhase7cMemoryResourceSample(JSON.parse(input.stdout.trim()), scenario)
        });
    }
    catch {
        return fail();
...
export async function verifyPhase7cMemory(projectRootValue, options = {}) {
    const root = safeProjectRoot(projectRootValue);
    const samples = options.samples ?? Object.freeze(Object.fromEntries(PHASE_7C_MEMORY_RESOURCE_SCENARIOS.map(scenario => [
        scenario,
        Object.freeze(Array.from({ length: PHASE_7C_MEMORY_RESOURCE_SAMPLES }, () => runFreshMemorySample(root, scenario)))
    ])));
...
export async function main() {
    ...
    if (arguments_.length === 2 && arguments_[0] === RESOURCE_SAMPLE_ARGUMENT) {
        ...
        if (!PHASE_7C_MEMORY_RESOURCE_SCENARIOS.includes(scenario))
