...
import { decodeMemoryOutboxEventV1, encodeMemoryRecordV1 } from '../agent/memory/memory-codec.js';
import { createMemoryRecordV1, createMemoryRevisionV1, createMemorySourceV1, createQqIdentitySnapshotV1 } from '../agent/memory/memory-domain.js';
...
import { MEMORY_HOT_CACHE_KEYS, MEMORY_HOT_CACHE_LUA_SCRIPT, MEMORY_HOT_CACHE_STATIC_BYTES, RedisMemoryHotCache, memoryHotCacheHeadFieldV1, memoryHotCacheHeadValueV1, memoryHotCacheIndexEntryBytesV1, memoryHotCacheMetadataEntryBytesV1, memoryHotCacheNamespaceFieldV1, memoryHotCacheRecordEntryBytesV1, memoryHotCacheRecordFieldV1 } from '../agent/memory/redis-memory-hot-cache.js';
import { openSqliteMemoryDatabaseV1 } from '../agent/memory/sqlite-memory-database.js';
...
export function normalizePhase7bMaxRssBytes(rawMaxRss, currentRssBytes) {
...
export function validatePhase7bMemoryResourceSample(value, expectedScenario) {
    ...
    if (sample.scenario !== 'fixtureLifecycle' ||
        ...
        sample.outcome !== PHASE_7B_MEMORY_RESOURCE_OUTCOMES.fixtureLifecycle)
        return fail();
    ...
    boundedNonnegativeInteger(sample.sqliteWalFileBytes, MEMORY_RESOURCE_LIMITS.sqliteWalJournalLimitBytes);
    ...
    boundedPositiveInteger(sample.canonicalRevisionRecords, MEMORY_RESOURCE_LIMITS.memoryRetainedRevisions);
    boundedPositiveInteger(sample.canonicalLogicalBytes, MEMORY_RESOURCE_LIMITS.namespaceCanonicalLogicalBytes);
    ...
    boundedNonnegativeInteger(sample.redisDynamicMetadataBytes, MEMORY_RESOURCE_LIMITS.redisHotLogicalBytes);
    if (positiveInteger(sample.redisStaticBytes) !== MEMORY_HOT_CACHE_STATIC_BYTES)
        return fail();
    const redisLogicalBytes = boundedPositiveInteger(sample.redisLogicalBytes, MEMORY_RESOURCE_LIMITS.redisHotLogicalBytes);
    ...
    if (nonnegativeInteger(sample.contentLeakCount) !== 0 ||
        nonnegativeInteger(sample.identityLeakCount) !== 0)
        return leakDetected();
    if (sample.sqliteClosed !== true || sample.redisReleased !== true)
        return lifecycleFailed();
    if (nonnegativeInteger(sample.timerResourceDelta) !== 0)
        return lifecycleFailed();
...
export function buildPhase7bMemoryOutboxLeakSentinels(record, revision) {
...
export function countPhase7bMemoryOutboxLeaks(wires, sentinels) {
    if (!Array.isArray(wires) || wires.length > MEMORY_RESOURCE_LIMITS.operationBatchRecords ||
        ...
        [...wires, ...sentinels.content, ...sentinels.identity].some(value => (typeof value !== 'string' || value === '')))
        return fail();
    const count = (values) => values.reduce((total, sentinel) => total + wires.filter(wire => wire.includes(sentinel)).length, 0);
    return Object.freeze({
        contentLeakCount: count(sentinels.content),
        identityLeakCount: count(sentinels.identity)
    });
}
class ScenarioRedisClient {
    #put;
    #peek;
    #confirm;
    #usage;
    ...
    constructor(record) {
        ...
        const dynamicMetadataBytes = memoryHotCacheMetadataEntryBytesV1(generationField, generation) + memoryHotCacheMetadataEntryBytesV1(headField, headValue);
        this.#wire = wire;
    ...
    async eval(script, options) {
        if (this.#released || script !== MEMORY_HOT_CACHE_LUA_SCRIPT ||
            !exactStrings(options.keys, MEMORY_HOT_CACHE_KEYS))
            return fail();
        const expected = [this.#put, this.#peek, this.#confirm, ['usage']][this.#operationIndex];
        if (expected === undefined || expected === null ||
            !exactStrings(options.arguments, expected))
            return fail();
        ...
        if (options.arguments[0] === 'confirm_hit')
            return 'hit';
        if (this.#usage === null)
            return fail();
        return Object.freeze([
...
function pragmaInteger(store, name) {
...
async function runFixture() {
    ...
    try {
        ...
        const capability = issueMemoryAccessCapabilityV1(createMemoryAccessCapabilityIssuerV1(() => true), {
            schemaVersion: 1,
            botInstanceId: BOT_INSTANCE_ID,
            adapter: 'qq',
            accountId: ACCOUNT_ID,
            scene: { kind: 'private', peerUserId: SUBJECT_USER_ID }
        }, [fixture.namespace], FIXED_NOW);
        ...
        const dynamicLeakSentinels = buildPhase7bMemoryOutboxLeakSentinels(fixture.record, revision);
    ...
    `).all();
        ...
        if (outboxRows.reduce((bytes, row) => bytes + Number(row.logical_bytes), 0) !==
            usage.value.outboxLogicalBytes)
            return fail();
            ...
            try {
                if (client !== undefined) {
                    ...
                    try {
                        ...
                        try {
                            redisReleased = client.isReleased();
                        }
                        catch { }
                    }
                }
    ...
    if (measured === undefined || !sqliteClosed || !redisReleased || !directoryRemoved)
        return fail();
    return Object.freeze({ ...measured, sqliteClosed, redisReleased });
}
export async function runPhase7bMemoryResourceScenario(options = {}) {
    ...
    const peakRssBytes = Math.max(...observations, normalizePhase7bMaxRssBytes(resourceUsage().maxRSS, retainedRssBytes));
