...
function parseResp(buffer, offset = 0) {
    ...
    if (marker === '+' || marker === '-' || marker === ':') {
        if (marker === '-')
            throw new Error('Redis command was rejected');
        if (marker === ':') {
            const value = Number(header);
            if (!Number.isSafeInteger(value))
                throw new TypeError('Redis integer response is invalid');
    ...
    if (marker === '$') {
        ...
        if (length === -1)
            return Object.freeze({ value: null, offset: bodyOffset });
...
class RespRedisClient {
    ...
    async get(key) {
        ...
        if (result !== null && typeof result !== 'string')
            throw new TypeError('Redis GET result is invalid');
        return result;
    }
    async set(key, value, options) {
        const parts = ['SET', key, value];
        if (options?.EX !== undefined)
            parts.push('EX', String(options.EX));
        if (options?.NX === true)
            parts.push('NX');
        const result = await this.#command(parts);
        if (result !== null && result !== 'OK')
            throw new TypeError('Redis SET result is invalid');
        return result;
    }
    async del(key) {
    ...
    async ttl(key) {
        ...
        if (!Number.isSafeInteger(result))
            throw new TypeError('Redis TTL result is invalid');
        return Number(result);
    }
    async scan(cursor, options) {
        ...
        if (!Number.isSafeInteger(next) || next < 0)
            throw new TypeError('Redis SCAN cursor is invalid');
        return { cursor: next, keys: result[1] };
    }
    ...
    async #command(parts) {
        ...
        return await new Promise((resolve, reject) => {
            ...
            const receive = (chunk) => {
                ...
                try {
                    ...
                    for (const reply of replies.slice(0, -1)) {
                        if (reply !== 'OK')
                            throw new Error('Redis connection setup failed');
                    }
            ...
            socket.once('end', () => {
                if (!settled)
                    finish(new Error('Redis smoke connection ended early'));
            });
        });
    }
}
export async function createPhase6RedisClient(url) {
    return Object.freeze({
...
export async function runPhase6RedisSmoke(options = {}) {
    ...
    try {
        ...
        const initial = await Promise.all(candidates.map(async (candidate) => (await traceStore.upsertEngine(candidate))));
        if (initial.some(receipt => receipt.kind !== 'stored')) {
            throw new Error('Redis smoke could not store fresh trace records');
        }
        const repeated = await Promise.all(candidates.map(async (candidate) => (await traceStore.upsertEngine(candidate))));
        const idempotent = repeated.every(receipt => receipt.kind === 'unchanged');
        const loaded = await Promise.all(candidates.map(async (candidate) => (await traceStore.load(candidate.runRef))));
        ...
        if (!ttlBounded)
            throw new Error('Redis smoke TTL check failed');
        return Object.freeze({
            schemaVersion: 1,
            kind: 'passed',
            records: 2,
