...
import { MEMORY_HOT_CACHE_STATIC_BYTES } from '../agent/memory/redis-memory-hot-cache.js';
...
import { PHASE_7B_MEMORY_RESOURCE_SCENARIOS, PHASE_7B_MEMORY_SQLITE_SHM_LIMIT_BYTES, runPhase7bMemoryResourceScenario, validatePhase7bMemoryResourceSample } from './phase-7b-memory-scenario.js';
...
function sourceHasParseDiagnostics(sourceFile) {
    const diagnostics = sourceFile.parseDiagnostics;
    return diagnostics === undefined || diagnostics.length !== 0;
}
export function phase7bMemoryControlFieldIsForbidden(value) {
...
export function buildPhase7bMemoryResourceReport(value) {
    const input = validateResourceInput(value);
    const scenarios = Object.freeze(Object.fromEntries(PHASE_7B_MEMORY_RESOURCE_SCENARIOS.map(scenario => [
        scenario,
        scenarioReport(input[scenario])
    ])));
    const processIds = Object.freeze(PHASE_7B_MEMORY_RESOURCE_SCENARIOS.flatMap(scenario => (input[scenario].map(entry => entry.processId))));
    const samples = PHASE_7B_MEMORY_RESOURCE_SCENARIOS.flatMap(scenario => (input[scenario].map(entry => entry.sample)));
    const gates = Object.freeze({
        everySampleRss: samples.every(sample => sample.retainedRssBytes - sample.baselineRssBytes <= PHASE_7B_MEMORY_RSS_LIMIT_BYTES &&
        ...
        sqlitePageCache: samples.every(sample => sample.sqlitePageCacheBytes === MEMORY_RESOURCE_LIMITS.sqlitePageCacheBytes),
        sqliteFileLimits: samples.every(sample => sample.sqliteMainFileBytes > 0 &&
        ...
        canonicalLimits: samples.every(sample => sample.canonicalActiveRecords > 0 &&
        ...
        outboxLimits: samples.every(sample => sample.outboxRecords > 0 &&
        ...
        redisLimits: samples.every(sample => sample.redisRecords > 0 &&
        ...
        noSensitiveProjection: samples.every(sample => sample.contentLeakCount === 0 && sample.identityLeakCount === 0),
        handlesReleased: samples.every(sample => sample.sqliteClosed && sample.redisReleased && sample.timerResourceDelta === 0),
        memoryResourceLimits: sameNumericRecord(MEMORY_RESOURCE_LIMITS, PHASE_7B_EXPECTED_MEMORY_RESOURCE_LIMITS)
    });
    return Object.freeze({
        schemaVersion: 1,
        samplesPerScenario: PHASE_7B_MEMORY_RESOURCE_SAMPLES,
...
async function boundedTypeScriptTree(root, relativeDirectory, maximumFiles) {
    const files = [];
    let complete = true;
...
function localImportResolution(owner, specifier, sources) {
...
function externalPluginSpecifier(value) {
...
function computedImportBoundaryIsClosed(sourceFile) {
    const declarations = sourceFile.statements.filter((node) => (ts.isFunctionDeclaration(node) && node.name?.text === 'importFirst'));
    if (declarations.length !== 1)
        return false;
    const printer = ts.createPrinter({ removeComments: true, newLine: ts.NewLineKind.LineFeed });
    if (printer.printNode(ts.EmitHint.Unspecified, declarations[0], sourceFile) !==
        PHASE_7B_IMPORT_FIRST_SURFACE)
        return false;
    const calls = [];
    const computedImports = [];
    let identifierCount = 0;
    const visit = (node) => {
        if (ts.isIdentifier(node) && node.text === 'importFirst')
            identifierCount += 1;
        if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) &&
            node.expression.text === 'importFirst')
            calls.push(node);
    ...
    if (calls.length !== 3 || identifierCount !== calls.length + 1 ||
        computedImports.length !== 1)
        return false;
    let owner = computedImports[0].parent;
    while (owner !== undefined && owner !== sourceFile && owner !== declarations[0]) {
        owner = owner.parent;
    }
    if (owner !== declarations[0])
        return false;
    const specifiers = [];
...
export function phase7bComputedImportBoundaryIsClosed(value) {
    if (typeof value !== 'string' || Buffer.byteLength(value, 'utf8') > MAX_AUDIT_FILE_BYTES) {
        return false;
    }
    const sourceFile = ts.createSourceFile('src/runtime/tools/yunzai-tool-runtime.ts', value, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
    return !sourceHasParseDiagnostics(sourceFile) && computedImportBoundaryIsClosed(sourceFile);
}
function sourceImportSpecifiers(relativePath, source) {
...
export function auditPhase7bProductionReachability(value) {
    ...
    for (const relativePath of paths) {
        if (!/^src\/(?:[a-z0-9._-]+\/)*[a-z0-9._-]+\.ts$/i.test(relativePath) ||
            ...
            Buffer.byteLength(input[relativePath], 'utf8') > MAX_AUDIT_FILE_BYTES) {
            throw new TypeError('Phase 7B production source graph is invalid');
        }
        sources[relativePath] = input[relativePath];
    }
    ...
    while (pending.length > 0 && complete) {
        ...
        visited.add(current);
        const imports = sourceImportSpecifiers(current, sources[current]);
        if (!imports.complete) {
            complete = false;
            break;
        }
        for (const specifier of imports.specifiers) {
            ...
            if (resolved.kind === 'resolved' && resolved.relativePath !== undefined &&
                !visited.has(resolved.relativePath))
                pending.push(resolved.relativePath);
        }
    }
...
function exactInterfaceProperties(source, interfaceName, expected) {
    const sourceFile = ts.createSourceFile(`${interfaceName}.ts`, source, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
    if (sourceHasParseDiagnostics(sourceFile))
        return false;
    const declarations = sourceFile.statements.filter((node) => (ts.isInterfaceDeclaration(node) && node.name.text === interfaceName));
    if (declarations.length !== 1)
        return false;
    ...
    return declaration.members.every((member, index) => {
        ...
        if (modifiers.length !== 1 || modifiers[0] !== ts.SyntaxKind.ReadonlyKeyword)
            return false;
...
export function phase7bBridgeDependenciesAreClosed(value) {
    if (typeof value !== 'string' || Buffer.byteLength(value, 'utf8') > MAX_AUDIT_FILE_BYTES) {
        return false;
    }
...
function normalizedImportTarget(owner, specifier) {
    if (!specifier.startsWith('.'))
        return specifier;
    const joined = path.posix.normalize(path.posix.join(path.posix.dirname(owner), specifier));
    return joined.endsWith('.js') ? `${joined.slice(0, -3)}.ts` : joined;
}
export function phase7bMemoryTelemetrySourceFindings(relativePath, value, role) {
    ...
    let findings = imports.specifiers.filter(specifier => {
        ...
        return role === 'memory'
            ? /(?:runtime\/(?:observability|logging)|safe-chat-logging|request-observation|metrics|content-journal)/.test(target)
            : target.includes('agent/memory/');
    }).length;
    if (role === 'memory') {
        const sourceFile = ts.createSourceFile(relativePath, value, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
        const visit = (node) => {
...
async function memoryTelemetryEdges(root) {
    const memoryTree = await boundedTypeScriptTree(root, 'src/agent/memory', 128);
    if (!memoryTree.complete)
        return 1;
    let findings = 0;
    for (const file of memoryTree.files) {
        findings += phase7bMemoryTelemetrySourceFindings(file.relativePath, file.source, 'memory');
    }
    ...
    if (!observationTree.complete) {
        ...
        for (const file of observationTree.files) {
            findings += phase7bMemoryTelemetrySourceFindings(file.relativePath, file.source, 'consumer');
        }
    }
    ...
    if (contentJournal === null) {
        ...
        findings += phase7bMemoryTelemetrySourceFindings('src/agent/run/run-content-journal.ts', contentJournal, 'consumer');
    }
    const memoryDomain = await boundedSource(root, 'src/agent/memory/memory-domain.ts');
    if (!phase7bOutboxSurfaceIsBodyFree(memoryDomain))
        findings += 1;
    return findings;
}
export function phase7bOutboxSurfaceIsBodyFree(value) {
    if (typeof value !== 'string' || Buffer.byteLength(value, 'utf8') > MAX_AUDIT_FILE_BYTES) {
        return false;
    }
    return exactInterfaceProperties(value, 'MemoryOutboxEventV1', PHASE_7B_EXPECTED_OUTBOX_FIELDS);
}
async function runColdImportAudit(root) {
    ...
    const productionModule = new URL('../runtime/production-yunzai-agent.js', import.meta.url).href;
...
`;
    try {
        if (modules.length === 0)
            throw new TypeError('cold import modules are unavailable');
        ...
        const value = exactRecord(JSON.parse(result.stdout.trim()), 'cold import result');
        ...
        if (![timerCalls, redisEvalCalls, createdFiles].every(current => Number.isSafeInteger(current) && current >= 0)) {
...
export async function auditPhase7bMemoryWiring(projectRootValue) {
    ...
    const sources = Object.fromEntries(await Promise.all(Object.entries(paths).map(async ([key, value]) => ([key, await boundedSource(root, value)]))));
    ...
    let reachability = Object.freeze({
        complete: false,
        reachableFiles: 0,
        forbiddenMemoryModules: 0,
        forbiddenContextSourceModules: 0
    ...
    if (sourceTree.complete) {
        try {
            reachability = auditPhase7bProductionReachability(Object.fromEntries(sourceTree.files.map(file => [file.relativePath, file.source])));
        }
        catch {
    ...
    const runtimeSourceGraph = Object.fromEntries(runtimeTree.files.map(file => [file.relativePath, file.source]));
    ...
    const toolFactories = [...toolSource.matchAll(/\b(create[A-Z][A-Za-z0-9]*Tool)\(/g)]
        .map(match => match[1]);
    const forbiddenMemoryToolFactories = toolFactories.filter(name => /Memory|Remember|Forget|Qdrant/.test(name)).length;
    ...
    let productionToolNames = Object.freeze([]);
    try {
...
export function parsePhase7bMemoryChildObservation(value) {
    const fail = () => {
    ...
    try {
        ...
        const parsed = JSON.parse(input.stdout.trim());
        return Object.freeze({
...
function runFreshMemorySample(root) {
...
export async function verifyPhase7bMemory(projectRootValue, options = {}) {
    const root = safeProjectRoot(projectRootValue);
    const samples = options.samples ?? Object.freeze({
        fixtureLifecycle: Object.freeze(Array.from({ length: PHASE_7B_MEMORY_RESOURCE_SAMPLES }, () => runFreshMemorySample(root)))
    });
