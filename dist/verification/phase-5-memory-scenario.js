...
import { createOpenAICompatibleCompletionFacade } from '../runtime/completion-facade.js';
...
function fixtureResponse(requestNumber) {
...
function positiveSettleMs(value) {
...
export async function runPhase5MemoryScenario(scenario, options = {}) {
    ...
    const createFacade = () => createOpenAICompatibleCompletionFacade({
    ...
    if (scenario === 'singleRun' || scenario === 'checkpointRecovery') {
        if (scenario === 'checkpointRecovery')
            retained.push(checkpointFixture());
        ...
        try {
            await Promise.race([
                ...
                new Promise((_resolve, reject) => {
                    barrierTimer = setTimeout(() => reject(new Error('timed out waiting for two fixture requests')), 1_000);
                    barrierTimer.unref?.();
                })
            ]);
            observations.push(memoryUsage().rss);
