...
import { createMemoryExtractionJobV1, memoryCandidateValueRejectionReasonV1, memoryCredentialRejectionReasonV1 } from '../agent/memory/memory-candidate-pipeline.js';
import { createMemoryNamespaceV1 } from '../agent/memory/memory-namespace.js';
...
export function evaluatePhase7dRetrievalCorpus(value) {
...
export function evaluatePhase7dCandidateCorpus(value) {
    ...
    for (const [index, raw] of value.entries()) {
        ...
        if (![
            ...
            throw new TypeError('Phase 7D candidate outcome is invalid');
        ...
        if (observedStored)
            observedAdmitted += 1;
        if (!expectedStored && observedStored)
            falsePositiveCount += 1;
        if (outcome === 'rejected_credential')
        ...
        exact = exact && outcome === expected.expectedOutcome &&
            (!observedStored || (input.provenancePreserved === true && input.conflictPreserved === true));
    }
...
export function evaluatePhase7dDeletionCorpus(value) {
    const input = exactRecord(value, PHASE_7D_DELETION_CARRIERS, 'Phase 7D deletion corpus');
    const residualRecords = PHASE_7D_DELETION_CARRIERS.reduce((total, carrier) => (total + nonnegativeInteger(input[carrier], `Phase 7D ${carrier} residual`)), 0);
...
function initializeLexicalRows(database) {
    database.prepare(`
  ...
  `);
    ...
    for (const [index, corpus] of PHASE_7D_RETRIEVAL_CORPUS.entries()) {
        ...
        insert.run(namespaceRef, `memory:${String(index + 1).repeat(64)}`, String(index + 1).repeat(64), body, Buffer.byteLength(body, 'utf8'), Date.parse(FIXED_NOW) + index);
    }
    database.prepare(`
    UPDATE lexical_index_state
...
function observeRetrievalCorpus(database, observeLatency) {
    const allowedNamespace = 'a'.repeat(64);
...
function observeCandidateCorpus() {
    ...
    const credentialRejected = credentialTexts.map(value => (memoryCredentialRejectionReasonV1(value) !== null));
    return evaluatePhase7dCandidateCorpus(PHASE_7D_CANDIDATE_CORPUS.map(corpus => {
        ...
        if (corpus.caseId.startsWith('credential_')) {
            ...
            if (credentialRejected[credentialIndex] !== true)
                outcome = 'shadow_stored';
        }
        return Object.freeze({
            caseId: corpus.caseId,
...
export function validatePhase7dMemoryResourceSample(value, expectedScenario) {
    const input = exactRecord(value, RESOURCE_SAMPLE_FIELDS, 'Phase 7D memory resource sample');
    const scenario = input.scenario;
    if (typeof scenario !== 'string' || !PHASE_7D_MEMORY_RESOURCE_SCENARIOS.includes(scenario) || (expectedScenario !== undefined && scenario !== expectedScenario)) {
...
export async function runPhase7dMemoryResourceScenario(options = {}) {
    ...
    const observeLatency = (operation) => {
        const before = monotonicNow();
        operation();
        maximumOperationLatencyMicros = Math.max(maximumOperationLatencyMicros, Math.ceil((monotonicNow() - before) * 1_000));
        observe();
    };
    const observeAsyncLatency = async (operation) => {
        const before = monotonicNow();
    ...
    try {
        ...
        if (scenario === 'retrievalCorpus') {
            ...
            if (!retrieval.passed)
                throw new TypeError('Phase 7D retrieval resource corpus failed');
        }
            ...
            for (const tag of ['one', 'two', 'three']) {
                ...
                if (result.status !== 'stored')
                    throw new TypeError('Phase 7D candidate queue failed');
            }
            const rejected = await observeAsyncLatency(async () => await queue.enqueue(createResourceJob('secret', 'API key: sk-example-1234567890')));
            ...
            const cache = new Map([[queued.jobId, 'opaque']]);
            const context = [queued.jobId];
            lexical.database.exec('DELETE FROM lexical_documents');
            const claim = await observeAsyncLatency(async () => await queue.claim('phase7d-delete-worker', 1));
            ...
            if (ack.status !== 'acked')
                throw new TypeError('Phase 7D deletion queue ack failed');
            cache.clear();
            context.length = 0;
            const canonicalHeads = canonical.database.prepare('SELECT count(*) AS count FROM heads').get()?.count;
            ...
            if (!deletion.passed)
                throw new TypeError('Phase 7D deletion resource corpus failed');
            observe();
        }
        ...
        if (!sqliteClosed)
            throw new TypeError('Phase 7D SQLite resources remained open');
