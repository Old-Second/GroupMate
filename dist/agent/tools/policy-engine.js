...
const mediaActions = Object.freeze({
    draw: 'image',
...
function currentTarget(target, facts) {
    if (facts.channel.kind === 'private')
        return target.kind === 'private' && target.userId === facts.channel.userId;
    return (target.kind === 'group' || target.kind === 'member' || target.kind === 'message') &&
        target.groupId === facts.channel.groupId;
}
function explicitTarget(target, facts, intent) {
    if (target.kind === 'none')
        return true;
    if (target.kind === 'member') {
        if (target.userId === facts.actor.userId)
            return true;
...
function actorCanManageMember(facts) {
    if (facts.actor.isBotMaster)
        return true;
    if (facts.actorGroupRole === 'owner')
        return facts.targetRole === 'admin' || facts.targetRole === 'member';
    if (facts.actorGroupRole === 'admin')
        return facts.targetRole === 'member';
    return false;
}
function hardManagementGate(input) {
    ...
    if (facts.channel.kind !== 'group' || target.groupId !== facts.channel.groupId)
        return deny('target_invalid');
    ...
    if (target.kind === 'member') {
        if (target.userId === facts.botId || facts.targetIsBotMaster)
            return deny('target_protected');
        const isSelf = target.userId === facts.actor.userId;
        if (isSelf && definition.name === 'jinyan') {
            const seconds = input.input.seconds;
            if (!Number.isInteger(seconds) || typeof seconds !== 'number' || seconds < 0)
                return deny('invalid_arguments');
            if (seconds === 0)
                return deny('self_unmute_denied');
            if (seconds > 60)
                return deny('self_mute_duration_exceeded');
            return null;
        }
        if (isSelf && definition.name === 'editCard')
            return null;
        if (!actorCanManageMember(facts))
            return deny(facts.actorGroupRole === 'member' && !facts.actor.isBotMaster ? 'permission_denied' : 'role_hierarchy_denied');
        if (definition.name === 'kickOut' && facts.targetRole !== 'member')
            return deny('role_hierarchy_denied');
    }
    if (definition.name === 'kickOut' && !facts.actor.isBotMaster && facts.actorGroupRole !== 'owner') {
        return deny('permission_denied');
    }
    if (definition.name === 'setTitle') {
        if (facts.botGroupRole !== 'owner')
            return deny('bot_permission_denied');
...
function requiresSafeApproval(input) {
    const { definition, target, facts } = input;
    if (definition.name === 'sendMessage' || definition.name === 'kickOut' || definition.name === 'handleMsg')
        return true;
    if (target.kind === 'member' && target.userId !== facts.actor.userId &&
        ['jinyan', 'editCard', 'setTitle'].includes(definition.name))
        return true;
    return false;
}
export class ToolPolicyEngine {
    decide(input) {
        ...
        if (input.definition.effect === 'read_only') {
            if (input.definition.permission === 'any_user')
                return allowed();
            ...
            if (input.definition.permission === 'bot_master_cross_channel') {
                if (!input.facts.actor.isBotMaster)
                    return deny('permission_denied');
                if ((input.target.kind !== 'group' && input.target.kind !== 'private') ||
                    currentTarget(input.target, input.facts) || !input.facts.targetExists ||
                    !explicitTarget(input.target, input.facts, input.intent))
                    return deny('target_invalid');
                return allowed();
            }
            if (input.facts.channel.kind !== 'group')
                return deny('target_invalid');
            if (!currentTarget(input.target, input.facts) || !input.facts.targetExists)
                return deny('target_invalid');
        ...
        if (input.definition.permission === 'current_channel') {
            if (!currentTarget(input.target, input.facts))
                return deny('target_invalid');
            if (action === null || !input.intent.actions.includes(action))
                return deny('explicit_intent_required');
        }
            ...
            if (input.target.kind !== 'group' && input.target.kind !== 'private')
                return deny('target_invalid');
            if (currentTarget(input.target, input.facts))
                return deny('current_channel_uses_normal_reply');
            if (!input.facts.actor.isBotMaster)
                return deny('permission_denied');
            if (!input.facts.targetExists)
                return deny('target_not_found');
