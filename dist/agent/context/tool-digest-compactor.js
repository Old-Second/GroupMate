...
import { createContextArtifactV1, MAX_CONTEXT_ARTIFACT_CONTENT_BYTES, MAX_CONTEXT_ARTIFACT_REFS } from './context-artifact.js';
import { CONTEXT_COMPACTION_REQUEST_HASH_DOMAIN, MAX_CONTEXT_COMPACTION_TOOL_CALLS } from './context-planner.js';
import { contextSpanHash, domainSeparatedContextHash, parseContextSourceRefs, parseContextSpanV1 } from './context-span.js';
import { canonicalizeContextJsonValue, canonicalJsonStringify, CONTEXT_TOKEN_ESTIMATOR_VERSION, inspectContextArray, inspectContextRecord, requireContextAscii, requireSafeInteger } from './context-token-estimator.js';
...
const INVALID_EVIDENCE = Object.freeze({
    status: 'unavailable',
    code: 'invalid_compaction_evidence'
});
...
const PROTOCOL_LIMIT = Object.freeze({
    status: 'unavailable',
    code: 'tool_protocol_limit_exceeded'
});
...
function expectedSourceRefs(span) {
...
function assertDeepFrozenData(value, depth = 0, state = { nodes: 0, ancestors: new Set() }) {
    state.nodes += 1;
    if (state.nodes > 8_192 || depth > 32)
    ...
    try {
        ...
        if (Array.isArray(value)) {
            ...
            if (keys.some(key => key !== 'length' && (typeof key !== 'string' || !/^(0|[1-9]\d*)$/.test(key) || Number(key) >= value.length)) || Object.keys(value).length !== value.length)
                throw new TypeError('invalid compaction evidence');
            for (const item of value)
                assertDeepFrozenData(item, depth + 1, state);
            return;
        }
        if (prototype !== Object.prototype && prototype !== null)
            throw new TypeError('invalid compaction evidence');
        const descriptors = Object.getOwnPropertyDescriptors(value);
        if (Object.getOwnPropertySymbols(value).length > 0 || Object.entries(descriptors).some(([, descriptor]) => (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value'))))
            throw new TypeError('invalid compaction evidence');
...
function resultMatchesTerminalStatus(terminalStatus, result) {
...
function parseEvidence(value, span) {
    try {
        ...
        const calls = Object.freeze(inspectContextArray(input.calls, MAX_CONSUMED_TOOL_DIGEST_CALLS).map((rawCall, index) => {
        ...
        if (calls.length === 0 || calls.length !== span.toolProtocol?.callIds.length)
            return null;
        return calls;
    }
    catch {
...
function digestContent(sources, request, span) {
    const protocol = span.toolProtocol;
    if (protocol === null)
        return null;
    const sourceSpanHash = contextSpanHash(span);
    const sourceRefsHash = domainSeparatedContextHash(CONSUMED_TOOL_SOURCE_REFS_HASH_DOMAIN, canonicalJsonStringify(canonicalizeContextJsonValue(request.sourceRefs)));
    ...
    const protocolHash = domainSeparatedContextHash(CONSUMED_TOOL_PROTOCOL_HASH_DOMAIN, canonicalJsonStringify(canonicalizeContextJsonValue(Object.freeze({
    ...
    const calls = Object.freeze(sources
        .slice(0, MAX_CONSUMED_TOOL_DIGEST_DISPLAY_CALLS)
        .map((source, index) => {
        const complete = completeCalls[index];
        if (complete === undefined)
            throw new TypeError('complete digest call is missing');
        ...
        return Object.freeze({
            ...
            argumentsBytes,
            argumentsHash: domainSeparatedContextHash(CONSUMED_TOOL_ARGUMENTS_HASH_DOMAIN, source.argumentsText),
            argumentsPreview: truncateUtf8(source.argumentsText, MAX_CONSUMED_TOOL_ARGUMENTS_PREVIEW_BYTES),
            argumentsTruncated: argumentsBytes > MAX_CONSUMED_TOOL_ARGUMENTS_PREVIEW_BYTES,
            resultBytes,
            resultHash: domainSeparatedContextHash(CONSUMED_TOOL_RESULT_HASH_DOMAIN, source.resultCanonical),
    ...
    const content = `${CONSUMED_TOOL_DIGEST_PREFIX}${canonicalJsonStringify(canonicalizeContextJsonValue(payload))}`;
    return Buffer.byteLength(content, 'utf8') <= MAX_CONTEXT_ARTIFACT_CONTENT_BYTES
        ? content
        : null;
}
export function compactConsumedToolSpan(requestValue, spanValue, evidenceValue) {
    let span;
    try {
    ...
    if (span.toolProtocol.callIds.length > MAX_CONSUMED_TOOL_DIGEST_CALLS)
    ...
    if (assistant?.role !== 'assistant' || assistant.toolCalls === undefined ||
        assistant.toolCalls.length !== span.messages.length - 1)
        return INCOMPLETE_PROTOCOL;
    const sources = [];
