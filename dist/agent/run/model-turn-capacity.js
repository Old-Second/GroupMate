    ...
    try {
        if (Array.isArray(value)) {
            ...
            if (Object.getPrototypeOf(value) !== Array.prototype ||
                keys.length !== value.length + 1 || keys.some((key, index) => (index < value.length ? key !== String(index) : key !== 'length')))
                throw new TypeError('tool schema is invalid');
            const items = [];
...
export function estimateOpenAIToolSchemaTokens(tools) {
    try {
        if (!Array.isArray(tools))
            throw new TypeError();
        const wire = canonicalJson(tools);
        if (Buffer.byteLength(wire, 'utf8') > MAX_TOOL_WIRE_BYTES)
            throw new TypeError();
        if (tools.length === 0)
            return 0;
...
export function createToolWireSnapshotV1(tools) {
    let wire;
    try {
...
export function parseToolWireSnapshotV1(value) {
...
export function availableModelOutputTokens(input) {
