...
function exactKeys(value, allowed, required, label) {
    ...
    if (unknown !== undefined)
        throw new TypeError(`unknown ${label} key: ${unknown}`);
    const missing = required.find(key => !Object.hasOwn(value, key));
    if (missing !== undefined)
        throw new TypeError(`${label} key is missing: ${missing}`);
}
function boundedString(value, label, maxLength = 262_144) {
...
function validateSessionAddress(value) {
...
function validateModelConfig(value) {
    ...
    if (typeof model.streaming !== 'boolean')
        throw new TypeError('run model streaming is invalid');
    ...
    if (model.temperature !== undefined)
        finiteNumber(model.temperature, 'run model temperature');
    if (model.topP !== undefined)
        finiteNumber(model.topP, 'run model top P');
}
function validateModelMessages(value, profileId, profileVersion) {
    if (!Array.isArray(value))
        throw new TypeError('run model messages are invalid');
    for (const raw of value) {
        ...
        exactKeys(message, ['role', 'content', 'toolCalls', 'providerState'], ['role', 'content'], 'model message');
...
function validateToolSnapshot(value) {
    ...
    for (const rawEntry of snapshot.manifest) {
        ...
        exactKeys(entry, ['name', 'version', 'schemaHash', 'policyHash', 'schedulingHash'], ['name', 'version', 'schemaHash', 'policyHash', 'schedulingHash'], 'tool snapshot manifest entry');
    ...
    if (snapshot.fingerprint !== expected)
        throw new TypeError('tool snapshot fingerprint is invalid');
}
function validateToolLedgers(value, snapshotId) {
    if (!Array.isArray(value) || value.length > 8)
        throw new TypeError('tool ledgers are invalid');
...
function validatePreparedBatch(value, snapshotId) {
    if (value === null)
        return;
...
function validateBudgets(limitsValue, countersValue) {
    ...
    for (const key of BUDGET_COUNTER_KEYS)
        nonNegative(Number(counters[key]), `run budget counter ${key}`);
...
function splitCheckpoint(checkpoint) {
...
function parseLoadedRunCheckpoint(value) {
    ...
    if (parsed.output !== null)
        parseAgentMessage(parsed.output);
    if (parsed.error !== null)
        validateSerializedError(parsed.error);
...
function parseJsonText(raw, label, maxBytes) {
...
export function recoverExecutingRunCheckpoint(checkpoint, changes, events, updatedAt) {
...
export function appendRunCheckpointEvents(checkpoint, events) {
    if (events.length === 0)
        return checkpoint;
    validateEvents(events, checkpoint.nextEventSequence, checkpoint.runId, checkpoint.sessionId);
    const updatedAt = events.at(-1)?.occurredAt;
    if (updatedAt === undefined)
        throw new TypeError('run event timestamp is missing');
    timestamp(updatedAt, 'run update time');
    return freezeCheckpoint({
        ...checkpoint,
        revision: checkpoint.revision + 1,
