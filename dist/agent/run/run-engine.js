...
function detachModelUsage(value) {
    if (value === undefined)
        return undefined;
...
function modelTurnUsageDataProperty(turn) {
...
function sameModelUsage(left, right) {
...
function synchronizedObservationCounters(current, budget, ledgers, interruption, approvalHistory, completedModelTurns) {
...
function unavailableProviderObservations(current) {
    return Object.freeze({
        ...current,
...
function unavailableEngineObservations(current) {
    return Object.freeze({
        ...current,
...
export class RunEngine {
    ...
    async #recoverObservationReservations(checkpoint) {
    ...
    async #drive(initial, controller) {
        ...
        while (!isTerminalRunStatus(checkpoint.status)) {
            ...
            try {
                ...
                if (error instanceof ModelSuccessPersistenceError)
                    throw error.failure;
    ...
    #reserveModelTurn(checkpoint, kind) {
        const maxOutputTokens = this.#availableOutputTokens(checkpoint, checkpoint.budgetCounters, checkpoint.estimatedInputTokens, kind);
    ...
    async #reserveProviderDispatch(checkpoint) {
        ...
        const observationCounters = Object.freeze({
            ...
            providerRetries: knownObservation(checkpoint.observationCounters.providerRetries, checkpoint.budgetCounters.providerRetries),
            recoveryAttempts: knownObservation(checkpoint.observationCounters.recoveryAttempts, checkpoint.budgetCounters.recoveryAttempts),
            correctionTurns: knownObservation(checkpoint.observationCounters.correctionTurns, checkpoint.budgetCounters.correctionTurns),
            providerInputTokens: 'unavailable',
            providerOutputTokens: 'unavailable',
            providerTotalTokens: 'unavailable'
        });
    ...
    async #evaluateModelTurn(checkpoint, attempted, correction) {
        ...
        try {
            ...
            const observationCounters = Object.freeze({
                ...checkpoint.observationCounters,
                providerInputTokens: mergedProviderUsage(attempted.previousProviderUsage.providerInputTokens, usage?.inputTokens),
                providerOutputTokens: mergedProviderUsage(attempted.previousProviderUsage.providerOutputTokens, usage?.outputTokens),
                providerTotalTokens: mergedProviderUsage(attempted.previousProviderUsage.providerTotalTokens, usage?.totalTokens),
                providerActiveDurationMs: knownObservation(checkpoint.observationCounters.providerActiveDurationMs, counters.usedActiveRuntimeMs)
            });
            ...
            try {
                ...
                if (budgetError !== null) {
                    ...
                    plan = this.#planSuccessfulModelTurn(checkpoint, attempted, detachedTurn, counters, correction, common, attemptedEvent);
                }
            }
            catch (error) {
                journalTurn = null;
        ...
        return await this.#persistSuccessfulModelTurn(checkpoint, plan, attempted.occurredAt);
    }
    #planSuccessfulModelTurn(checkpoint, attempted, turn, counters, correction, initialCommon, attemptedEvent) {
        ...
        try {
            ...
            if (toolCalls.length > 0) {
                if (correction || finishReason !== 'tool_calls') {
                    return Object.freeze({
                        kind: 'fail',
                        error: modelProtocolError(correction ? 'correction_contains_tool_calls' : 'tool_calls_finish_reason_missing'),
                        changes: common,
                        drafts: Object.freeze([attemptedEvent, completedEvent])
                    });
                }
            ...
            try {
                ...
                const maxOutputTokens = this.#availableOutputTokens(checkpoint, correctionCounters, attempted.estimatedInputTokens, 'correction');
    ...
    async #persistSuccessfulModelTurn(checkpoint, plan, occurredAt) {
        try {
            if (plan.kind === 'fail') {
                return await this.#fail(checkpoint, plan.error, plan.drafts, plan.changes, plan.detailOverride, occurredAt);
            }
            return await this.#commit(checkpoint, plan.status, plan.changes, plan.drafts, true, occurredAt);
        }
        catch (error) {
            if (error instanceof ConcurrentTerminalError)
                throw error;
    ...
    async #preflightTools(checkpoint, signal) {
        ...
        let counters;
        try {
            counters = this.#runBudget(checkpoint).reserveToolBatch(checkpoint.budgetCounters, ledger.calls.length);
        }
        catch (error) {
    ...
    #next(checkpoint, status, changes, drafts, activityBoundary = 'degrade', occurredAt = this.#timestamp()) {
        ...
        const observationCounters = synchronizedObservationCounters(boundedChanges.observationCounters ?? checkpoint.observationCounters, budgetCounters, toolLedgers, interruption, approvalHistory, drafts.filter(draft => draft.type === 'model.completed').length);
        return nextRunCheckpoint(checkpoint, status, {
            ...boundedChanges,
            observationCounters
        }, events, occurredAt);
    }
    #boundaryObservationChanges(checkpoint, status, changes, activityBoundary) {
        if (activityBoundary === false)
            return changes;
        if (!isTerminalRunStatus(status) && status !== 'waiting_approval')
            return changes;
        ...
        if (engineActivity.state === 'reserved') {
            ...
            if (exactEngineSpan && startedAt !== undefined) {
                observationCounters = Object.freeze({
                    ...observationCounters,
                    engineActiveDurationMs: incrementObservation(observationCounters.engineActiveDurationMs, boundedMonotonicDurationMs(startedAt, this.#safeMonotonicNow()))
                });
            }
            else {
                observationCounters = unavailableEngineObservations(observationCounters);
    ...
    async #fail(checkpoint, error, prefixEvents = [], changes = {}, detailOverride, occurredAt) {
        if (isTerminalRunStatus(checkpoint.status))
            return checkpoint;
