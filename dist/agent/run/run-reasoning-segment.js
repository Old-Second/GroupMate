...
function ownDataRecord(value) {
    ...
    if (unknown !== undefined)
        throw new TypeError('reasoning segment contains unknown key');
    const missing = SEGMENT_KEYS.find(key => !Object.hasOwn(descriptors, key));
    if (missing !== undefined)
        throw new TypeError(`reasoning segment key is missing: ${missing}`);
    ...
    return Object.freeze(Object.fromEntries(SEGMENT_KEYS.map(key => [key, descriptors[key]?.value])));
}
...
export function parseRunReasoningSegments(value) {
...
export function appendRunReasoningSegment(segments, input) {
    ...
    if (parsed.length >= MAX_RUN_REASONING_SEGMENTS)
        return segments;
    ...
    if (remaining <= 0)
        return segments;
    const points = [...text];
    const retained = Math.min(points.length, MAX_REASONING_SEGMENT_CODE_POINTS, remaining);
    if (retained <= 0)
        return segments;
