...
export class RunAdmission {
    ...
    async recover(checkpoint, signal) {
        let parsed;
        try {
    ...
    #normalizeAddress(address) {
        ...
        if (parsed === null)
            throw new TypeError('run admission address is invalid');
    ...
    async #tryStart(address, sessionKey, recovery = false) {
        ...
        try {
            if (recovery) {
                await recoverAdmissionClaim(this.#client, sessionKey, leaseId, this.#leaseTtlSeconds);
            }
            else {
                const claimed = await acquireAdmissionClaim(this.#client, sessionKey, leaseId, this.#leaseTtlSeconds);
                if (!claimed)
                    return null;
            }
            let released = false;
    ...
    async #drain() {
        ...
        try {
            while (this.#queue.length > 0 &&
                ...
                if (this.#activeSessions.has(waiter.sessionKey))
                    return;
                this.#queue.shift();
                ...
                try {
                    const lease = await this.#tryStart(waiter.address, waiter.sessionKey, waiter.recovery);
                    if (lease === null) {
                        if (waiter.onAbort !== undefined) {
