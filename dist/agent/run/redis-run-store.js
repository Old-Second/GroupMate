...
export function redisRunReferenceKey(runRef) {
...
export class RedisRunStore {
    ...
    async commitTerminal(expected, next, snapshot) {
        ...
        if (!Array.isArray(result) || result.length !== 6 || result[0] !== 'ok') {
            ...
            throw storageUnavailable('commit_terminal', new TypeError('unexpected terminal commit receipt'));
        }
        const numbers = result.slice(1).map(value => Number(value));
        if (numbers.some(value => !Number.isSafeInteger(value) || value < 0)) {
            throw storageUnavailable('commit_terminal', new TypeError('invalid terminal commit receipt'));
        }
