...
function normalizedText(value) {
    if (typeof value !== 'string')
        throw new TypeError('completion text must be a string');
    const text = value.trim().normalize('NFC');
    if (text.length === 0)
        throw new TypeError('completion text must not be empty');
    return text;
}
export function parseCompletionDisposition(value) {
    ...
    const unknownEntryKey = entryKeys.find(key => typeof key !== 'string' || !['kind', 'text', 'source', 'reason'].includes(key));
    if (unknownEntryKey !== undefined) {
        throw new TypeError(`completion disposition contains unknown key: ${String(unknownEntryKey)}`);
    }
    ...
    if (disposition.kind === 'reply_text') {
        ...
        if (unknownKey !== undefined) {
            throw new TypeError(`completion disposition contains unknown key: ${String(unknownKey)}`);
        }
        if (keys.length !== 2 ||
...
export function completionFromTerminalOutput(input) {
    ...
    if (input.output === null)
        throw new TypeError('terminal output is required');
