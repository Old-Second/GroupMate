...
function parseTerminalFacts(value) {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
        throw new TypeError('run terminal facts are invalid');
    }
    exactOwnKeys(value, ['snapshot', 'receipt'], 'run terminal facts');
...
function cancellationObservation(reason) {
...
export function parseRunAdvanceResult(value) {
    ...
    if (result.kind === 'cancelled' && terminal !== null &&
        terminal.snapshot.cancellationReason !== cancellationObservation(result.reason)) {
