...
import { memoryProposalListCursorV1, memoryRevisionHistoryCursorV1 } from './memory-control-repository.js';
import { decodeDeletionStatusV1, decodeMemoryLifecycleAuditV1, decodeMemoryProposalV2, decodeMemoryRevisionV2 } from './memory-lifecycle-codec.js';
import { MEMORY_DELETION_TOMBSTONE_ID_DOMAIN_V1, createDeletionStatusV1, memoryLifecycleDomainHashV1, memoryProposalDeadlineV2, parseMemoryLifecycleInstantV1 } from './memory-lifecycle-domain.js';
...
import { memoryNamespaceRefV1, memoryNamespaceWireV1, parseMemoryNamespaceRefV1, parseMemoryNamespaceV1 } from './memory-namespace.js';
...
function loadNamespaceGeneration(database, namespaceRef) {
  ...
  `).get(namespaceRef);
    ...
    const parsed = canonicalWire(rowValue(row, 'namespace_wire'), rowValue(row, 'namespace_wire_bytes'), wire => parseMemoryNamespaceV1(JSON.parse(exactString(wire))));
    if (storedRef !== namespaceRef || memoryNamespaceRefV1(parsed.value) !== namespaceRef ||
        memoryNamespaceWireV1(parsed.value) !== parsed.wire)
        throw new CanonicalControlDataErrorV1();
    return generation;
}
function validateProposalRow(row) {
    const proposal = canonicalWire(rowValue(row, 'proposal_wire'), rowValue(row, 'proposal_wire_bytes'), decodeMemoryProposalV2).value;
...
function proposalProjection(proposal, snapshotAt) {
...
function headProofFromRow(row, snapshotAt) {
...
function validateRevisionRow(row) {
    const revision = canonicalWire(rowValue(row, 'revision_wire'), rowValue(row, 'revision_wire_bytes'), decodeMemoryRevisionV2).value;
...
function validateHeadRevision(row, revision) {
    const cursorRef = exactString(rowValue(row, 'cursor_ref'));
    const expectedCursorRef = createHash('sha256')
        .update(MEMORY_CURSOR_HASH_DOMAIN_V1, 'utf8')
        .update('\0', 'utf8')
...
function recordProjectionFromRow(row, snapshotAt) {
    ...
    if (projected.state !== head.lifecycleState)
        throw new CanonicalControlDataErrorV1();
    return Object.freeze({
        schemaVersion: 1,
...
function safeRecordProjectionFromRow(row, snapshotAt) {
...
function pageWireBytes(records, corruptRefs, metadata = {}) {
    return Buffer.byteLength(JSON.stringify({ records, corruptRefs, ...metadata }), 'utf8');
}
function pageFits(request, records, corruptRefs, metadata = {}) {
...
function proposalList(database, request, snapshotAt) {
    ...
    `).get(request.namespaceRef, request.generation, request.cursorAnchor.proposalId);
        if (anchorRow === undefined ||
            exactInteger(rowValue(anchorRow, 'proposed_at_ms')) !== Date.parse(request.cursorAnchor.proposedAt) ||
            !request.states.includes(exactString(rowValue(anchorRow, 'state')))) {
            return Object.freeze({ status: 'invalid_cursor', operation: request.operation });
        }
    }
    ...
    LIMIT ?
  `).all(request.namespaceRef, request.generation, ...request.states, request.cursorAnchor?.proposalId ?? null, request.cursorAnchor === null ? 0 : Date.parse(request.cursorAnchor.proposedAt), request.cursorAnchor === null ? 0 : Date.parse(request.cursorAnchor.proposedAt), request.cursorAnchor?.proposalId ?? '', request.limit + 1);
    const records = [];
    const corruptRefs = [];
    let lastConsumed;
    let hasMore = false;
    ...
    for (const [index, row] of selected.entries()) {
        ...
        try {
            ...
            if (!/^proposal:[0-9a-f]{64}$/.test(ref))
                throw new CanonicalControlDataErrorV1();
            corruptRef = ref;
        }
...
`;
function recordList(database, request, snapshotAt) {
    ...
    if (request.operation === 'record.listSafe')
        parameters.push(Date.parse(snapshotAt));
...
function loadHeadRow(database, namespaceRef, generation, memoryId) {
    return database.prepare(`${HEAD_SELECT}
    WHERE h.namespace_ref = ? AND h.namespace_generation = ? AND h.memory_id = ?
  `).get(namespaceRef, generation, memoryId) ?? null;
}
function revisionList(database, request, snapshotAt) {
    ...
    if (head.lifecycleState === 'purge_due') {
        return Object.freeze({ status: 'record_purge_due', operation: request.operation,
            snapshotAt, head });
    }
    ...
    `).get(request.namespaceRef, request.generation, request.memoryId, request.cursorAnchor.revision);
        if (anchorRow === undefined) {
            return Object.freeze({ status: 'invalid_cursor', operation: request.operation });
        }
        let anchorRevision;
    ...
    LIMIT ?
  `).all(request.namespaceRef, request.generation, request.memoryId, start, request.limit + 1);
    const records = [];
    let lastConsumed;
    let hasMore = false;
    const selected = rows.slice(0, request.limit);
    ...
    const nextCursorAnchor = hasMore && lastConsumed !== undefined
        ? Object.freeze({ schemaVersion: 1, revision: lastConsumed.revision,
    ...
    if (!pageFits(request, records, [], metadata))
        return pageBudgetFailure();
    return Object.freeze({
        status: 'page',
        operation: request.operation,
...
function metadataList(database, request, snapshotAt) {
    ...
    `).get(request.namespaceRef, request.targetGeneration, anchorId);
        if (row === undefined) {
            return Object.freeze({ status: 'invalid_cursor', operation: request.operation });
        }
    ...
    LIMIT ?
  `).all(request.namespaceRef, request.targetGeneration, anchorId, anchorTime, anchorTime, anchorId ?? '', request.limit + 1);
    const records = [];
    const wireRecords = [];
    const corruptRefs = [];
    let lastConsumed;
    ...
    for (const [index, row] of selected.entries()) {
        ...
        try {
            if (isTombstone) {
                const tombstone = canonicalWire(rowValue(row, 'canonical_wire'), rowValue(row, 'canonical_wire_bytes'), value => decodeMemoryTombstoneV1(exactString(value))).value;
                ...
                const audit = canonicalWire(rowValue(row, 'canonical_wire'), rowValue(row, 'canonical_wire_bytes'), decodeMemoryLifecycleAuditV1).value;
...
function globalUsage(database, snapshotAt) {
    const row = database.prepare(`
  ...
  `).get();
    if (row === undefined)
        throw new CanonicalControlDataErrorV1();
    const field = (name) => exactInteger(rowValue(row, name));
...
function loadDeletionTombstone(database, namespaceRef, deletionRef) {
    const row = database.prepare(`
  ...
  `).get(namespaceRef, deletionTombstoneId(deletionRef));
    if (row === undefined)
        return null;
    const tombstone = canonicalWire(rowValue(row, 'tombstone_wire'), rowValue(row, 'tombstone_wire_bytes'), value => decodeMemoryTombstoneV1(exactString(value))).value;
...
function deletionCarrierKinds(database, namespaceRef, generation, memoryId) {
...
function deletionDerivedCleanup(database, namespaceRef, currentGeneration, deletingGeneration, memoryId) {
  ...
  `).get(namespaceRef, generation, memoryId === null ? 'namespace' : 'record', memoryId ?? namespaceRef, memoryId === null ? 'namespace_deleted' : 'record_forgotten') === undefined ? 'applied' : 'queued';
}
function deletionStatus(database, namespaceRef, currentGeneration, deletionRef, snapshotAt) {
  ...
  `).all(namespaceRef, deletionRef);
    ...
    const canonicalReceipt = receiptRows.length === 0
        ? null
        : resolveDeletionReceipt(database, namespaceRef, exactString(rowValue(receiptRows[0], 'command_ref')), deletionRef);
    if (receiptRows.length === 1 && canonicalReceipt === null) {
        throw new CanonicalControlDataErrorV1();
    }
    if (canonicalReceipt !== null && (canonicalReceipt.tombstoneId !== tombstone.tombstoneId ||
  ...
  `).get(namespaceRef, deletionRef);
    ...
    if (checkpointRow !== undefined) {
        stored = canonicalWire(rowValue(checkpointRow, 'checkpoint_wire'), rowValue(checkpointRow, 'checkpoint_wire_bytes'), decodeDeletionStatusV1).value;
    ...
    const remaining = deletionCarrierKinds(database, namespaceRef, deletingGeneration, tombstone.memoryId);
    if (stored?.stage === 'canonical_complete' && remaining.length > 0) {
        throw new CanonicalControlDataErrorV1();
    }
    const derivedCleanup = deletionDerivedCleanup(database, namespaceRef, currentGeneration, deletingGeneration, tombstone.memoryId);
    const canonicalBodies = remaining.length === 0 ? 'verified_absent'
        : stored === null ? 'unverified' : 'scrub_pending';
...
function resolveDeletionReceipt(database, namespaceRef, commandRef, deletionRef) {
    const row = database.prepare(`
  ...
  `).get(namespaceRef, commandRef);
    ...
    if (receipt.deletionRef !== deletionRef)
        return null;
    ...
    if (receipt.namespaceRef !== namespaceRef ||
        ...
        exactString(rowValue(row, 'aggregate_ref_hash')) !== memoryLifecycleDomainHashV1('groupmate.memory.lifecycle-aggregate-ref.v1', aggregateRef) ||
        receipt.committedAt !== new Date(committedAt).toISOString()) {
        throw new CanonicalControlDataErrorV1();
    }
    return receipt;
}
function executeLocked(database, envelope, snapshotAt) {
    ...
    if (request.operation === 'usage.getGlobal')
        return globalUsage(database, snapshotAt);
    ...
    if (request.operation === 'revision.list')
        return revisionList(database, request, snapshotAt);
    if (request.operation === 'proposal.inspect') {
      ...
      FROM proposals
      WHERE namespace_ref = ? AND namespace_generation = ? AND proposal_id = ?
    `).get(request.namespaceRef, request.generation, request.proposalId);
        if (row === undefined)
            return Object.freeze({ status: 'not_found', operation: request.operation,
                snapshotAt });
        const proposal = validateProposalRow(row);
        const lifecycle = projectMemoryProposalLifecycleV2(proposal, snapshotAt);
        if (!lifecycle.fullWireReadable)
            throw new CanonicalControlDataErrorV1();
    ...
    if (request.operation === 'revision.get') {
      ...
      WHERE r.namespace_ref = ? AND r.namespace_generation = ?
        AND r.memory_id = ? AND r.revision = ?
    `).get(request.namespaceRef, request.generation, request.memoryId, request.revision);
        if (row === undefined)
            return Object.freeze({ status: 'not_found', operation: request.operation,
                snapshotAt });
        return Object.freeze({ status: 'found', operation: request.operation, snapshotAt,
    ...
    if (request.operation === 'deletion.getStatus') {
        const status = deletionStatus(database, request.namespaceRef, request.generation, request.deletionRef, snapshotAt);
        if (status === null)
            return Object.freeze({
                status: 'not_found', operation: request.operation, snapshotAt
            });
        return Object.freeze({
            status: 'found',
            operation: request.operation,
    ...
    if (request.operation === 'deletion.resolve') {
        const receipt = resolveDeletionReceipt(database, request.namespaceRef, request.commandRef, request.deletionRef);
        if (receipt === null)
            return Object.freeze({
                status: 'not_found', operation: request.operation, snapshotAt
            });
...
export function createSqliteMemoryControlRepositoryV1(options) {
    if (options === null || typeof options !== 'object' || utilTypes.isProxy(options) ||
        ...
        throw new TypeError('invalid memory control repository options');
