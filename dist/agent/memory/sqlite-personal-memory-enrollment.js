...
import { createPersonalMemoryEnrollmentPolicyV1, decodePersonalMemoryEnrollmentCommandWireV1, decodePersonalMemoryEnrollmentPolicyV1, encodePersonalMemoryEnrollmentPolicyV1, parsePersonalMemoryEnrollmentDecisionEnvelopeV1, personalMemoryEnrollmentActorRefHashV1, personalMemoryEnrollmentCommandHashV1, personalMemoryEnrollmentCommandRefHashV1, personalMemoryEnrollmentSourceRefHashV1, personalMemoryEnrollmentSourceSceneRefV1 } from './personal-memory-enrollment.js';
...
function parseReadRequest(value) {
    ...
    return Object.freeze({ schemaVersion: 1, namespace, namespaceRef });
}
...
function requireRequestedNamespace(stored, requested) {
    if (memoryNamespaceWireV1(stored.namespace) !== memoryNamespaceWireV1(requested)) {
        throw new CanonicalPersonalMemoryEnrollmentDataErrorV1();
    }
}
function loadPolicy(database, namespaceRef) {
    const row = database.prepare(`
...
function requireConservativeGlobalUsage(database, stored) {
    const row = database.prepare(`
    SELECT
...
function insertEmptyNamespace(database, namespace, generation, nowMs) {
...
function commandExpectedPolicy(envelope, policyGeneration, updatedAt) {
...
function storePolicy(database, policy, previousGeneration) {
    ...
    `).run(policy.namespaceRef, policy.namespaceGeneration, policy.state, policy.candidateMode, policy.policyGeneration, policy.decidedByActorRefHash, policy.decisionSourceRefHash, policy.policyHash, wire, bytes, updatedAtMs).changes);
        return;
    }
    requireOneChange(database.prepare(`
  ...
  `).run(policy.state, policy.candidateMode, policy.policyGeneration, policy.decidedByActorRefHash, policy.decisionSourceRefHash, policy.policyHash, wire, bytes, updatedAtMs, policy.namespaceRef, policy.namespaceGeneration, previousGeneration).changes);
}
function revalidateAuthority(envelope, freshNow) {
    const wire = decodePersonalMemoryEnrollmentCommandWireV1(envelope.command.wire);
...
function readEnrollment(database, request) {
...
function decideEnrollment(database, envelope, freshNow, aborted) {
    const wire = decodePersonalMemoryEnrollmentCommandWireV1(envelope.command.wire);
    ...
    if (namespace === null) {
        ...
        const capacity = insertEmptyNamespace(database, envelope.namespace, wire.expectedNamespaceGeneration, Date.parse(freshNow));
        if (capacity !== null)
            return capacity;
    ...
    const expected = commandExpectedPolicy(envelope, wire.expectedPolicyGeneration + 1, current?.updatedAt ?? freshNow);
    if (current !== null && current.commandHash === expected.commandHash) {
        if (current.policyHash !== expected.policyHash) {
            throw new CanonicalPersonalMemoryEnrollmentDataErrorV1();
        }
        return Object.freeze({ status: 'unchanged', policy: current });
    }
    ...
    const policy = commandExpectedPolicy(envelope, wire.expectedPolicyGeneration + 1, freshNow);
    if (aborted())
        return Object.freeze({ status: 'aborted' });
    storePolicy(database, policy, current?.policyGeneration ?? null);
    if (aborted())
        return Object.freeze({ status: 'aborted' });
    return Object.freeze({ status: 'stored', policy });
}
export function createSqlitePersonalMemoryEnrollmentAdapterV1(optionsValue) {
    ...
    const decide = async (envelopeValue, signal) => {
        ...
        try {
            ...
            const result = decideEnrollment(options.database, envelope, freshNow, signalScope.isAborted);
