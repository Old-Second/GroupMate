...
import { closeSync, fsyncSync, linkSync, lstatSync, openSync, unlinkSync } from 'node:fs';
...
const LEXICAL_SCHEMA_FINGERPRINT_DOMAIN_V1 = 'groupmate.memory.sqlite-lexical-schema.v1';
...
export const MEMORY_LEXICAL_SQLITE_SCHEMA_FINGERPRINT_V1 = fingerprintRows(SCHEMA_OBJECTS_V1);
...
function parseOptions(value) {
    ...
    return Object.freeze({ location: input.location, now: input.now });
}
...
export function assertSqliteMemoryFts5AvailableV1(execute) {
...
DROP TABLE temp.groupmate_memory_fts5_probe;`
        ...
        if (result !== undefined)
            throw new TypeError('SQLite execute must complete synchronously');
    }
    catch {
        try {
...
function ownedSchemaRows(database) {
...
export function sqliteMemoryLexicalSchemaFingerprintV1(database) {
    return fingerprintRows(ownedSchemaRows(database));
}
function validateFtsShadowTables(database) {
    const rows = database.prepare(`
  ...
  `).all();
    if (rows.length !== FTS_SHADOW_TABLES.size)
        return schemaUnsupported();
    for (const row of rows) {
...
function validateDatabase(database) {
  ...
  `).get();
    if (state === undefined ||
        ...
        (state.status === 'empty' && (state.projection_generation !== 0 || state.source_last_sequence !== 0 ||
            state.indexed_records !== 0 || state.indexed_logical_bytes !== 0)) || (state.status !== 'empty' && state.projection_generation === 0)) {
        return schemaUnsupported();
    }
}
...
function preflightExisting(location) {
    ...
    try {
        ...
        if (error instanceof SqliteMemoryLexicalDatabaseErrorV1)
            throw error;
        return schemaUnsupported();
    }
    finally {
...
function bootstrapFile(location, now) {
...
function openWritable(location, identity) {
...
export function openSqliteMemoryLexicalDatabaseV1(optionsValue) {
    const options = parseOptions(optionsValue);
    databaseConstructor();
    if (options.location === ':memory:')
        return openInMemory(options.now);
    try {
        if (!locationExists(options.location))
            bootstrapFile(options.location, options.now);
