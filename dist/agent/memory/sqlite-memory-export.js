...
import { constants as fsConstants, existsSync, lstatSync, mkdirSync, openSync, readdirSync, unlinkSync } from 'node:fs';
...
import { decodeMemoryProposalV2, decodeMemoryRevisionV2, decodeMemoryLifecycleAuditV1, encodeMemoryLifecycleAuditV1 } from './memory-lifecycle-codec.js';
import { createMemoryLifecycleAuditV1, memoryLifecycleDomainHashV1, parseMemoryLifecycleInstantV1 } from './memory-lifecycle-domain.js';
import { decodeMemoryTombstoneV1 } from './memory-codec.js';
import { acquireMemoryExportGenerateAttemptV1, createMemoryExportStableResultV1, decodeMemoryExportCommandWireV1, decodeMemoryExportStableResultWireV1, deriveMemoryExportIdV1, encodeMemoryExportStableResultWireV1, memoryExportClaimReceiptHashV1, memoryExportCommandHashV1, memoryExportCommandRefHashV1, memoryExportDeliveryRefHashV1, memoryExportStableResultHashV1, MEMORY_EXPORT_EXCLUSIONS_HASH_V1, MEMORY_EXPORT_MAX_CHUNK_BYTES_V1, MEMORY_EXPORT_MAX_WIRE_BYTES_V1 } from './memory-export-port.js';
...
const ARTIFACT_TOKEN_PATTERN = /^memory-export-v1-([0-9a-f]{64})-([1-9][0-9]*)-([0-9a-f]{64})\.jsonl$/;
...
function addMilliseconds(instant, durationMs) {
...
function parseOptions(value) {
    ...
    return Object.freeze({
        ...
        consumeArtifact: input.consumeArtifact
    });
}
...
function artifactToken(exportId, fencingToken, artifactSha256 = '0'.repeat(64)) {
...
function artifactPaths(directory, token) {
    if (!ARTIFACT_TOKEN_PATTERN.test(token))
        throw new CanonicalExportDataErrorV1();
    return Object.freeze({
...
function artifactDirectoryBytes(directory) {
    const names = readdirSync(directory);
    if (names.length > MAX_ARTIFACT_DIRECTORY_ENTRIES)
        throw new ExportArtifactCapacityErrorV1();
    let total = 0;
    for (const name of names) {
        const base = name.replace(/\.(partial|redeeming|lock)$/, '');
        if (!ARTIFACT_TOKEN_PATTERN.test(base))
            throw new ExportArtifactErrorV1();
...
class SecureArtifactSinkV1 {
    ...
    #wireBytes = 0;
    #committed = false;
    constructor(directory, partialPath, finalPath, capacityRemaining, file) {
    ...
    static async create(directory, token, capacityRemaining) {
        const paths = artifactPaths(directory, token);
        const file = await open(paths.partial, fsConstants.O_CREAT | fsConstants.O_EXCL | fsConstants.O_WRONLY |
            (fsConstants.O_NOFOLLOW ?? 0), 0o600);
        await file.chmod(0o600);
        return new SecureArtifactSinkV1(directory, paths.partial, paths.final, capacityRemaining, file);
    }
    get wireBytes() {
    ...
    async write(chunk, signal) {
        if (this.#file === null || this.#committed || signal?.aborted === true ||
            ...
            if (signal?.aborted === true)
                throw new ExportAbortedErrorV1();
            throw new ExportArtifactErrorV1();
        }
        ...
        while (offset < chunk.byteLength) {
            const written = await this.#file.write(chunk, offset, chunk.byteLength - offset, this.#wireBytes + offset);
    ...
    async commit() {
        ...
        const directory = await open(this.#directory, fsConstants.O_RDONLY | fsConstants.O_DIRECTORY);
        try {
...
class FileArtifactSourceV1 {
    #file;
    #streamed = false;
    constructor(file) {
        this.#file = file;
    }
    async streamInto(sink, signal) {
        const file = this.#file;
        if (file === null || this.#streamed)
            throw new ExportArtifactErrorV1();
        this.#streamed = true;
...
function cleanArtifactPaths(directory, token) {
    const paths = artifactPaths(directory, token);
    let deleted = 0;
...
function cleanArtifactFence(directory, exportId, fencingToken) {
    const prefix = `memory-export-v1-${exportId.slice('export:'.length)}-${fencingToken}-`;
    const tokens = new Set();
    for (const name of readdirSync(directory)) {
        const base = name.replace(/\.(partial|redeeming|lock)$/, '');
        if (base.startsWith(prefix) && ARTIFACT_TOKEN_PATTERN.test(base))
            tokens.add(base);
    }
    let deleted = 0;
    for (const token of tokens)
        deleted += cleanArtifactPaths(directory, token);
    return deleted;
}
async function syncArtifactDirectory(directoryPath) {
...
function parseJobRow(row) {
    ...
    const fields = Object.freeze({
        exportId: (() => {
            ...
            if (!EXPORT_ID_PATTERN.test(value))
                throw new CanonicalExportDataErrorV1();
            return value;
        })(),
    ...
    if (fields.expiresAtMs !== fields.preparedAtMs +
        ...
        (fields.manifestWire !== null && (byteLength(fields.manifestWire) !== positiveInteger(rowValue(row, 'manifest_wire_bytes')) ||
            encodeMemoryExportStableResultWireV1(decodeMemoryExportStableResultWireV1(fields.manifestWire)) !== fields.manifestWire)) || jobWireBytes(fields) !== positiveInteger(rowValue(row, 'job_wire_bytes'))) {
        throw new CanonicalExportDataErrorV1();
    }
    ...
    if (fields.state === 'deliverable' || fields.state === 'delivered' || fields.state === 'failed') {
        ...
        if (manifest.exportId !== fields.exportId || manifest.namespaceRef !== fields.namespaceRef ||
            ...
            manifest.commandHash !== fields.terminalCommandHash)
            throw new CanonicalExportDataErrorV1();
    ...
    if ((fields.state === 'delivered') !== claimed || (claimed && (fields.claimCommandHash === null || fields.deliveryRefHash === null ||
        fields.claimReceiptHash === null || fields.deliveredAtMs === null ||
...
`;
function loadJob(database, exportId) {
    const row = database.prepare(JOB_SELECT).get(exportId);
    return row === undefined ? null : parseJobRow(row);
}
function preparedManifest(envelope, contentEpoch, preparedAt, artifactExpiresAt) {
    const wire = decodeMemoryExportCommandWireV1(envelope.command.wire);
    if (wire.operation !== 'export.prepare')
        throw new CanonicalExportDataErrorV1();
...
function reservationWire(manifest, commandHash) {
...
function decodeReservationWire(raw) {
    let value;
    try {
    ...
    const manifest = decodeMemoryExportStableResultWireV1(encodeMemoryExportStableResultWireV1(input.preparedManifest));
...
function loadReservation(database, exportId) {
    const row = database.prepare(`
...
function exportRefHash(exportId) {
    if (!EXPORT_ID_PATTERN.test(exportId))
        throw new CanonicalExportDataErrorV1();
    return memoryLifecycleDomainHashV1(EXPORT_REF_HASH_DOMAIN_V1, exportId);
}
function actorRefHash(actorRef) {
    if (!/^actor:[0-9a-f]{64}$/.test(actorRef))
        throw new CanonicalExportDataErrorV1();
    return memoryLifecycleDomainHashV1(EXPORT_ACTOR_REF_HASH_DOMAIN_V1, actorRef);
}
function createExportAudit(operation, envelope, exportId, occurredAt, outcome, receiptHash) {
...
function insertAudit(database, audit) {
  ...
  `).run(audit.namespaceRef, audit.namespaceGeneration, audit.auditId, audit.operation, audit.commandRefHash, audit.aggregateRefHash, audit.authorizedByActorRefHash, audit.executedByActorRefHash, Date.parse(audit.sourceCommittedAt), Date.parse(audit.recordedAt), Date.parse(audit.expiresAt), wire, byteLength(wire)).changes);
}
function authorityDenial(envelope, freshNow, operation) {
    const command = decodeMemoryExportCommandWireV1(envelope.command.wire);
...
function databaseLocation(database) {
...
function parseCanonicalJson(wire) {
    try {
...
class SnapshotJsonlWriterV1 {
    ...
    async line(type, value, contentKind, comparisonContent, signal) {
        if (signal?.aborted === true)
            throw new ExportAbortedErrorV1();
        ...
        if (contentKind !== null) {
            ...
            if (!Number.isSafeInteger(this.contentBytes))
                throw new ExportArtifactCapacityErrorV1();
        }
        if (comparisonContent) {
            if (this.#snapshotDigest !== null)
                throw new CanonicalExportDataErrorV1();
    ...
    snapshotDigest() {
        if (this.#snapshotDigest === null)
            this.#snapshotDigest = this.#snapshotHash.digest('hex');
        return this.#snapshotDigest;
    }
}
function assertMillis(instant, milliseconds) {
    if (Date.parse(instant) !== milliseconds)
        throw new CanonicalExportDataErrorV1();
}
async function writeProposals(database, writer, namespaceRef, generation, signal) {
  ...
  `);
    const readProposal = (row) => {
        ...
        if ((proposal.state === 'approved') !== (resultingMemoryId !== null) ||
            (proposal.state === 'approved' && proposal.decision !== null && (proposal.decision.resultingMemoryId !== resultingMemoryId ||
    ...
    for (const row of statement.iterate(namespaceRef, generation)) {
...
async function writeHeads(database, writer, namespaceRef, generation, signal) {
    const rows = database.prepare(`
  ...
  `).iterate(namespaceRef, generation);
    for (const row of rows) {
        ...
        if (revision.record.namespaceRef !== namespaceRef ||
            ...
            revision.record.contentHash !== contentHash)
            throw new CanonicalExportDataErrorV1();
        const value = {
            memoryId,
            currentRevision,
...
async function writeRevisions(database, writer, namespaceRef, generation, signal) {
    const rows = database.prepare(`
...
async function writeTombstones(database, writer, namespaceRef, generation, signal) {
    const rows = database.prepare(`
...
async function writeAudits(database, writer, namespaceRef, generation, signal) {
    const rows = database.prepare(`
  ...
  `).iterate(namespaceRef, generation);
    for (const row of rows) {
        ...
        await writer.line('lifecycle_audit', parseCanonicalJson(wire), 'lifecycleAudit', !audit.operation.startsWith('export_'), signal);
    }
}
async function generateSnapshot(sourceDatabase, directory, capacityBytes, state, signal) {
...
async function verifyArtifactFile(file, token, manifest) {
    ...
    const typeToCount = Object.freeze({
        proposal: 'proposal',
    ...
    const processLine = (lineBuffer) => {
        if (lineBuffer.byteLength === 0 || lineBuffer.byteLength + 1 >
            MEMORY_EXPORT_MAX_CHUNK_BYTES_V1 || sawFooter)
            throw new ExportArtifactErrorV1();
        ...
        if (JSON.stringify(parsed) !== line || parsed === null || typeof parsed !== 'object' ||
            Array.isArray(parsed) || utilTypes.isProxy(parsed))
            throw new ExportArtifactErrorV1();
        const record = parsed;
        ...
        if (lineIndex === 0) {
            ...
            const comparisonContent = type !== 'lifecycle_audit' ||
                ...
                typeof record.value.operation !== 'string' ||
                !record.value.operation.startsWith('export_');
            if (comparisonContent) {
    ...
    for await (const value of stream) {
        ...
        if (bytes > MEMORY_EXPORT_MAX_WIRE_BYTES_V1)
            throw new ExportArtifactErrorV1();
...
async function verifyArtifact(path, token, manifest) {
...
function failedManifest(state, category, failedAt = state.snapshotAt) {
    return createMemoryExportStableResultV1({
        schemaVersion: 1,
        status: 'failed',
        commandRefHash: state.commandRefHash,
...
function terminalAuditOutcome(terminal) {
    ...
    return Object.freeze({
        ...
        outcome: `failed_${terminal.category}`
    });
}
...
function mapAdapterError(error) {
    ...
    if (error instanceof ExportArtifactCapacityErrorV1) {
        return Object.freeze({ status: 'capacity', category: 'artifact_bytes' });
    }
    ...
    if (isBusy(error)) {
        return Object.freeze({ status: 'unavailable', category: 'busy', retryable: true });
    }
    return Object.freeze({ status: 'unavailable', category: 'io', retryable: true });
}
function insertPreparedState(database, envelope, freshNow) {
    const command = decodeMemoryExportCommandWireV1(envelope.command.wire);
    ...
    if (existing !== null) {
        ...
        const replay = preparedManifest(envelope, existing.contentEpoch, new Date(existing.preparedAtMs).toISOString(), new Date(existing.expiresAtMs).toISOString());
    ...
    const expiresAt = addMilliseconds(freshNow, MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleExportTerminalTtlMs);
  ...
  `).run(fields.exportId, fields.namespaceRef, fields.namespaceGeneration, fields.contentEpoch, fields.preparedCommandHash, fields.fencingToken, fields.preparedAtMs, fields.expiresAtMs, jobWireBytes(fields)).changes);
    requireOneChange(database.prepare(`
  ...
  `).run(exportId, command.namespaceRef, command.expectedNamespaceGeneration, commandHash, MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleAuditWireBytes, reservation, byteLength(reservation), Date.parse(expiresAt)).changes);
    insertAudit(database, createExportAudit('export_prepared', envelope, exportId, freshNow, 'prepared', manifest.receiptHash));
...
function acquireGenerateLease(options, envelope, attempt, attemptStates) {
    const database = options.database;
    const command = decodeMemoryExportCommandWireV1(envelope.command.wire);
    if (command.operation !== 'export.generate' || command.exportId === null ||
        command.expectedManifestHash === null)
        throw new CanonicalExportDataErrorV1();
    const commandHash = memoryExportCommandHashV1(envelope.command);
    ...
    `).run(fencingToken, options.leaseOwnerId, leaseToken, leasedUntilMs, counter).changes);
    ...
    `).run(options.leaseOwnerId, leaseToken, fencingToken, leasedUntilMs, jobWireBytes(writing), job.exportId, job.preparedCommandHash).changes);
...
async function claimDelivery(options, envelope) {
    const command = decodeMemoryExportCommandWireV1(envelope.command.wire);
    ...
    if (initialManifest.status !== 'deliverable')
        throw new CanonicalExportDataErrorV1();
    ...
    try {
        ...
        const deliveryRef = `delivery:${memoryLifecycleDomainHashV1(EXPORT_DELIVERY_REF_DOMAIN_V1, `${job.exportId}\0${commandHash}`)}`;
    ...
    `).run(commandHash, deliveryRefHash, claimReceiptHash, delivered.deliveredAtMs, jobWireBytes(delivered), job.exportId, job.terminalCommandHash).changes);
...
function terminalForOutcome(state, outcome, freshNow) {
    ...
    const category = outcome === 'aborted'
        ? 'aborted'
        : outcome === 'invalid'
            ? 'corrupt'
            : 'unavailable';
...
async function redeemDelivery(options, delivery) {
    if (!EXPORT_ID_PATTERN.test(delivery.exportId) ||
        ...
        !HASH_PATTERN.test(delivery.claimReceiptHash))
        throw new CanonicalExportDataErrorV1();
    ...
    for (let attempt = 0; attempt < 2 && lock === null; attempt += 1) {
        try {
            lock = await open(paths.lock, fsConstants.O_CREAT | fsConstants.O_EXCL | fsConstants.O_WRONLY |
                (fsConstants.O_NOFOLLOW ?? 0), 0o600);
            ...
            if (existsSync(paths.redeeming)) {
                ...
                return Object.freeze({ status: 'already_consumed' });
            }
            try {
                await unlink(paths.lock);
            }
    ...
    try {
        ...
        const file = await open(paths.redeeming, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0));
...
export function createSqliteMemoryExportAdapterV1(optionsValue) {
    const options = parseOptions(optionsValue);
    ensureArtifactDirectory(options.artifactDirectory);
    const attemptStates = new WeakMap();
    const execute = async (envelope, signal, generateAttempt) => {
        if (signal?.aborted === true)
        ...
        try {
            ...
            if (acquired.result !== null) {
                if (acquired.result !== null && typeof acquired.result === 'object' &&
                    ...
                    ['deliverable', 'snapshot_changed', 'failed'].includes(String(acquired.result.status))) {
                    const replay = acquired.result;
                    if (replay.status === 'deliverable') {
                        const job = loadJob(options.database, replay.exportId);
                        if (job === null || job.artifactToken === null)
                            throw new CanonicalExportDataErrorV1();
            ...
            try {
                state.terminal = await generateSnapshot(options.database, options.artifactDirectory, options.artifactCapacityBytes, state, signal);
            }
            catch (error) {
                state.terminal = failedManifest(state, error instanceof ExportAbortedErrorV1
                    ? 'aborted'
    ...
    const cleanupPartial = async (attempt) => {
        ...
        if (job !== null && job.terminalCommandHash === state.commandHash &&
            job.state === 'deliverable' && job.artifactToken === state.artifactToken)
            return;
        cleanArtifactFence(options.artifactDirectory, state.exportId, state.fencingToken);
    };
    const finalizeGenerate = async (attempt, outcome) => {
        const state = attemptStates.get(attempt);
        if (state === undefined)
            throw new CanonicalExportDataErrorV1();
        beginImmediate(options.database);
      ...
      `).run(state.exportId, reservation.manifest.commandHash).changes);
            ...
            insertAudit(options.database, createExportAudit(auditState.operation, state.envelope, state.exportId, freshNow, auditState.outcome, terminal.receiptHash));
      ...
      `).run(terminalJob.state, state.commandHash, manifestWire, byteLength(manifestWire), terminalJob.artifactToken, terminalJob.terminalAtMs, jobWireBytes(terminalJob), state.exportId, state.leaseOwnerId, state.leaseToken, state.fencingToken).changes);
            requireOneChange(options.database.prepare(`
      ...
      `).run(state.fencingToken, state.leaseOwnerId, state.leaseToken).changes);
            ...
            options.database.exec('COMMIT');
            attemptStates.delete(attempt);
            return Object.freeze({ status: 'terminal', manifest: terminal });
        }
        catch (error) {
            rollbackQuietly(options.database);
            throw error;
    ...
    const persistentDelivery = Object.freeze({
        redeemOnce: async (delivery) => redeemDelivery(options, delivery)
    });
    const cleanupExpired = async (limitValue = MEMORY_EXPORT_CLEANUP_BATCH_V1) => {
      ...
      `).all(nowMs, limitValue + 1);
            ...
            for (const namespaceRef of namespaces)
                assertUsageExact(options.database, namespaceRef);
            let deletedArtifacts = 0;
            for (const job of jobs) {
                if (job.artifactToken !== null) {
                    deletedArtifacts += cleanArtifactPaths(options.artifactDirectory, job.artifactToken);
                }
                deletedArtifacts += cleanArtifactFence(options.artifactDirectory, job.exportId, job.fencingToken);
