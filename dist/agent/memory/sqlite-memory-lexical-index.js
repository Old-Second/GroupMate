...
import { parseCanonicalMemoryRecordV1 } from './memory-canonical-wire.js';
import { createMemoryLexicalIndexPortV1, memoryLexicalBodyHashV1 } from './memory-lexical-index.js';
...
function parseOptions(value) {
    ...
    if (input.database === null || typeof input.database !== 'object' ||
        ...
        !methodIsDataFunction(input.database, 'exec'))
        return invalidMemoryValue();
    if (input.transliterator !== undefined && (input.transliterator === null || typeof input.transliterator !== 'object' ||
...
function parseProjection(value) {
    ...
    const documents = inspectMemoryArray(input.documents, MEMORY_LEXICAL_PROJECTION_BATCH_RECORDS_V1).map(value => {
        const entry = inspectMemoryRecord(value, ['record', 'revisionHash']);
        const record = parseCanonicalMemoryRecordV1(entry.record);
        if (!MEMORY_ID.test(record.memoryId))
            return invalidMemoryValue();
        return Object.freeze({
            record,
    ...
    const identities = documents.map(({ record }) => (`${record.namespaceRef}\0${record.namespaceGeneration}\0${record.memoryId}`));
...
function parseProjectionControl(value) {
    const input = inspectMemoryRecord(value, [
...
function searchResult(options, request) {
  ...
  `).all(normalized.query, ...request.namespaceRefs, request.maxHits);
    const hits = rows.map((row, index) => {
        if (typeof row.namespace_ref !== 'string' ||
            ...
            !Number.isFinite(row.lexical_score))
            return invalidMemoryValue();
        return Object.freeze({
...
export function createSqliteMemoryLexicalIndexV1(optionsValue) {
    ...
    `);
        for (const { record, revisionHash } of documents) {
            if (!LEXICAL_CONTENT.test(record.text))
                continue;
            ...
            insert.run(record.namespaceRef, record.namespaceGeneration, record.memoryId, record.revision, revisionHash, normalized.body, Buffer.byteLength(normalized.body, 'utf8'), Date.parse(record.updatedAt));
        }
    };
    const beginProjection = (value) => {
    ...
    const applyProjectionBatch = (value) => {
    ...
    const completeProjection = (value) => {
        const projection = parseProjectionControl(value);
        let started = false;
        try {
            options.database.exec('BEGIN IMMEDIATE');
