...
import { decodeMemoryLifecycleCommandWireV1, memoryLifecycleCommandHashV1, parseMemoryLifecycleCommandV1 } from './memory-lifecycle-command.js';
import { encodeMemoryConsentEvidenceV1, encodeMemoryProposalV2, encodeMemoryRevisionV2 } from './memory-lifecycle-codec.js';
import { memoryLifecycleDomainHashV1, parseMemoryLifecycleInstantV1 } from './memory-lifecycle-domain.js';
import { createMemoryLifecycleResultV1, decodeMemoryLifecycleStableResultWireV1, encodeMemoryLifecycleStableResultWireV1, memoryLifecycleStableResultHashV1 } from './memory-lifecycle-result.js';
...
const DIRECT_SAVE_RECEIPT_HASH_DOMAIN_V1 = 'groupmate.memory.lifecycle-direct-save-receipt.v1';
const LIFECYCLE_AGGREGATE_REF_HASH_DOMAIN_V1 = 'groupmate.memory.lifecycle-aggregate-ref.v1';
...
function denied(commandHash, category) {
    return resultFor('proposal.createAndApprove', commandHash, { status: 'denied', category });
}
function authorityDenial(envelope, freshNow, namespaceRef, generation, actorRef) {
    if (!memoryAccessCapabilityAllowsV1(envelope.access, namespaceRef, freshNow))
        return 'access';
    if (envelope.authority.kind !== 'actor' ||
        memoryLifecycleActorCapabilityRoleV1(envelope.authority.capability) !== 'personal_subject') {
        return 'authority';
    }
    const request = (action, requiredAuthority) => ({
    ...
    return memoryLifecycleActorCapabilityAllowsV1(envelope.authority.capability, request('propose_create', 'safe'), freshNow) && memoryLifecycleActorCapabilityAllowsV1(envelope.authority.capability, request('approve', 'ordinary'), freshNow)
        ? null
        : 'authority';
}
function coreUsageFromRow(row) {
...
function assertNotUndercounted(stored, actual) {
...
function validateConservativeUsage(database, namespaceRef, generation, namespace, global) {
...
function loadNamespaceState(database, bundle) {
  ...
  `).get(proposal.namespaceRef);
    if (row === undefined) {
        if (proposal.namespaceGeneration !== 1) {
            return Object.freeze({ generationMismatch: true });
        }
        return Object.freeze({
    ...
    let storedNamespace;
    try {
        storedNamespace = parseMemoryNamespaceV1(JSON.parse(storedWire));
    }
    catch {
        throw new CanonicalLifecycleProposalDataErrorV1();
    }
  ...
  `).get(proposal.namespaceRef, proposal.namespaceGeneration);
  ...
  `).get(proposal.namespaceRef);
    ...
    return Object.freeze({
        ...
        generationLifecycle: Object.freeze({
            ...
            lifecycleCommandLogicalBytes: exactInteger(rowValue(usageRow, 'lifecycle_command_logical_bytes'))
        }),
        lifecycle: Object.freeze({
            lifecycleCommandRecords: exactInteger(rowValue(lifecycleRow, 'lifecycle_command_records')),
            lifecycleCommandLogicalBytes: exactInteger(rowValue(lifecycleRow, 'lifecycle_command_logical_bytes'))
        }),
        namespaceWireBytes
    });
}
function insertNamespaceState(database, bundle, namespaceWireBytes, nowMs) {
    const proposal = bundle.proposal;
    const namespaceWire = memoryNamespaceWireV1(proposal.namespace);
  ...
  `).run(proposal.namespaceRef, namespaceWire, namespaceWireBytes, proposal.namespaceGeneration, nowMs, nowMs).changes);
    requireOneChange(database.prepare(`
    INSERT INTO usage(
...
function insertEvent(database, prepared) {
  ...
  `).run(event.sequence, event.eventId, event.namespaceRef, event.namespaceGeneration, event.aggregate, event.aggregateId, event.revision, event.eventKind, Date.parse(event.occurredAt), Date.parse(event.occurredAt), prepared.wire, prepared.wireBytes).changes);
}
function existingLedgerResult(database, wire, bundle, commandHash) {
    const row = database.prepare(`
  ...
  `).get(wire.namespaceRef, wire.commandRef);
    ...
    const expectedAggregateRefHash = memoryLifecycleDomainHashV1(LIFECYCLE_AGGREGATE_REF_HASH_DOMAIN_V1, bundle.record.memoryId);
...
function directSaveReceiptHash(commandHash, bundle) {
    return memoryLifecycleDomainHashV1(DIRECT_SAVE_RECEIPT_HASH_DOMAIN_V1, JSON.stringify({
        commandHash,
...
function capacityFailure(usage, lifecycle, global) {
    ...
    if (usage.canonicalLogicalBytes > NAMESPACE_CANONICAL_SOFT_BYTES ||
        global.canonicalLogicalBytes > DEPLOYMENT_CANONICAL_SOFT_BYTES)
        return 'canonical_bytes';
    if (global.pendingOutboxRecords > OUTBOX_SOFT_RECORDS)
        return 'outbox_records';
    if (global.outboxLogicalBytes > OUTBOX_SOFT_BYTES)
        return 'outbox_bytes';
...
function executeCreateAndApprove(database, envelope, freshNow) {
    ...
    const bundle = command.material;
    const denial = authorityDenial(envelope, freshNow, wire.namespaceRef, wire.expectedNamespaceGeneration, wire.initiatedByActorRef);
    if (denial !== null)
        return denied(commandHash, denial);
    const replay = existingLedgerResult(database, wire, bundle, commandHash);
    if (replay === 'conflict') {
    ...
    if (namespace.generationMismatch)
        return denied(commandHash, 'authority');
    validateConservativeUsage(database, wire.namespaceRef, wire.expectedNamespaceGeneration, namespace, globalBefore);
    ...
    SELECT 1 FROM heads WHERE namespace_ref = ? AND namespace_generation = ?
      AND memory_id = ?
    LIMIT 1
  `).get(wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.proposal.proposalId, wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.record.memoryId);
    ...
    const events = [
        prepareEvent(sequence, wire.namespaceRef, wire.expectedNamespaceGeneration, 'proposal', bundle.proposal.proposalId, bundle.proposal.revision, 'proposal_changed', wire.occurredAt),
        prepareEvent(sequence + 1, wire.namespaceRef, wire.expectedNamespaceGeneration, 'record', bundle.record.memoryId, bundle.record.revision, 'record_upserted', wire.occurredAt)
    ];
    ...
    const lifecycle = Object.freeze({
        ...
        lifecycleCommandLogicalBytes: namespace.lifecycle.lifecycleCommandLogicalBytes + resultWireBytes
    });
    const generationLifecycle = Object.freeze({
        lifecycleCommandRecords: namespace.generationLifecycle.lifecycleCommandRecords + 1,
        lifecycleCommandLogicalBytes: namespace.generationLifecycle.lifecycleCommandLogicalBytes + resultWireBytes
    });
    const global = Object.freeze({
        ...
        lifecycleCommandLogicalBytes: globalBefore.lifecycleCommandLogicalBytes + resultWireBytes
    });
    const capacity = capacityFailure(usage, lifecycle, global);
    if (capacity !== null)
        return resultFor(wire.operation, commandHash, {
  ...
  `).run(wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.revision.memoryId, bundle.revision.revisionHash, Date.parse(bundle.revision.changedAt), revisionBytes).changes);
    requireOneChange(database.prepare(`
    INSERT INTO revision_payloads(
      namespace_ref, namespace_generation, memory_id, revision, revision_wire
    ) VALUES (?, ?, ?, 1, ?)
  `).run(wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.revision.memoryId, revisionWire).changes);
  ...
  `).run(wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.record.memoryId, bundle.revision.revisionHash, bundle.record.contentHash, cursorRef, Date.parse(bundle.record.updatedAt), Date.parse(bundle.record.retention.validUntil), Date.parse(bundle.record.retention.purgeAt)).changes);
    requireOneChange(database.prepare(`
  ...
  `).run(wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.proposal.proposalId, Date.parse(bundle.proposal.proposedAt), Date.parse(bundle.proposal.decision.decidedAt), bundle.record.memoryId, bundle.revision.revisionHash, proposalWire, proposalBytes).changes);
    requireOneChange(database.prepare(`
  ...
  `).run(wire.namespaceRef, wire.expectedNamespaceGeneration, bundle.consentEvidence.evidenceId, bundle.proposal.proposalId, bundle.consentEvidence.evidenceHash, evidenceWire, evidenceBytes, Date.parse(bundle.consentEvidence.approvedAt)).changes);
  ...
  `).run(wire.namespaceRef, wire.expectedNamespaceGeneration, wire.commandRef, commandHash, memoryLifecycleDomainHashV1(LIFECYCLE_AGGREGATE_REF_HASH_DOMAIN_V1, bundle.record.memoryId), resultWire, resultWireBytes, memoryLifecycleStableResultHashV1(resultWire), Date.parse(freshNow), Date.parse(freshNow) + MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleCommandLedgerTtlMs).changes);
    requireOneChange(database.prepare(`
  ...
  `).run(usage.activeMemoryRecords, usage.retainedRevisionRecords, usage.canonicalLogicalBytes, usage.pendingOutboxRecords, usage.outboxLogicalBytes, generationLifecycle.lifecycleCommandRecords, generationLifecycle.lifecycleCommandLogicalBytes, Date.parse(freshNow), wire.namespaceRef, wire.expectedNamespaceGeneration).changes);
    requireOneChange(database.prepare(`
    UPDATE lifecycle_namespace_usage
    SET lifecycle_command_records = ?, lifecycle_command_logical_bytes = ?, updated_at_ms = ?
    WHERE namespace_ref = ?
  `).run(lifecycle.lifecycleCommandRecords, lifecycle.lifecycleCommandLogicalBytes, Date.parse(freshNow), wire.namespaceRef).changes);
    requireOneChange(database.prepare(`
    UPDATE global_usage SET namespace_records = ?, active_memory_records = ?,
      ...
      lifecycle_command_records = ?, lifecycle_command_logical_bytes = ?, updated_at_ms = ?
    WHERE singleton = 1
  `).run(global.namespaceRecords, global.activeMemoryRecords, global.retainedRevisionRecords, global.canonicalLogicalBytes, global.pendingOutboxRecords, global.outboxLogicalBytes, global.lifecycleCommandRecords, global.lifecycleCommandLogicalBytes, Date.parse(freshNow)).changes);
    requireOneChange(database.prepare(`
...
function failureResult(error, commandHash) {
...
export function createSqliteMemoryLifecycleProposalAdapterV1(options) {
    if (options === null || typeof options !== 'object' || utilTypes.isProxy(options) ||
        ...
        throw new TypeError('invalid lifecycle proposal adapter options');
