...
import { buildMemoryProposalDecisionV2, projectMemoryProposalLifecycleV2, projectMemoryRecordLifecycleV2 } from './memory-lifecycle-builder.js';
import { decodeDeletionStatusV1, decodeMemoryConsentEvidenceV1, decodeMemoryLifecycleAuditV1, decodeMemoryProposalV2, decodeMemoryV1ToV2AggregateManifestV1, encodeDeletionStatusV1, encodeMemoryLifecycleAuditV1, encodeMemoryProposalV2 } from './memory-lifecycle-codec.js';
import { createDeletionStatusV1, createMemoryLifecycleAuditV1, MEMORY_PROPOSAL_DEADLINE_DAYS_V2, MEMORY_RETENTION_PURGE_GRACE_DAYS_V2, memoryLifecycleDomainHashV1, parseMemoryLifecycleInstantV1 } from './memory-lifecycle-domain.js';
import { MEMORY_LIFECYCLE_MAINTENANCE_ACTOR_REF_V1 } from './memory-lifecycle-port.js';
import { decodeMemoryMaintenanceCommandWireV1, decodeMemoryMaintenanceBatchManifestV1, encodeMemoryMaintenanceBatchManifestV1, createMemoryMaintenanceBatchManifestV1, memoryMaintenanceBatchManifestHashV1, memoryMaintenanceCommandHashV1, memoryMaintenanceCommandRefHashV1 } from './memory-maintenance-port.js';
import { decodeMemoryTombstoneV1, encodeMemoryOutboxEventV1, encodeMemoryTombstoneV1 } from './memory-codec.js';
import { createMemoryOutboxEventV1, createMemoryTombstoneV1 } from './memory-domain.js';
import { decodeMemoryLifecycleStableResultWireV1, memoryLifecycleStableResultHashV1 } from './memory-lifecycle-result.js';
...
import { CanonicalLifecycleMutationDataErrorV1, deleteValidatedSqliteRecordCarriersV1, loadValidatedSqliteRecordCarriersV1 } from './sqlite-memory-lifecycle-mutation.js';
...
const MAINTENANCE_AGGREGATE_REF_HASH_DOMAIN_V1 = 'groupmate.memory.maintenance-aggregate-ref.v1';
const MAINTENANCE_ITEM_RECEIPT_HASH_DOMAIN_V1 = 'groupmate.memory.maintenance-item-receipt.v1';
const MAINTENANCE_ACTOR_REF_HASH_DOMAIN_V1 = 'groupmate.memory.maintenance-actor-ref.v1';
const RETENTION_TOMBSTONE_ID_HASH_DOMAIN_V1 = 'groupmate.memory.retention-tombstone-id.v1';
const EXPORT_REF_HASH_DOMAIN_V1 = 'groupmate.memory.export-ref.v1';
const DELETION_TOMBSTONE_ID_HASH_DOMAIN_V1 = 'groupmate.memory.deletion-tombstone-id.v1';
const MUTATION_AGGREGATE_REF_HASH_DOMAIN_V1 = 'groupmate.memory.lifecycle-aggregate-ref.v1';
...
function requireOneChange(value) {
    if (value !== 1 && value !== 1n)
        throw new CanonicalMaintenanceDataErrorV1();
}
function canonicalWire(wireValue, byteValue, decode) {
...
function freezeTrustedNow(database, now) {
  ...
  `).get();
    ...
    if (!Number.isFinite(trusted.getTime()))
        throw new CanonicalMaintenanceDataErrorV1();
...
function loadNamespaceGeneration(database, namespaceRef) {
    const row = database.prepare(`
  ...
  `).get(namespaceRef);
    if (row === undefined)
        return null;
    const loaded = canonicalWire(rowValue(row, 'namespace_wire'), rowValue(row, 'namespace_wire_bytes'), wire => parseMemoryNamespaceV1(JSON.parse(exactString(wire))));
...
function lockedAuthorizationDenial(envelope, freshNow) {
    ...
    if (!memoryMaintenanceCapabilityAllowsRequestV1(envelope.maintenance, request, freshNow))
        return Object.freeze({ status: 'denied', category: 'authority' });
    return null;
}
function generationUsageActual(database, namespaceRef, generation) {
...
function namespaceLifecycleActual(database, namespaceRef) {
    const row = database.prepare(`
    SELECT
  ...
  `).get(...Array.from({ length: 10 }, () => namespaceRef));
    if (row === undefined)
        throw new CanonicalMaintenanceDataErrorV1();
    return row;
}
function globalUsageActual(database) {
    const row = database.prepare(`
    SELECT
...
function assertSameFields(stored, actual, fields) {
...
function assertUsageExact(database, namespaceRef) {
  ...
  `).all(namespaceRef);
    ...
    for (const row of rows) {
        assertSameFields(row, generationUsageActual(database, namespaceRef, positiveInteger(rowValue(row, 'namespace_generation'))), USAGE_FIELDS);
    }
  ...
  `).get(namespaceRef);
    ...
    const global = database.prepare('SELECT * FROM global_usage WHERE singleton = 1').get();
    if (global === undefined)
        throw new CanonicalMaintenanceDataErrorV1();
    assertSameFields(global, globalUsageActual(database), GLOBAL_USAGE_FIELDS);
}
function capacityAllowed(database, namespaceRef) {
    ...
    if (exactInteger(rowValue(global, 'namespace_records')) > MEMORY_RESOURCE_LIMITS.deploymentNamespaces ||
        ...
        exactInteger(rowValue(global, 'lifecycle_audit_logical_bytes')) +
            ...
            LIFECYCLE_AUDIT_BYTES_PER_DEPLOYMENT)
        return false;
    const rows = database.prepare(`
    SELECT namespace_generation FROM usage WHERE namespace_ref = ?
  `).all(namespaceRef);
    for (const row of rows) {
        const actual = generationUsageActual(database, namespaceRef, positiveInteger(rowValue(row, 'namespace_generation')));
...
function synchronizeUsage(database, namespaceRef, nowMs) {
    const rows = database.prepare(`
    SELECT namespace_generation FROM usage WHERE namespace_ref = ?
...
function itemReceiptHash(operation, aggregateRef, committedAt, outcome) {
    return memoryLifecycleDomainHashV1(MAINTENANCE_ITEM_RECEIPT_HASH_DOMAIN_V1, JSON.stringify({ operation, aggregateRef, committedAt, outcome }));
}
function aggregateRefHash(value) {
    return memoryLifecycleDomainHashV1(MAINTENANCE_AGGREGATE_REF_HASH_DOMAIN_V1, value);
}
function maintenanceActorHash() {
    return memoryLifecycleDomainHashV1(MAINTENANCE_ACTOR_REF_HASH_DOMAIN_V1, MEMORY_LIFECYCLE_MAINTENANCE_ACTOR_REF_V1);
}
function validateProposalRow(row) {
...
function validateLifecycleAuditRow(row, expectedNamespaceRef) {
    const audit = canonicalWire(rowValue(row, 'audit_wire'), rowValue(row, 'audit_wire_bytes'), decodeMemoryLifecycleAuditV1).value;
...
function proposalRows(database, namespaceRef, generation, operation, freshNow) {
  ...
  `).all(namespaceRef, generation);
    ...
    const expiredGraceMs = (MEMORY_PROPOSAL_DEADLINE_DAYS_V2 + MEMORY_RETENTION_PURGE_GRACE_DAYS_V2) * 24 * 60 * 60 * 1_000;
    return database.prepare(`
    SELECT namespace_ref, namespace_generation, proposal_id, revision, state,
      proposed_at_ms, decided_at_ms, resulting_memory_id, resulting_revision,
      resulting_revision_hash, proposal_wire, proposal_wire_bytes
  ...
  `).all(namespaceRef, generation, Date.parse(freshNow) - expiredGraceMs, Date.parse(freshNow) - graceMs);
}
function insertProposalEvent(database, proposal, occurredAt) {
  ...
  `).get();
    ...
    if (!Number.isSafeInteger(sequence))
        throw new CanonicalMaintenanceDataErrorV1();
    const eventId = `event:${memoryLifecycleDomainHashV1(MEMORY_OUTBOX_EVENT_ID_HASH_DOMAIN_V1, JSON.stringify({
        sequence,
  ...
  `).run(sequence, event.eventId, event.namespaceRef, event.namespaceGeneration, event.aggregateId, event.revision, Date.parse(occurredAt), Date.parse(occurredAt), wire, bytes).changes);
}
function validateAndDeleteProposalCarriers(database, proposal) {
  ...
  `).all(proposal.namespaceRef, proposal.namespaceGeneration, proposal.proposalId);
    if (evidenceRows.length > 1 ||
        (proposal.state === 'approved') !== (evidenceRows.length === 1)) {
        throw new CanonicalMaintenanceDataErrorV1();
    }
    for (const row of evidenceRows) {
        const evidence = canonicalWire(rowValue(row, 'evidence_wire'), rowValue(row, 'evidence_wire_bytes'), decodeMemoryConsentEvidenceV1).value;
  ...
  `).all(proposal.namespaceRef, proposal.namespaceGeneration, proposal.proposalId);
    ...
    for (const row of manifests) {
        const manifest = canonicalWire(rowValue(row, 'manifest_wire'), rowValue(row, 'manifest_wire_bytes'), decodeMemoryV1ToV2AggregateManifestV1).value;
...
function insertProposalPrunedAudit(database, commandRefHash, proposal, freshNow, itemHash) {
  ...
  `).run(audit.namespaceRef, audit.namespaceGeneration, audit.auditId, audit.operation, audit.commandRefHash, audit.aggregateRefHash, audit.authorizedByActorRefHash, audit.executedByActorRefHash, Date.parse(audit.sourceCommittedAt), Date.parse(audit.recordedAt), Date.parse(audit.expiresAt), wire, Buffer.byteLength(wire, 'utf8')).changes);
}
function maintainProposals(database, operation, namespaceRef, generation, commandRef, limit, freshNow) {
    const rows = proposalRows(database, namespaceRef, generation, operation, freshNow);
    if (operation === 'proposal.expireDue' &&
    ...
    for (const before of selected) {
      ...
      `).run(Date.parse(freshNow), wire, Buffer.byteLength(wire, 'utf8'), namespaceRef, generation, before.proposalId).changes);
            insertProposalEvent(database, proposal, freshNow);
        }
        const lifecycle = projectMemoryProposalLifecycleV2(proposal, freshNow);
        const pruned = !lifecycle.fullWireReadable;
        const itemHash = itemReceiptHash(operation, proposal.proposalId, freshNow, pruned ? 'expired_and_pruned' : 'expired');
        if (pruned) {
...
function insertRecordForgottenEvent(database, namespaceRef, generation, memoryId, revision, occurredAt) {
    ...
    INSERT INTO outbox(
      ...
      NULL, NULL, NULL, 0, NULL)
  `).run(sequence, event.eventId, namespaceRef, generation, memoryId, revision, Date.parse(occurredAt), Date.parse(occurredAt), wire, Buffer.byteLength(wire, 'utf8')).changes);
}
function purgeExpiredRecords(database, namespaceRef, generation, operation, limit, freshNow) {
    const rows = database.prepare(`
  ...
  `).all(namespaceRef, generation, Date.parse(freshNow));
    const selected = rows.slice(0, limit);
    const hashes = [];
    for (const row of selected) {
        const memoryId = exactString(rowValue(row, 'memory_id'));
        const loaded = loadValidatedSqliteRecordCarriersV1(database, namespaceRef, generation, memoryId, freshNow);
        if (loaded === null ||
            projectMemoryRecordLifecycleV2(loaded.revision.record, freshNow).state !== 'purge_due') {
            throw new CanonicalMaintenanceDataErrorV1();
        }
        deleteValidatedSqliteRecordCarriersV1(database, namespaceRef, generation, memoryId, loaded.carriers);
        const tombstoneId = `tombstone:${memoryLifecycleDomainHashV1(RETENTION_TOMBSTONE_ID_HASH_DOMAIN_V1, JSON.stringify({ namespaceRef, generation, memoryId, revision: loaded.revision.revision }))}`;
    ...
    `).run(namespaceRef, generation, tombstone.tombstoneId, memoryId, tombstone.deletedRevision, Date.parse(tombstone.deletedAt), Date.parse(tombstone.expiresAt), tombstone.receiptHash, tombstoneWire, Buffer.byteLength(tombstoneWire, 'utf8')).changes);
        insertRecordForgottenEvent(database, namespaceRef, generation, memoryId, loaded.revision.revision, freshNow);
...
function deletionTombstoneId(deletionRef) {
    return `tombstone:${memoryLifecycleDomainHashV1(DELETION_TOMBSTONE_ID_HASH_DOMAIN_V1, deletionRef)}`;
}
function carrierKinds(database, namespaceRef, generation, memoryId) {
  ...
  `).get(...args) !== undefined;
    const result = [
        ...
        revisionPayload ? 'revision_payload' : null,
        contentOutbox ? 'content_outbox' : null
    ].filter((kind) => kind !== null);
    return Object.freeze(result);
}
function loadCheckpoint(database, namespaceRef, deletionRef) {
    const row = database.prepare(`
  ...
  `).get(namespaceRef, deletionRef);
    if (row === undefined)
        return null;
    const loaded = canonicalWire(rowValue(row, 'checkpoint_wire'), rowValue(row, 'checkpoint_wire_bytes'), decodeDeletionStatusV1);
...
function storeCheckpoint(database, status, receiptHash) {
  ...
  `).get(status.namespaceRef, status.deletionRef);
    ...
    `).run(status.namespaceRef, status.deletionRef, status.deletingGeneration, status.observedCurrentGeneration, status.canonicalBodies, status.payloadDeletion, status.walCheckpoint, status.derivedCleanup, status.stage, receiptHash, wire, bytes, Date.parse(status.observedAt)).changes);
        return;
    }
    requireOneChange(database.prepare(`
  ...
  `).run(status.observedCurrentGeneration, status.canonicalBodies, status.payloadDeletion, status.walCheckpoint, status.derivedCleanup, status.stage, wire, bytes, Date.parse(status.observedAt), status.namespaceRef, status.deletionRef, receiptHash).changes);
}
function assertNoOrphanScrubCarriers(database, namespaceRef, generation) {
    const row = database.prepare(`
    ...
    LIMIT 1
  `).get(namespaceRef, generation, namespaceRef, generation, namespaceRef, generation, namespaceRef, generation, namespaceRef, generation, namespaceRef, generation);
    if (row !== undefined)
        throw new CanonicalMaintenanceDataErrorV1();
}
function scrubDeletedNamespace(database, namespaceRef, currentGeneration, targetGeneration, deletionRef, operation, limit, freshNow) {
  ...
  `).all(namespaceRef, targetGeneration, namespaceRef, targetGeneration, namespaceRef, targetGeneration);
    const selected = candidates.slice(0, limit);
    const hashes = [];
    for (const row of selected) {
        ...
        if (kind === 'record') {
            const loaded = loadValidatedSqliteRecordCarriersV1(database, namespaceRef, targetGeneration, aggregateId, freshNow);
            if (loaded !== null) {
                deleteValidatedSqliteRecordCarriersV1(database, namespaceRef, targetGeneration, aggregateId, loaded.carriers);
            }
            else {
                database.prepare(`
      ...
      `).get(namespaceRef, targetGeneration, aggregateId);
            if (exact !== undefined)
                validateAndDeleteProposalCarriers(database, validateProposalRow(exact));
            database.prepare(`
        DELETE FROM outbox WHERE namespace_ref = ? AND namespace_generation = ?
...
function verifyDeletedNamespace(database, namespaceRef, currentGeneration, targetGeneration, deletionRef, operation, freshNow) {
    const checkpoint = loadCheckpoint(database, namespaceRef, deletionRef);
    if (checkpoint === null || checkpoint.status.deletingGeneration !== targetGeneration) {
        throw new CanonicalMaintenanceDataErrorV1();
    }
    ...
    return Object.freeze({
        hashes: Object.freeze([
            itemReceiptHash(operation, deletionRef, freshNow, remaining.length === 0 ? 'verified_absent' : 'remaining')
        ]),
        hasMore: false
    });
}
function loadDeletionReceiptFromCommandRow(row) {
    const loaded = canonicalWire(rowValue(row, 'result_wire'), rowValue(row, 'result_wire_bytes'), decodeMemoryLifecycleStableResultWireV1);
    ...
    if (result.status !== 'deletion_pending' && result.status !== 'deletion_complete')
        return null;
    ...
    if (receipt.operation !== expectedReceiptOperation || aggregateRef === null ||
        ...
        receipt.commandRefHash !== memoryLifecycleDomainHashV1('groupmate.memory.lifecycle-command-ref.v1', exactString(rowValue(row, 'command_ref'))) ||
        exactString(rowValue(row, 'aggregate_ref_hash')) !== memoryLifecycleDomainHashV1(MUTATION_AGGREGATE_REF_HASH_DOMAIN_V1, aggregateRef) ||
        receipt.committedAt !== new Date(committedAt).toISOString()) {
        throw new CanonicalMaintenanceDataErrorV1();
    }
    return receipt;
}
function deletionReceiptForRef(database, namespaceRef, deletionRef) {
    const rows = database.prepare(`
...
function forgottenDeletionReceipts(database, namespaceRef) {
    const rows = database.prepare(`
    SELECT namespace_ref, namespace_generation, command_ref, command_hash, operation,
      aggregate_ref_hash, result_wire, result_wire_bytes, result_hash, committed_at_ms,
      expires_at_ms
  ...
  `).all(namespaceRef);
    ...
    for (const row of rows) {
        ...
        if (receipt === null)
            continue;
        const expectedAggregateHash = memoryLifecycleDomainHashV1(MUTATION_AGGREGATE_REF_HASH_DOMAIN_V1, receipt.memoryId);
...
function validateTombstoneRow(row, expectedNamespaceRef) {
    const tombstone = canonicalWire(rowValue(row, 'tombstone_wire'), rowValue(row, 'tombstone_wire_bytes'), decodeMemoryTombstoneV1).value;
...
function tombstoneForDeletion(database, receipt) {
    const row = database.prepare(`
  ...
  `).get(receipt.namespaceRef, receipt.generationAfter, receipt.tombstoneId);
    if (row === undefined)
        throw new CanonicalMaintenanceDataErrorV1();
...
function deletionMemoryId(database, namespaceRef, deletionRef) {
    const receipt = deletionReceiptForRef(database, namespaceRef, deletionRef);
    if (receipt !== null) {
        tombstoneForDeletion(database, receipt);
...
function derivedCleanupState(database, status, memoryId) {
...
function secureDeleteState(database) {
...
function prepareDeletionCheckpointBatch(database, namespaceRef, currentGeneration, limit, freshNow) {
  ...
  `).all(namespaceRef);
    ...
    if (refs.length < limit && !hasMore) {
        for (const receipt of forgottenDeletionReceipts(database, namespaceRef)) {
            if (receipt.generationAfter > currentGeneration)
                continue;
            if (database.prepare(`
        SELECT 1 AS present FROM namespace_deletion_checkpoints
        WHERE namespace_ref = ? AND deletion_ref = ?
      `).get(namespaceRef, receipt.deletionRef) !== undefined)
                continue;
            ...
            tombstoneForDeletion(database, receipt);
            const remaining = carrierKinds(database, namespaceRef, receipt.deletingGeneration, receipt.memoryId);
    ...
    for (const deletionRef of refs) {
        ...
        const remaining = carrierKinds(database, namespaceRef, checkpoint.status.deletingGeneration, memoryId);
...
function finalizeDeletionCheckpointBatch(database, namespaceRef, currentGeneration, operation, refs, hasMore, walCheckpoint, freshNow) {
    const hashes = [];
    for (const deletionRef of refs) {
        const checkpoint = loadCheckpoint(database, namespaceRef, deletionRef);
        if (checkpoint === null)
...
function checkpointForTombstone(database, namespaceRef, tombstoneId) {
    const rows = database.prepare(`
    SELECT deletion_ref FROM namespace_deletion_checkpoints
...
function purgeExpiredTombstones(database, namespaceRef, operation, limit, freshNow) {
    const selected = [];
    let hasMore = false;
    ...
    `).all(namespaceRef, Date.parse(freshNow), cursorExpiresAt, cursorExpiresAt, cursorTombstoneId, cursorExpiresAt, cursorTombstoneId, cursorGeneration);
        if (rows.length === 0)
            break;
        for (const row of rows) {
            ...
            const eligible = matching !== null
                ...
                : tombstone.deletedByActorRef === MEMORY_LIFECYCLE_MAINTENANCE_ACTOR_REF_V1 &&
                    carrierKinds(database, namespaceRef, tombstone.namespaceGeneration, tombstone.memoryId).length === 0 && database.prepare(`
            SELECT 1 AS present FROM outbox
            WHERE namespace_ref = ? AND namespace_generation = ?
              AND aggregate = 'record' AND aggregate_id = ?
              AND event_kind = 'record_forgotten' LIMIT 1
          `).get(namespaceRef, tombstone.namespaceGeneration, tombstone.memoryId) === undefined;
            if (!eligible)
                continue;
    ...
    for (const row of selected) {
      ...
      DELETE FROM tombstones WHERE namespace_ref = ? AND namespace_generation = ?
        AND tombstone_id = ?
    `).run(namespaceRef, positiveInteger(rowValue(row, 'namespace_generation')), tombstoneId).changes);
...
function purgeExpiredAudits(database, namespaceRef, operation, limit, freshNow) {
    const rows = database.prepare(`
  ...
  `).all(namespaceRef, Date.parse(freshNow));
    const selected = rows.slice(0, limit);
    const hashes = [];
    for (const row of selected) {
...
function purgeExpiredCommands(database, namespaceRef, currentCommandRef, operation, limit, freshNow) {
    const selected = [];
    let hasMore = false;
    let cursorExpiresAt = -1;
    ...
    `).all(namespaceRef, Date.parse(freshNow), currentCommandRef, cursorExpiresAt, cursorExpiresAt, cursorCommandRef);
        if (rows.length === 0)
            break;
        for (const row of rows) {
            cursorExpiresAt = exactInteger(rowValue(row, 'expires_at_ms'));
            cursorCommandRef = exactString(rowValue(row, 'command_ref'));
            const wire = canonicalWire(rowValue(row, 'result_wire'), rowValue(row, 'result_wire_bytes'), value => exactString(value)).wire;
            if (exactString(rowValue(row, 'namespace_ref')) !== namespaceRef) {
                throw new CanonicalMaintenanceDataErrorV1();
            }
            ...
            let purgeable = true;
            if (MEMORY_MAINTENANCE_OPERATIONS_V1.includes(storedOperation)) {
          ...
          `).get(receipt.namespaceRef, receipt.deletionRef) !== undefined;
                    if (!tombstoneExists) {
                        ...
                        if (checkpoint === null || checkpoint.status.stage !== 'canonical_complete') {
                            ...
                            carrierKinds(database, receipt.namespaceRef, receipt.deletingGeneration, receipt.memoryId).length > 0) {
                            throw new CanonicalMaintenanceDataErrorV1();
                        }
                    }
                }
...
function releaseExpiredExportReservations(database, namespaceRef, operation, limit, freshNow) {
  ...
  `).all(namespaceRef);
    ...
    const rows = candidateRows.filter(row => !terminalExportHashes.has(memoryLifecycleDomainHashV1(EXPORT_REF_HASH_DOMAIN_V1, exactString(rowValue(row, 'export_id')))));
    const selected = rows.slice(0, limit);
    const hashes = [];
    for (const row of selected) {
...
function executeOperation(database, envelope, freshNow) {
    const wire = decodeMemoryMaintenanceCommandWireV1(envelope.command.wire);
    if (wire.operation === 'proposal.expireDue' || wire.operation === 'proposal.purgeDecided') {
        return maintainProposals(database, wire.operation, wire.namespaceRef, wire.targetGeneration, wire.commandRef, wire.limit, freshNow);
    }
    if (wire.operation === 'record.purgeExpired') {
        return purgeExpiredRecords(database, wire.namespaceRef, wire.targetGeneration, wire.operation, wire.limit, freshNow);
    }
    if (wire.operation === 'namespace.scrubDeleted') {
        return scrubDeletedNamespace(database, wire.namespaceRef, wire.currentGeneration, wire.targetGeneration, wire.deletionRef, wire.operation, wire.limit, freshNow);
    }
    if (wire.operation === 'namespace.verifyScrubbed') {
        return verifyDeletedNamespace(database, wire.namespaceRef, wire.currentGeneration, wire.targetGeneration, wire.deletionRef, wire.operation, freshNow);
    }
    if (wire.operation === 'tombstone.purgeExpired') {
        return purgeExpiredTombstones(database, wire.namespaceRef, wire.operation, wire.limit, freshNow);
    }
    ...
    if (wire.operation === 'command.purgeExpired') {
        return purgeExpiredCommands(database, wire.namespaceRef, wire.commandRef, wire.operation, wire.limit, freshNow);
    }
    if (wire.operation === 'export.releaseExpiredReservations') {
        return releaseExpiredExportReservations(database, wire.namespaceRef, wire.operation, wire.limit, freshNow);
    }
...
function existingResult(database, envelope) {
  ...
  `).get(wire.namespaceRef, wire.commandRef);
    ...
    const loaded = canonicalWire(rowValue(row, 'result_wire'), rowValue(row, 'result_wire_bytes'), decodeMemoryMaintenanceBatchManifestV1);
    ...
    if (storedCommandHash !== commandHash || storedOperation !== wire.operation ||
        storedGeneration !== wire.currentGeneration)
        return 'conflict';
...
function insertResult(database, envelope, batch, freshNow) {
    const command = envelope.command;
    const wire = decodeMemoryMaintenanceCommandWireV1(command.wire);
    const commandHash = memoryMaintenanceCommandHashV1(command);
  ...
  `).run(wire.namespaceRef, wire.currentGeneration, wire.commandRef, commandHash, wire.operation, aggregateRefHash(wire.deletionRef ?? wire.namespaceRef), resultWire, Buffer.byteLength(resultWire, 'utf8'), memoryMaintenanceBatchManifestHashV1(manifest), Date.parse(freshNow), Date.parse(freshNow) + MEMORY_LIFECYCLE_RESOURCE_LIMITS.lifecycleCommandLedgerTtlMs).changes);
    return manifest;
}
function executeLocked(database, envelope, freshNow) {
    ...
    if (!capacityAllowed(database, wire.namespaceRef))
        throw new MaintenanceCapacityErrorV1();
...
function storageFailure(error) {
    if (error instanceof MaintenanceCapacityErrorV1) {
        return Object.freeze({ status: 'capacity', category: 'maintenance_capacity' });
    }
...
async function executeDeletionCheckpoint(database, now, envelope, signal) {
    const wire = decodeMemoryMaintenanceCommandWireV1(envelope.command.wire);
    let started = false;
    ...
    try {
        ...
        assertUsageExact(database, wire.namespaceRef);
        const prepared = prepareDeletionCheckpointBatch(database, wire.namespaceRef, wire.currentGeneration, wire.limit, freshNow);
        refs = prepared.refs;
        ...
        assertUsageExact(database, wire.namespaceRef);
        const batch = finalizeDeletionCheckpointBatch(database, wire.namespaceRef, wire.currentGeneration, wire.operation, refs, hasMore, walCheckpoint, finalizedAt);
...
export function createSqliteMemoryMaintenanceAdapterV1(options) {
    if (options === null || typeof options !== 'object' || utilTypes.isProxy(options) ||
        ...
        throw new TypeError('invalid sqlite memory maintenance options');
