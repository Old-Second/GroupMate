...
import { deriveMemoryCandidateProvenanceV1, memoryCandidateHashV1, memoryCandidateProvenanceHashV1, memoryCandidateValueRejectionReasonV1, parseMemoryExtractionJobV1, parseMemoryExtractorResultV1 } from './memory-candidate-pipeline.js';
...
import { MEMORY_DERIVATIVE_RESOURCE_LIMITS, MEMORY_RESOURCE_LIMITS, memoryAsciiWithinLimit, memoryCanonicalTextWithinLimits } from './memory-resource-limits.js';
...
function emptyBatch(status, overrides = {}) {
...
function readRss(rssBytes) {
    let value;
    try {
        value = Reflect.apply(rssBytes, undefined, []);
    }
    catch {
...
function parsePolicyDecision(value) {
    ...
    if (discriminator.status === 'disabled') {
        ...
        const reasons = ['deployment_off', 'user_opted_out', 'group_not_allowed', 'stale_policy'];
        if (typeof input.reason !== 'string' || !reasons.includes(input.reason)) {
            return invalidMemoryValue();
        }
        return Object.freeze({ status: 'disabled', reason: input.reason });
    }
    if (discriminator.status !== 'enabled')
        return invalidMemoryValue();
    const input = inspectMemoryRecord(value, ['status', 'mode']);
    if (input.mode !== 'shadow' && input.mode !== 'automatic')
        return invalidMemoryValue();
    return Object.freeze({ status: 'enabled', mode: input.mode });
}
function relatedMemoryIds(value) {
    const result = inspectMemoryArray(value, MEMORY_RESOURCE_LIMITS.conflictRefs).map(item => {
        if (typeof item !== 'string' || !MEMORY_ID.test(item))
            return invalidMemoryValue();
        return item;
    });
...
function parseClassification(value) {
    ...
    if (discriminator.status === 'distinct') {
        ...
        return Object.freeze({ status: 'distinct' });
    }
    ...
    if (!memoryCanonicalTextWithinLimits(input.note, MEMORY_RESOURCE_LIMITS.reasonTextUtf8Bytes, MEMORY_RESOURCE_LIMITS.reasonTextCodePoints) || input.note.trim() === '')
        return invalidMemoryValue();
    return Object.freeze({
        status: 'conflict',
        relatedMemoryIds: relatedMemoryIds(input.relatedMemoryIds),
...
function parseSubmissionResult(value) {
    const discriminator = inspectMemoryRecord(value, ['status'], ['outcome', 'proposalId', 'reason', 'retryable']);
...
function parseSubmissionConflict(value) {
    const input = inspectMemoryRecord(value, ['state', 'relatedMemoryIds', 'note']);
    if (input.state !== 'none' && input.state !== 'possible')
        return invalidMemoryValue();
    const relatedMemoryIds = inspectMemoryArray(input.relatedMemoryIds, MEMORY_RESOURCE_LIMITS.conflictRefs).map(item => {
        if (typeof item !== 'string' || !MEMORY_ID.test(item))
            return invalidMemoryValue();
        return item;
    });
    if (new Set(relatedMemoryIds).size !== relatedMemoryIds.length)
        return invalidMemoryValue();
    if (input.state === 'none') {
        if (relatedMemoryIds.length !== 0 || input.note !== null)
            return invalidMemoryValue();
        return Object.freeze({
            state: 'none',
    ...
    if (relatedMemoryIds.length === 0 || !memoryCanonicalTextWithinLimits(input.note, MEMORY_RESOURCE_LIMITS.reasonTextUtf8Bytes, MEMORY_RESOURCE_LIMITS.reasonTextCodePoints) || input.note.trim() === '')
        return invalidMemoryValue();
    return Object.freeze({
        state: 'possible',
...
export function parseMemoryCandidateSubmissionV1(value) {
    const input = inspectMemoryRecord(value, [
    ...
    if (extracted.status !== 'candidates')
        return invalidMemoryValue();
...
function conflictFor(classification) {
    return classification.status === 'conflict'
        ...
        : Object.freeze({ state: 'none', relatedMemoryIds: Object.freeze([]), note: null });
}
function canAutomaticallyApprove(job, candidate, policy, classification) {
...
export function memoryCandidateSubmissionAllowsPolicyApprovalV1(value) {
...
function auditId(jobId, candidateHash, outcome, reasonCode, proposalId) {
...
async function withTimeout(operation, callerSignal) {
    if (isAborted(callerSignal))
        throw new CandidateAbortErrorV1();
...
export function createMemoryCandidateWorkerV1(optionsValue) {
    ...
    const recordAudit = async (input) => {
        ...
        const result = await options.queue.recordAudit({
            auditId: auditId(input.job.jobId, candidateHash, input.outcome, input.reasonCode, input.proposalId),
        ...
        if (result.status === 'unavailable')
            throw new CandidateRetryErrorV1('audit_unavailable');
    };
    const processJob = async (jobValue, signal) => {
        ...
        await withTimeout(async (boundedSignal) => {
            ...
            const extracted = parseMemoryExtractorResultV1(await options.extractor.extract(job, boundedSignal), job);
            ...
            for (const candidate of extracted.candidates) {
                ...
                const provenance = deriveMemoryCandidateProvenanceV1(job, candidate);
                const classification = parseClassification(await options.classifier.classify(Object.freeze({ job, candidate, provenance }), boundedSignal));
                ...
                if (result.status === 'capacity' || result.status === 'unavailable') {
                    throw new CandidateRetryErrorV1(result.status === 'capacity' ? 'proposal_capacity' : 'proposal_unavailable');
                }
    ...
    return Object.freeze({
        async runBatch(signal) {
            ...
            running = true;
            try {
                const claim = await options.queue.claim(options.workerId, MEMORY_DERIVATIVE_RESOURCE_LIMITS.extractionWorkerBatchRecords, signal);
                if (claim.status === 'empty')
                    return emptyBatch('idle');
                if (claim.status === 'aborted')
                    return emptyBatch('aborted');
                if (claim.status === 'unavailable')
                    return emptyBatch('unavailable');
                let completed = 0;
                let retried = 0;
                let deadLettered = 0;
