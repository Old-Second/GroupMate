...
import { decodeMemoryOutboxEventV1, decodeMemoryProposalV1, decodeMemoryRevisionV1, decodeMemoryTombstoneV1 } from './memory-codec.js';
...
const V2_NAMESPACE_CANONICAL_SOFT_BYTES = MEMORY_RESOURCE_LIMITS.namespaceCanonicalLogicalBytes - (2 * 1_024 * 1_024);
const V2_DEPLOYMENT_CANONICAL_SOFT_BYTES = MEMORY_RESOURCE_LIMITS.deploymentCanonicalLogicalBytes - (8 * 1_024 * 1_024);
const V2_PENDING_PROPOSAL_SOFT_RECORDS = 240;
const V2_OUTBOX_SOFT_RECORDS = MEMORY_RESOURCE_LIMITS.unackedOutboxRecords - 256;
const V2_OUTBOX_SOFT_BYTES = MEMORY_RESOURCE_LIMITS.unackedOutboxLogicalBytes - (1 * 1_024 * 1_024);
const NAMESPACE_REF_CHECK = `
  length(namespace_ref) = 64 AND
  namespace_ref = lower(namespace_ref) AND
  namespace_ref NOT GLOB '*[^0-9a-f]*'
`.trim();
const HASH_CHECK = (column) => `
  length(${column}) = 64 AND
  ${column} = lower(${column}) AND
  ${column} NOT GLOB '*[^0-9a-f]*'
`.trim();
const SCHEMA_OBJECTS_V1 = Object.freeze([
    Object.freeze({
        type: 'table',
...
function sqliteAlteredTableSqlV2(sql, columns) {
    const marker = ') STRICT';
    const markerIndex = sql.lastIndexOf(marker);
    if (markerIndex < 0 || columns.length === 0)
        return invalidMemoryValue();
    return `${sql.slice(0, markerIndex)}, ${columns.join(', ')}${sql.slice(markerIndex)}`;
}
function sqliteAlteredTableBeforeConstraintsSqlV2(sql, columns) {
    const marker = ',\n  PRIMARY KEY';
...
const ALTERED_SCHEMA_SQL_V2 = Object.freeze({
    namespaces: sqliteAlteredTableSqlV2(v1SchemaObject('namespaces').sql, ['content_epoch INTEGER NOT NULL DEFAULT 0 CHECK(content_epoch >= 0)']),
    usage: sqliteAlteredTableBeforeConstraintsSqlV2(v1SchemaObject('usage').sql, USAGE_COLUMNS_V2),
    globalUsage: sqliteAlteredTableSqlV2(v1SchemaObject('global_usage').sql, GLOBAL_USAGE_COLUMNS_V2)
});
const PROPOSALS_TABLE_STAGE_SQL_V2 = v1SchemaObject('proposals').sql
    ...
    .replace("state IN ('pending', 'approved', 'rejected', 'expired')", "state IN ('pending', 'approved', 'rejected', 'expired', 'withdrawn')")
    .replace('resulting_revision IS NULL OR resulting_revision = 1', `resulting_revision IS NULL OR (resulting_revision BETWEEN 1 AND ${MEMORY_RESOURCE_LIMITS.memoryRetainedRevisions})`);
const PROPOSALS_TABLE_SQL_V2 = PROPOSALS_TABLE_STAGE_SQL_V2.replace('CREATE TABLE proposals_v2_stage(', 'CREATE TABLE "proposals"(');
const PROPOSAL_INDEX_OBJECTS_V1 = Object.freeze(SCHEMA_OBJECTS_V1.filter(object => (object.type === 'index' && object.tableName === 'proposals')));
...
const NEW_SCHEMA_OBJECTS_V2 = Object.freeze([
    Object.freeze({
        type: 'table',
...
const SCHEMA_OBJECTS_V2 = Object.freeze([
...
export const MEMORY_SQLITE_MIGRATION_V1 = Object.freeze({
    version: 1,
...
export const MEMORY_SQLITE_MIGRATION_V2 = Object.freeze({
    version: 2,
  ...
  `).all();
    ...
    return rows.map(row => {
        ...
        if (sqlBytes !== row.sql_bytes || sqlBytes > MAXIMUM_SCHEMA_OBJECT_SQL_BYTES ||
            ...
            row.total_sql_bytes > MAXIMUM_SCHEMA_SQL_BYTES)
            return invalidMemoryValue();
...
export function sqliteMemorySchemaFingerprintV1(database) {
...
function parseMigration(value, expectedVersion) {
    ...
    if (input.version !== expectedVersion || typeof input.sql !== 'string' ||
        ...
        !HASH_PATTERN.test(input.schemaFingerprint))
        return invalidMemoryValue();
    return Object.freeze({
...
function parseMigrationSequence(value) {
...
function validateAppliedMigrations(database, currentVersion, migrations) {
    const rows = database.prepare(`
...
function parseMigrationManifestsV2(value) {
    if (!Array.isArray(value) || utilTypes.isProxy(value) ||
    ...
    try {
        ...
        const aggregates = new Set(manifests.map(manifest => (`${manifest.namespaceRef}\0${manifest.namespaceGeneration}\0` +
        ...
        if (error instanceof MemorySqliteMigrationErrorV2)
            throw error;
        throw new MemorySqliteMigrationErrorV2('memory_migration_manifest_invalid');
    }
}
function exactNonnegativeIntegerV2(value) {
...
function exactPositiveIntegerV2(value) {
...
function exactStringV2(value) {
    if (typeof value !== 'string') {
        throw new MemorySqliteMigrationErrorV2('memory_migration_manifest_invalid');
    }
    return value;
}
function manifestAggregateKeyV2(namespaceRef, generation, kind, aggregateId) {
    return `${namespaceRef}\0${generation}\0${kind}\0${aggregateId}`;
}
function validateV1MigrationCapacityV2(database, manifests) {
    const manifestBytesByNamespace = new Map();
    let deploymentManifestBytes = 0;
    for (const manifest of manifests) {
        const wireBytes = Buffer.byteLength(encodeMemoryV1ToV2AggregateManifestV1(manifest), 'utf8');
        manifestBytesByNamespace.set(manifest.namespaceRef, (manifestBytesByNamespace.get(manifest.namespaceRef) ?? 0) + wireBytes);
  ...
  `).all();
    ...
    const canonicalByNamespace = new Map();
    ...
    for (const row of usageRows) {
        ...
        canonicalByNamespace.set(namespaceRef, (canonicalByNamespace.get(namespaceRef) ?? 0) + canonical);
...
function validateV1AggregateCoverageV2(database) {
...
function validateV1AncillaryCanonicalRowsV2(database) {
...
function validateV1MigrationManifestsV2(database, manifests) {
  ...
  `).all();
    ...
    try {
        for (const row of namespaceRows) {
            ...
            const parsed = parseMemoryNamespaceV1(JSON.parse(wire));
  ...
  `).all();
    ...
    const manifestsByAggregate = new Map(manifests.map(manifest => [
        manifestAggregateKeyV2(manifest.namespaceRef, manifest.namespaceGeneration, manifest.aggregate.kind, manifest.aggregate.aggregateId),
        manifest
    ]));
    ...
    for (const row of proposalRows) {
        ...
        const kind = resultingMemoryId === null ? 'proposal' : 'memory';
...
function persistV1MigrationManifestsV2(database, manifests, appliedAtMs) {
  ...
  `);
    const bytesByUsage = new Map();
    let totalBytes = 0;
    for (const manifest of manifests) {
        const wire = encodeMemoryV1ToV2AggregateManifestV1(manifest);
        const wireBytes = Buffer.byteLength(wire, 'utf8');
        const inserted = insert.run(manifest.namespaceRef, manifest.namespaceGeneration, manifest.manifestId, manifest.aggregate.kind, manifest.aggregate.aggregateId, manifest.manifestHash, wire, wireBytes, appliedAtMs);
...
function initializeLifecycleNamespaceUsageV2(database, appliedAtMs) {
    database.prepare(`
...
function backfillGlobalAggregateUsageV2(database) {
    const updated = database.prepare(`
    UPDATE global_usage
...
function legacyTrustedTimeHighWaterV2(database) {
...
function runParsedMigrationSequenceV1(database, migrations, appliedAt, hooks = {}) {
    ...
    try {
      ...
      `);
            ...
            insert.run(migration.version, migration.checksum, migration.schemaFingerprint, appliedAt);
            database.exec(`
...
export function runSqliteMemoryMigrationSequenceV1(database, optionsValue) {
...
export function assertSqliteMemoryMigrationV2Ready(database, manifestsValue) {
    ...
    runParsedMigrationSequenceV1(database, migrations, appliedAt, {
        onCurrent: currentVersion => {
            ...
            if (currentVersion === MEMORY_SQLITE_SCHEMA_VERSION_V1) {
                validateV1MigrationManifestsV2(database, manifests);
                trustedTimeHighWaterMs = Math.max(trustedTimeHighWaterMs, legacyTrustedTimeHighWaterV2(database));
            }
