...
function nativeSignalContainerPrototype(description) {
    const signal = new AbortController().signal;
    const key = Reflect.ownKeys(signal).find(value => (typeof value === 'symbol' && value.description === description));
    if (key === undefined)
...
function nativeSignalSetPrototype() {
    ...
    for (const [signal, description] of prototypes) {
        ...
        if (expected !== undefined && prototype !== expected)
            return invalidMemoryValue();
        expected = prototype;
    }
...
function parseNativeAbortSignal(value) {
    ...
    for (const key of keys) {
        ...
        if (key.description === 'kEvents' || key.description === 'kHandlers') {
            ...
            if (alreadyFound || container === null || typeof container !== 'object' ||
                utilTypes.isProxy(container))
                return invalidMemoryValue();
            let containerPrototype;
            let containerKeys;
            try {
                containerPrototype = Object.getPrototypeOf(container);
            ...
            if (key.description === 'kEvents')
                foundEventsSlot = true;
            else
                foundHandlersSlot = true;
            continue;
        }
        if (key.description === 'kSourceSignals' || key.description === 'kDependantSignals') {
            ...
            if (key.description === 'kSourceSignals')
                foundSourceSignalsSlot = true;
            else
                foundDependantSignalsSlot = true;
            continue;
        }
    ...
    if (!foundAbortedSlot || !foundEventsSlot || !foundHandlersSlot)
        return invalidMemoryValue();
