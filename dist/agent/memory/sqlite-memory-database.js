...
import { MEMORY_DERIVATIVE_RESOURCE_LIMITS, MEMORY_RESOURCE_LIMITS } from './memory-resource-limits.js';
import { assertSqliteMemoryMigrationV2Ready, assertSqliteMemoryMigrationV3Ready, MemorySqliteMigrationErrorV2, MEMORY_SQLITE_APPLICATION_ID_V1, MEMORY_SQLITE_MIGRATION_V1, MEMORY_SQLITE_MIGRATIONS_V2, MEMORY_SQLITE_MIGRATIONS_V3, MEMORY_SQLITE_SCHEMA_VERSION_V1, MEMORY_SQLITE_SCHEMA_VERSION_V2, MEMORY_SQLITE_SCHEMA_VERSION_V3, MEMORY_SQLITE_SCHEMA_FINGERPRINT_V1, MEMORY_SQLITE_SCHEMA_FINGERPRINT_V2, MEMORY_SQLITE_SCHEMA_FINGERPRINT_V3, runSqliteMemoryMigrationV2, runSqliteMemoryMigrationV3, runSqliteMemoryMigrationSequenceV1, sqliteMemorySchemaFingerprintV1 } from './sqlite-memory-migrations.js';
export { MEMORY_SQLITE_APPLICATION_ID_V1, MEMORY_SQLITE_SCHEMA_VERSION_V1, MEMORY_SQLITE_SCHEMA_VERSION_V2, MEMORY_SQLITE_SCHEMA_VERSION_V3 } from './sqlite-memory-migrations.js';
...
function exactMigrationMetadataV3(database) {
    const rows = database.prepare(`
    SELECT version, checksum, schema_fingerprint, applied_at
    FROM schema_migrations
    ORDER BY version ASC
    LIMIT 4
  `).all();
    if (rows.length !== MEMORY_SQLITE_MIGRATIONS_V3.length)
        return schemaUnsupported();
    rows.forEach((row, index) => {
...
function validateLifecycleDeploymentStateV2(database) {
...
function validateDatabaseIdentityV3(database) {
    requirePragmaValue(database, 'application_id', MEMORY_SQLITE_APPLICATION_ID_V1);
  ...
  `).all();
    if (projectors.length !== 2 || projectors[0]?.projector_kind !== 'lexical' ||
        projectors[1]?.projector_kind !== 'vector')
        return schemaUnsupported();
    for (const projector of projectors) {
        if ((projector.status !== 'disabled' && projector.status !== 'rebuild_required' &&
            ...
            (projector.index_fingerprint !== null && (typeof projector.index_fingerprint !== 'string' ||
                !/^[0-9a-f]{64}$/.test(projector.index_fingerprint))) || typeof projector.last_applied_sequence !== 'number' ||
            ...
            (projector.scan_namespace_ref !== null && (typeof projector.scan_namespace_ref !== 'string' ||
                !/^[0-9a-f]{64}$/.test(projector.scan_namespace_ref))) || (projector.scan_memory_id !== null && (typeof projector.scan_memory_id !== 'string' ||
            projector.scan_memory_id.length < 1 || projector.scan_memory_id.length > 128)) || ((projector.scan_namespace_ref === null) !==
        ...
        if ((inactive && (projector.projection_generation !== 0 || projector.index_fingerprint !== null ||
            projector.scan_namespace_ref !== null)) || (!inactive && (projector.projection_generation === 0 || projector.index_fingerprint === null)) || (projector.status === 'ready' && (projector.scan_namespace_ref !== null ||
...
function validateDatabaseIdentityForVersion(database, version) {
    ...
    if (version === MEMORY_SQLITE_SCHEMA_VERSION_V3)
        return validateDatabaseIdentityV3(database);
    return schemaUnsupported();
}
...
function preflightExistingDatabaseV3(location, manifests) {
    const { identity, schemaVersion } = readHeader(location, [
        MEMORY_SQLITE_SCHEMA_VERSION_V1,
        MEMORY_SQLITE_SCHEMA_VERSION_V2,
...
function bootstrapFileDatabaseV3(location, now, manifests) {
    const directory = path.dirname(location);
    const temporary = path.join(directory, `.${path.basename(location)}.bootstrap-v3-${process.pid}-${randomUUID()}`);
    let descriptor;
    let database;
    try {
        descriptor = openSync(temporary, 'wx', 0o600);
...
function openWritableDatabaseV3(location, expectedIdentity, schemaVersion, now, manifests) {
    const database = openDatabase(location, false);
    try {
        sameFileIdentity(location, expectedIdentity);
        ...
        validateDatabaseIdentityTransactionForVersion(database, true, MEMORY_SQLITE_SCHEMA_VERSION_V3);
        configureRuntime(database, false);
    }
    catch (error) {
        database.close();
...
function openInMemoryDatabaseV3(now, manifests) {
    const database = openDatabase(':memory:', false);
    try {
        configureRuntime(database, true);
        runSqliteMemoryMigrationV3(database, {
...
export function openSqliteMemoryDatabaseV3(optionsValue) {
    const options = parseOptionsV2(optionsValue);
    databaseConstructor();
    if (options.location === ':memory:') {
    ...
    try {
        ...
        return openWritableDatabaseV3(options.location, preflight.identity, preflight.schemaVersion, options.now, options.manifests);
    }
    catch (error) {
        if (error instanceof SqliteMemoryDatabaseErrorV1 ||
            error instanceof MemorySqliteMigrationErrorV2)
