...
function decodedIdentifier(value, label) {
    try {
...
export function resolveConversationScope(input) {
...
export function serializeConversationScope(scope) {
    switch (scope.kind) {
        case 'private':
...
export function canonicalSessionKey(address) {
...
export function legacySessionKey(scope) {
    return `${legacyPrefix}${serializeConversationScope(scope)}`;
}
export function parseCanonicalSessionKey(key) {
...
export function parseLegacySessionKey(key) {
    if (!key.startsWith(legacyPrefix))
        return null;
    const segments = key.slice(legacyPrefix.length).split(':');
    const numericId = (value) => /^\d+$/.test(value ?? '');
