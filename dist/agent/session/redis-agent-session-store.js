...
import { legacySessionCodec } from './legacy-session-codec.js';
...
function storageError(stage, operation, cause) {
    return new AgentError({
...
export function agentSessionKey(address) {
    const v1 = canonicalSessionKey(address);
    if (!v1.startsWith(V1_NAMESPACE))
        throw new TypeError('agent session address is invalid');
...
function sessionSummary(value, source) {
...
function defaultStarter(address) {
...
export class RedisAgentSessionStore {
    ...
    async get(address, options = {}) {
        ...
        assertNotAborted(options.signal);
        if (v2 !== null)
            return this.#decodeV2(v2, address);
        const sources = Object.freeze([
            Object.freeze({
                kind: 'v1',
                key: canonicalSessionKey(address),
                decode: (raw) => legacySessionCodec.decodeCanonical(raw, address)
            }),
            Object.freeze({
                kind: 'legacy',
                key: legacySessionKey(address.scope),
                decode: (raw) => legacySessionCodec.decodeLegacy(raw, {
                    address,
                    now: this.#now(),
        ...
        for (const source of sources) {
            ...
            if (ttl === 0 || ttl === -2)
                continue;
            if (ttl < -2)
                throw invalidData('agent.session.migrate', `ttl_${source.kind}`, new TypeError('invalid TTL'));
            let projected;
            try {
    ...
    async *list(query, options = {}) {
        ...
        for await (const key of this.#scan(`${AGENT_SESSION_NAMESPACE}${encodedBot}:*`, options.signal)) {
            if (yielded >= limit)
                return;
    ...
    #projectionFailed(source, address) {
        try {
    ...
    async #writeV2(value, ttlSeconds, stage) {
        let encoded;
        try {
        ...
        try {
            await this.#redis.set(agentSessionKey({ botId: value.botId, scope: value.scope }), encoded, ttlSeconds === undefined ? undefined : { EX: ttlSeconds });
        }
        catch (error) {
