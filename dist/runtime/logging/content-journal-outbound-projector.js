...
function projectedText(value, label, budget, allowEmpty = false) {
    const result = text(value, label, allowEmpty);
    budget.consumeText(result, label);
    return result;
}
function projectTextAtom(value, budget) {
...
function projectButtons(value, budget) {
...
function projectResource(value, budget) {
    ...
    if (url !== undefined)
        budget.consumeText(url, 'outbound resource URL');
    return {
        kind: input.kind,
        ...(url === undefined ? {} : { url }),
        mimeType,
...
function projectOutboundPart(value, budget) {
...
function projectReceipt(value, budget) {
    ...
    return {
        ...
        ...(input.messageId === undefined
            ? {}
            : {
                messageId: projectedText(input.messageId, 'outbound receipt message ID', budget)
            })
    };
}
function projectDeliveryResult(value, media, attempt, budget) {
