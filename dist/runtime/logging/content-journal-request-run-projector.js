...
import { boundedRecord, exactKeys, finiteNumber, jsonArray, jsonObject, ownDataRecord, projectAgentMessageResources, projectSessionAddress, projectToolResultResources, safeInteger, text, timestamp } from './content-journal-projection.js';
...
function parseReasoning(value, label) {
    const reasoning = jsonObject(value, label);
...
function projectTerminalCheckpointResources(checkpoint) {
    ...
    const preparedBatch = checkpoint.preparedBatch === null
        ...
        : Object.freeze({
            ...
            calls: Object.freeze(checkpoint.preparedBatch.calls.map(call => (call.kind === 'completed'
...
function projectTerminalEvent(input) {
...
export function projectRequestJournalEvent(request) {
    return {
        type: 'request.received',
        payload: { request: projectRequest(request) }
    };
}
export function projectRunJournalEvent(value) {
    const envelope = ownDataRecord(value, 'run journal event');
    if (envelope.type === 'run.terminal_committed')
        return projectTerminalEvent(envelope);
