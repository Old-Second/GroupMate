...
function field(name, label, bottomHelpMessage, component = 'Input', componentProps) {
    return {
        field: name,
        label,
        bottomHelpMessage,
...
export function buildGuobaSchemas({ vitsRoleOptions, voicevoxRoleOptions, azureRoleOptions }) {
    return [
        ...
        field('apiContextWindowTokens', '模型上下文窗口 Token', '0 表示使用模型 Profile；未知模型使用 32,768 Token 安全默认值。正整数只会缩小已知 Profile 能力，或为未知标准 OpenAI-compatible 模型显式声明窗口，并在新任务创建时冻结；暂停任务恢复时不会重读。大窗口仍受 512 KiB 请求、256 KiB checkpoint、Redis 与内存硬上限约束。修改后必须重启。', 'InputNumber', { min: 0, max: 1_000_000, step: 1 }),
        ...
        field('bymRecognizeLeadingAlias', '识别句首称呼', '开启后，消息以 Yunzai 机器人别名或助手称呼开头时视为明确点名并触发回复；关闭后句中、句尾称呼仍会触发。', 'Switch'),
