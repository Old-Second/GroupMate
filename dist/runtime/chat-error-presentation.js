...
const STATUS_PRESENTATIONS = new Map([
    [400, {
...
function createPresentation(code, message, statusCode = null, resetConversation = false) {
    return { code, message, statusCode, resetConversation };
}
export function readChatErrorMetadata(error) {
    return {
...
export function getChatErrorPresentation(error) {
    ...
    if (code && CONNECTION_CODES.has(code)) {
        return createPresentation('provider_connection_failed', '无法连接 AI 服务，请稍后重试', statusCode);
    }
    return createPresentation('provider_unknown_error', '处理请求时出现异常，请稍后重试', statusCode);
}
