...
function buildRecord(input, serializedBytes) {
    return Object.freeze({
        schemaVersion: 1,
        runRef: input.runRef,
...
function parseRecord(value, allowUnknownOptional) {
    ...
    const events = Object.freeze(internalExactTraceArray(input.events, 512, 'trace record events').map(event => internalParseTraceEvent(event, allowUnknownOptional)));
    ...
    if (presentation.kind === 'observed' && (presentation.value.runRef !== input.runRef ||
...
export function parseTraceRecord(value) {
    return parseRecord(value, false);
}
export function parseStoredTraceRecord(value) {
    return parseRecord(value, true);
}
function joinPresentation(runRef, observationId, value) {
...
function markerOf(events) {
    return events.find((event) => (event.type === 'trace_truncated')) ?? null;
}
export function mergeTracePresentation(input) {
    const candidate = parseTraceCandidate(input.candidate);
    const presentation = joinPresentation(candidate.runRef, candidate.observationId, input.presentation);
    const observed = Object.freeze({ kind: 'observed', value: presentation });
    const marker = markerOf(candidate.events);
    const source = candidate.events.filter((event) => event.type !== 'trace_truncated');
...
export function mergeStoredTracePresentation(input) {
    ...
    if (record.presentation.kind === 'observed') {
        if (JSON.stringify(record.presentation.value) === JSON.stringify(presentation))
            return record;
        throw new TypeError('trace presentation already exists');
    }
    const observed = Object.freeze({ kind: 'observed', value: presentation });
    const marker = markerOf(record.events);
    const source = record.events.filter((event) => event.type !== 'trace_truncated');
    const selected = internalSelectTraceEvents({
        events: source,
