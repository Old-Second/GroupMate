...
export const SESSION_PERSISTENCE_FAILED_MESSAGE = '本轮结果已生成，但对话记录保存失败，后续对话可能无法延续本轮上下文。';
function containsBlockedWord(candidate, words) {
    ...
    for (const word of words) {
        ...
        if (normalizedWord !== '' && normalizedCandidate.includes(normalizedWord))
            return true;
    }
    return false;
}
export function promptIsBlocked(prompt, words) {
    return containsBlockedWord(prompt, words);
}
export function responseIsBlocked(response, words) {
