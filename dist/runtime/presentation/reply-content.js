...
function isDoubleEmojiControl(value) {
...
function controlSuggestion(value) {
    const lower = value.toLowerCase();
    if (value.startsWith('#') || value === '确认' || value === '拒绝')
        return true;
    if (EXACT_CONTROL_SUGGESTIONS.has(value))
        return true;
...
export function normalizeCitationForwards(values) {
    const result = [];
...
export function normalizeSuggestions(values) {
    const result = [];
    const seen = new Set();
    for (const value of values) {
        if (result.length >= 6)
        ...
        if (controlSuggestion(bounded) || seen.has(bounded))
            continue;
        seen.add(bounded);
        result.push(bounded);
    }
...
export function buildChatSuggestionButtonRequest(suggestions) {
...
export function normalizeReasoningView(value) {
