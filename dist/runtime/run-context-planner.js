...
import { MAX_CONTEXT_PLANNER_INPUT_BYTES, planModelTurn as planContextModelTurn } from '../agent/context/context-planner.js';
import { createContextSpanV1, domainSeparatedContextHash, MAX_CONTEXT_SPANS, parseContextSpanV1 } from '../agent/context/context-span.js';
...
import { toolLedgerIsTerminal } from '../agent/run/tool-ledger.js';
...
function contextBudgetError(reason) {
    return new AgentError({
...
function sameCallIds(message, ledger) {
    if (message?.role !== 'assistant' || message.toolCalls === undefined ||
        message.toolCalls.length !== ledger.calls.length)
        return false;
    return message.toolCalls.every((call, index) => call.callId === ledger.calls[index]?.callId);
}
function protocolMessages(messages, ledger) {
    for (const [index, message] of messages.entries()) {
        if (!sameCallIds(message, ledger))
            continue;
        const protocol = messages.slice(index, index + ledger.calls.length + 1);
        if (protocol.length !== ledger.calls.length + 1 || protocol.slice(1).some((result, offset) => (result.role !== 'tool' || result.toolCallId !== ledger.calls[offset]?.callId)))
            continue;
        return Object.freeze(protocol);
    }
    return null;
}
function toolSpan(checkpoint, ledger, messages, phase, originGeneration) {
    ...
    return createContextSpanV1(Object.freeze({
        ...
        source: 'tool_chain',
        trust: 'trusted',
        requirement: phase === 'ready' ? 'mandatory' : 'optional',
        priority: phase === 'ready' ? 'critical' : 'normal',
        ...
        provenance: Object.freeze({
            ...
            contentHash: domainSeparatedContextHash(TOOL_SPAN_PROVENANCE_HASH_DOMAIN, `${identity}\0${wireHash}`)
        }),
        supersedes: null,
        messages,
...
export function projectRunToolProtocolSpans(checkpoint, namespaceRef, signal, originGenerations) {
    assertNotAborted(signal);
    if (checkpoint.runRef !== namespaceRef)
    ...
    for (const [index, ledger] of checkpoint.toolLedgers.entries()) {
        ...
        if (!toolLedgerIsTerminal(ledger)) {
            if (ready)
                throw checkpointError('pending_tool_ledger_not_terminal');
            continue;
        }
        const messages = protocolMessages(combined, ledger);
        if (messages === null) {
            if (ready)
                throw checkpointError('pending_tool_protocol_incomplete');
            continue;
        }
        ...
        const inferredOriginGeneration = Math.max(ledger.step + 1, ready ? planGeneration : planGeneration - generationsSinceLedger);
        ...
        spans.push(toolSpan(checkpoint, ledger, messages, ready ? 'ready' : 'consumed', originGeneration));
    }
...
function recoveryBaselineSpan(checkpoint) {
    ...
    return createContextSpanV1(Object.freeze({
        spanId: `recovery-baseline:${domainSeparatedContextHash(RECOVERY_BASELINE_ID_HASH_DOMAIN, previous.planHash)}`,
        namespaceRef: checkpoint.runRef,
        kind: 'message',
...
function correctionSpan(checkpoint, generation, semanticOrder) {
    ...
    const contentHash = domainSeparatedContextHash(CORRECTION_SPAN_CONTENT_HASH_DOMAIN, CORRECTION_INSTRUCTION);
    return createContextSpanV1(Object.freeze({
...
function plannerResultError(code) {
    if (code === 'context_budget_exceeded')
        return contextBudgetError(code);
    return new AgentError({
...
function evidenceFor(checkpoint, span) {
    const protocol = span.toolProtocol;
    if (protocol === null || protocol.phase !== 'consumed')
        return null;
    ...
    if (ledger === undefined || !toolLedgerIsTerminal(ledger) ||
        ledger.calls.length !== protocol.callIds.length)
        return null;
    const calls = ledger.calls.map((call, index) => {
        if (call.callId !== protocol.callIds[index] || call.result === null)
            return null;
        return Object.freeze({
            callId: call.callId,
...
function sourceSpanForRequest(spans, request) {
    if (request.sourceSpanIds.length !== 1)
        return null;
...
async function storeArtifact(store, artifact, minimumExpiresAtMs) {
    try {
...
async function hydrateCommittedArtifacts(refs, artifacts, store) {
    for (const ref of refs) {
        ...
        if (result.status === 'ready') {
            ...
            if (artifact.artifactId !== ref)
                throw checkpointError('artifact_identity_mismatch');
...
export function createRunContextPlanner(options) {
    const initialSpans = options.initialSpans === null
        ? null
        : Object.freeze(options.initialSpans.map(parseContextSpanV1));
    const artifactStore = options.artifactStore;
    const artifactPolicy = options.artifactPolicy ?? (artifactStore === undefined ? 'disabled' : 'enabled');
    const artifacts = new Map();
    const artifactSpans = new Map();
    for (const value of options.artifacts ?? []) {
        const artifact = parseContextArtifactV1(value);
        if (artifacts.has(artifact.artifactId))
            throw new TypeError('duplicate context artifact');
        artifacts.set(artifact.artifactId, artifact);
    }
    const toolOriginGenerations = new Map();
    let fixedRecoveryBaseline = null;
    let baselineCallIds = new Set();
    return Object.freeze({
        async planModelTurn(checkpoint, request, signal) {
            assertNotAborted(signal);
            ...
            const hydration = await hydrateCommittedArtifacts(checkpoint.contextArtifactRefs ?? Object.freeze([]), artifacts, artifactStore);
            assertNotAborted(signal);
            ...
            if (needsRecoveryBaseline && fixedRecoveryBaseline === null) {
                ...
                baselineCallIds = new Set(fixedRecoveryBaseline.messages.flatMap(message => (message.role === 'assistant' && message.toolCalls !== undefined
                    ? message.toolCalls.map(call => call.callId)
                    : [])));
            }
            const useRecoveryBaseline = fixedRecoveryBaseline !== null;
            const projectedToolSpans = projectRunToolProtocolSpans(checkpoint, options.namespaceRef, signal, toolOriginGenerations);
            let spans;
            if (useRecoveryBaseline) {
                ...
                spans = [
                    fixedRecoveryBaseline,
                    ...projectedToolSpans.filter(span => span.toolProtocol?.callIds.every(callId => !baselineCallIds.has(callId)))
                ];
            }
            else {
                if (initialSpans === null)
                    throw checkpointError('initial_spans_unavailable');
                ...
                const retainedArtifacts = (checkpoint.contextArtifactRefs ?? []).map(ref => {
                    ...
                    if (artifact === undefined)
                        throw checkpointError('artifact_not_hydrated');
                    ...
                    const materialized = contextArtifactToSpan(artifact, Math.min(...covered.map(span => span.semanticOrder)));
                    artifactSpans.set(ref, materialized);
                    return materialized;
                });
                const coveredSpanIds = new Set(retainedArtifacts.flatMap(span => (artifacts.get(span.spanId)?.sourceSpanIds ?? [])));
                spans = [
            ...
            if (result.status === 'requires_artifacts') {
                ...
                if (artifactPolicy !== 'enabled' || artifactStore === undefined) {
                    ...
                    for (const compactionRequest of result.compactionRequests) {
                        ...
                        const stored = await storeArtifact(artifactStore, compacted.artifact, minimumExpiresAtMs);
                        assertNotAborted(signal);
                        if (stored === null) {
                            storedAll = false;
                            break;
            ...
            if (result.status === 'blocked')
                throw plannerResultError(result.code);
            if (request.kind === 'correction') {
                ...
                if (result.messages.length !== expectedPrefix.length + 1 ||
                    ...
                    appended?.role !== 'user' || !contextArtifactRefsRetainedOrCleared(checkpoint.contextArtifactRefs ?? Object.freeze([]), result.plan.artifactRefs)) {
