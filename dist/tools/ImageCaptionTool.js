...
export function createImageCaptionTool(options) {
    ...
    return readOnlyDefinition({
        ...
        execute: async (input, context) => {
            ...
            if (isToolResult(image))
                return image;
            if (image.status < 200 || image.status >= 300)
                return upstreamFailure('图片暂时无法读取。');
            ...
            if (isToolResult(caption))
                return caption;
            if (caption.status < 200 || caption.status >= 300)
                return upstreamFailure('图片识别服务暂时不可用。');
