...
export function createSearchMusicTool(options) {
    ...
    return readOnlyDefinition({
        ...
        execute: async (input, context) => {
            ...
            if (keyword === '')
                return invalidArguments('音乐搜索关键词不能为空。');
            ...
            const songs = result !== null && typeof result === 'object' && Array.isArray(result.songs)
                ? result.songs : [];
            if (songs.length === 0)
                return upstreamFailure('音乐搜索暂时没有结果。', false);
            return textResult(boundedJson(songs.slice(0, limit).map(item => {
                const song = item !== null && typeof item === 'object' ? item : {};
                ...
                return {
                    ...
                    artists: artists.map(artist => artist !== null && typeof artist === 'object'
                        ? boundedText(artist.name, 100) : '').filter(Boolean),
